...
extension View {
  ...
  func announceOnChange<V: Equatable>(_ value: V, announcement: @escaping (V) -> String)
    -> some View
  {
...
struct MotionSensitiveView<Content: View>: View {
  ...
  @ViewBuilder let fullMotion: Content
  @ViewBuilder let reducedMotion: Content
...
extension View {
  ...
  func highContrastAware(normalColor: Color, highContrastColor: Color) -> some View {
    modifier(
      HighContrastColorModifier(normalColor: normalColor, highContrastColor: highContrastColor))
  }
}
...
extension View {
  func accessibilityRotorActions(_ actions: [(String, () -> Void)]) -> some View {
    accessibilityCustomContent(
      actions.map { name, action in
        AccessibilityCustomContentKey(name) { action() }
      })
  }

  private func accessibilityCustomContent(_: [AccessibilityCustomContentKey]) -> some View {
