...
class ProgressTracker: ObservableObject {
  ...
  @Published private(set) var stats: ProgressStats = .init()
  ...
  func recordDailyActivity() {
    ...
    if let lastActive = stats.lastActiveDate {
      ...
      let daysSinceActive =
        Calendar.current.dateComponents([.day], from: lastActiveDay, to: today).day ?? 0
...
struct JourneyProgressView: View {
    ...
    VStack(spacing: 16) {
      ...
      HStack(spacing: 16) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          ...
          Text(
            "\(badgeManager.uniqueShapesEarned().count)/\(SacredGeometryShape.allCases.count) unique shapes"
          )
          .font(.system(size: 10))
        ...
        ForEach(0..<7, id: \.self) { index in
          ...
          VStack(spacing: 4) {
            RoundedRectangle(cornerRadius: 4)
              .fill(
                index == Calendar.current.component(.weekday, from: Date()) - 1
  ...
  private var statsGrid: some View {
    LazyVGrid(
      ...
      ProgressStatCard(
        title: "Courses Started", value: "\(tracker.stats.coursesStarted)", icon: "book.fill",
        color: Palette.accent.gold)
      ProgressStatCard(
        title: "Courses Completed", value: "\(tracker.stats.coursesCompleted)",
        icon: "checkmark.seal.fill", color: .green)
      ProgressStatCard(
        title: "Lessons Done", value: "\(tracker.stats.lessonsCompleted)", icon: "list.bullet",
        color: Palette.primary.cyan)
      ProgressStatCard(
        title: "Practices", value: "\(tracker.stats.practicesCompleted)",
        icon: "figure.mind.and.body", color: .purple)
      ProgressStatCard(
        title: "Cosmos Explored", value: "\(tracker.stats.cosmosExplored)",
        icon: "globe.americas.fill", color: Palette.primary.violet)
      ProgressStatCard(
        title: "Dragon Talks", value: "\(tracker.stats.dragonInteractions)", icon: "flame.fill",
        color: .red)
    }
  }
