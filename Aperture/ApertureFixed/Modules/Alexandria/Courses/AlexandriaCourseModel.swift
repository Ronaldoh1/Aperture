...
struct AlexandriaCourse: Identifiable {
    ...
    var lessonCount: Int { modules.reduce(0) { $0 + $1.lessons.count } }
    var estimatedHours: Double { Double(lessonCount) * 0.25 }
}
...
extension ACModule: Equatable {
    static func == (lhs: ACModule, rhs: ACModule) -> Bool { lhs.id == rhs.id }
}
...
struct ACLesson: Identifiable {
    ...
    init(id: String, title: String, content: String, keyInsight: String, practice: String, interactiveElement: InteractiveElement? = nil) {
        self.id = id
        self.title = title
...
extension ACLesson: Equatable {
    static func == (lhs: ACLesson, rhs: ACLesson) -> Bool { lhs.id == rhs.id }
}
...
struct AlexandriaCourseRegistry {
