    ...
    VStack(spacing: 16) {
      ...
      VStack(spacing: 8) {
        ...
        Text(godModeManager.isUnlocked ? "The Monad Awakens" : "Open Your Eyes")
          ...
          .foregroundColor(
            godModeManager.isUnlocked
              ? Color(red: 1, green: 0.84, blue: 0).opacity(0.8) : Palette.text.secondary
          )
          .tracking(2)
      }
      ...
      if godModeManager.tapCount > 0 && !godModeManager.isUnlocked {
        HStack(spacing: 8) {
          ForEach(0..<3, id: \.self) { index in
            Circle()
              .fill(
                index < godModeManager.tapCount
                  ? Color(red: 1, green: 0.84, blue: 0) : Color.white.opacity(0.2)
              )
