...
final class ChronokeeperEngine: ObservableObject {
  ...
  private init() {
    ...
    if messages.isEmpty {
      let welcome =
        ChronokeeperPersona.welcomeMessages.randomElement()
        ?? ChronokeeperPersona.welcomeMessages[0]
  ...
  func send(_ text: String, readAloud: Bool = false) async {
    ...
    if let courseResponse = handleCourseQuery(trimmed) {
      await deliverResponse(
        courseResponse.text, readAloud: readAloud, courseCard: courseResponse.card)
      return
    }

    // Try "continue journey" response
    if trimmed.lowercased().contains("left off") || trimmed.lowercased().contains("continue")
      || trimmed.lowercased().contains("what's next")
    {
  ...
  private func deliverResponse(
    _ text: String, readAloud: Bool, courseCard: ChronokeeperCourseCard? = nil
  ) async {
  ...
  private func callClaudeAPI(userText: String, readAloud: Bool) async {
    ...
    guard let url = URL(string: "https://api.anthropic.com/v1/messages") else {
      await deliverResponse(
        "The Library's connection seems disrupted. Try again in a moment, seeker.",
        readAloud: readAloud)
      return
    }
    ...
    do {
      ...
      guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
        throw NSError(
          domain: "Chronokeeper", code: 1,
          userInfo: [NSLocalizedDescriptionKey: "API returned non-200"])
      }
      ...
      if responseText.isEmpty {
        throw NSError(
          domain: "Chronokeeper", code: 2, userInfo: [NSLocalizedDescriptionKey: "Empty response"])
      }
  ...
  private func handleCourseQuery(_ input: String) -> CourseQueryResult? {
    ...
    for course in allCourses {
      ...
      if lowered.contains(titleLower) || lowered.contains(idLower) {
        ...
        if percentage > 0, percentage < 1.0 {
          text =
            "You're \(Int(percentage * 100))% through \(course.title). \(course.dragonComment) Want me to read you the next lesson, or give you a summary of where you are?"
        } else if percentage >= 1.0 {
          text =
            "You've completed \(course.title) — well done, seeker. \(course.dragonComment) Want to revisit any lesson, or shall I suggest what to explore next?"
        } else {
          text =
            "\(course.title) — \(course.subtitle). This one has \(course.moduleCount) modules and \(course.lessonCount) lessons. \(course.dragonComment) Want me to walk you through the first lesson?"
        }
    ...
    if lowered.contains("course") || lowered.contains("what can i learn")
      || lowered.contains("what should i") || lowered.contains("recommend")
    {
      ...
      if started.isEmpty {
        return CourseQueryResult(
          text:
            "For someone just beginning, I'd recommend starting with Christ Consciousness — it reveals what Yeshua actually taught before they rewrote the story. Or if you want to understand how the world really works, Guilt and Power shows you how institutions weaponized shame for millennia. Both are eye-openers. Which speaks to you?",
          card: nil
        )
      } else {
        ...
        return CourseQueryResult(
          text:
            "You've started \(started.count) courses and completed \(completedCount). Based on your journey so far, I'd suggest exploring what you haven't touched yet. Want me to list what's waiting for you in the Library?",
          card: nil
        )
      }
    }
  ...
  private func buildContinueResponse() -> String {
    ...
    if let mostRecent = allProgress.values.max(by: { $0.lastAccessedAt < $1.lastAccessedAt }) {
      ...
      return
        "You left off in \(courseName) — Module \(moduleNum), Lesson \(lessonNum). You've completed \(mostRecent.completedLessonIds.count) lessons so far. Want me to read you the next one, or give you a quick recap of where we were?"
    }

    return
      "Looks like you're at the beginning of the journey, seeker. That's the best place to be — everything is about to change. Want me to recommend a starting course based on what interests you?"
  }
  ...
  private func buildFreeTierResponse(for input: String) -> String {
    ...
    if lowered.contains("cosmos") || lowered.contains("gnostic") || lowered.contains("archon")
      || lowered.contains("sophia")
    {
      return
        "The Cosmos module maps gnostic cosmology — the Pleroma (the fullness), the Aeons (divine emanations), and how Sophia's curiosity created the material world. The Archons are the rulers of this realm who feed on ignorance. But here's the key: you carry a divine spark that's beyond their reach. The Cosmos module walks you through all of it visually."
    }

    if lowered.contains("scan") || lowered.contains("food") || lowered.contains("ingredient")
      || lowered.contains("chemical")
    {
      return
        "Quantum Nourish is your food truth detector. Scan any label with the Vibe Scanner and I'll tell you what's really in it. The Kill List tracks 8 chemicals hiding under dozens of aliases — the ones making people sick while corporations profit. Want me to walk you through the worst offenders?"
    }

    if lowered.contains("hello") || lowered.contains("hi") || lowered.contains("hey")
      || lowered.contains("sup")
    {
      return ChronokeeperPersona.welcomeMessages.randomElement()
        ?? "Greetings, seeker. What truth are you seeking today?"
    }

    if lowered.contains("read") || lowered.contains("listen") || lowered.contains("speak")
      || lowered.contains("talk")
    {
      return
        "I can read any lesson to you — just tell me which course or topic you're interested in. If you're not sure where to start, I'll pick something powerful. Just say the word."
    }

    // Default
    return
      "That's a deep question, seeker. For the full Chronokeeper experience — where I can dive deep into any topic and walk you through lessons conversationally — unlock Premium. But I can still guide you through the courses, tell you about the Timeline, or explain what's on the Kill List. What interests you?"
  }
  ...
  private func loadConversation() {
    guard let data = UserDefaults.standard.data(forKey: conversationKey),
      let convo = try? JSONDecoder().decode(SavedConversation.self, from: data)
    else { return }
  ...
  func clearConversation() {
    ...
    // Re-add welcome
    let welcome =
      ChronokeeperPersona.welcomeMessages.randomElement() ?? ChronokeeperPersona.welcomeMessages[0]
    messages.append(
      ChronokeeperMessage(
        role: .assistant,
        content: welcome,
