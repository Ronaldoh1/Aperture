  ...
  private var vibeMapCard: some View {
    VStack(spacing: 16) {
      ...
      ZStack {
        ...
        Circle()
          .fill(
            RadialGradient(
              colors: [vibeColor.opacity(0.3), Color.clear], center: .center, startRadius: 0,
              endRadius: 60)
          )
          .frame(width: 120, height: 120)
      ...
      if !topFoods.isEmpty {
        VStack(spacing: 6) {
          Text("TODAY'S FUEL").font(.system(size: 8, weight: .bold)).tracking(2).foregroundColor(
            Palette.text.muted)
    ...
    .background(
      RoundedRectangle(cornerRadius: 20)
        .fill(
          LinearGradient(
            colors: [Color(hex: "#0a0a14"), Color(hex: "#0d1117")], startPoint: .top,
            endPoint: .bottom)
        )
...
struct QuantumNourishWidgetData: Codable {
  ...
  static func load() -> QuantumNourishWidgetData {
    guard let defaults = UserDefaults(suiteName: suiteName),
      ...
      let decoded = try? JSONDecoder().decode(QuantumNourishWidgetData.self, from: data)
    else {
      return QuantumNourishWidgetData(
        dailyMhz: 0, streak: 0, lastMeal: "No meals logged", lastMealMhz: 0, timestamp: Date())
    }
    return decoded
  }
}
...
enum VibeCheckEngine {
  ...
  static func vibeCheck(for query: String) -> String {
    ...
    guard let food = results.first else {
      return
        "I don't have frequency data for '\(query)' yet. Try scanning the ingredient label for a full analysis."
    }

    var response =
      "\(food.name) is in the \(food.tier.rawValue) tier at approximately \(food.mhzEstimate) megahertz. "
    ...
    case .mediumLow, .mediumHigh:
      response +=
        "Good choice. \(food.effects.first ?? "") This supports your consciousness hardware."
    case .high, .superHigh:
      response +=
        "Excellent. This is premium consciousness fuel. \(food.effects.first ?? "") Keep it up."
    }
