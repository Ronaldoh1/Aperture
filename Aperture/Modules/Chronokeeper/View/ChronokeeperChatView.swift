...
struct ChronokeeperChatView: View {
    ...
    private var inputBar: some View {
        VStack(spacing: 0) {
            ...
            HStack(alignment: .bottom, spacing: 10) {
                ...
                .disabled(inputText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || engine.chatState == .thinking)
            }
            .padding(.horizontal, 14)
            .padding(.vertical, 10)
    ...
    private func handleAction(_ action: ChronokeeperAction) {
        ...
        case .openCourse(let id):
            ...
            NotificationCenter.default.post(name: .chronokeeperNavigate, object: nil, userInfo: ["type": "course", "id": id])
        case .openModule(let id):
            NotificationCenter.default.post(name: .chronokeeperNavigate, object: nil, userInfo: ["type": "module", "id": id])
        }
    }
}
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            Text("In production, this proxies through your backend. For development, enter your key directly.")
                            ...
                            if showKeyField {
                                    ...
                                    SecureField("sk-ant-...", text: $apiKeyInput)
                                        ...
                                        .background(
                                            RoundedRectangle(cornerRadius: 10)
                                                .fill(Color.white.opacity(0.05))
                                                .overlay(RoundedRectangle(cornerRadius: 10).stroke(Palette.primary.cyan.opacity(0.2), lineWidth: 1))
                                        )
                                ...
                                Button(action: { showKeyField = true }) {
                                    ...
                                    .background(
                                        RoundedRectangle(cornerRadius: 10)
                                            .fill(Color.white.opacity(0.04))
                                            .overlay(RoundedRectangle(cornerRadius: 10).stroke(Palette.primary.cyan.opacity(0.15), lineWidth: 1))
                                    )
                                }
                            }
                        }
                        ...
                        HStack(spacing: 8) {
                            ...
                            Text("Conversations are stored locally on your device. When using the AI, your messages are sent to Claude's API for processing — never stored on their servers.")
                                .font(.system(size: 11, weight: .medium))
                                .foregroundColor(Palette.text.muted)
                                .lineSpacing(2)
                        }
