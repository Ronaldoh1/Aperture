...
struct ProgrammingDetectorView: View {
        ...
        VStack(spacing: 8) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 14).stroke(Color(hex: "#E74C3C").opacity(0.3), lineWidth: 1))
            )
        }
            ...
            TextEditor(text: $scannedText)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color(hex: "#E74C3C").opacity(0.3), lineWidth: 1))
                )
    ...
    private func threatHeader(_ result: ProgrammingScanResult) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay(RoundedRectangle(cornerRadius: 16).stroke(result.threatLevel.displayColor.opacity(0.3), lineWidth: 1))
        )
    }
    ...
    private func scannedTextCard(_ result: ProgrammingScanResult) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                ...
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(Palette.primary.cyan.opacity(0.15), lineWidth: 1))
        )
    }
            ...
            Text(result.decodedMessage)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 12).stroke(Palette.accent.gold.opacity(0.2), lineWidth: 1))
                )
        }
    }
    ...
    private func psychEffectsSection(_ result: ProgrammingScanResult) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                ...
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(Palette.primary.violet.opacity(0.15), lineWidth: 1))
        )
    }
    ...
    private func buildPatterns(from result: PsyopScanResult) -> [ProgrammingPattern] {
        ...
        if result.fearScore > 0.3 {
            patterns.append(ProgrammingPattern(
                ...
                explanation: "Uses fear-based language to bypass rational thinking and trigger survival instincts."
            ))
        }
        ...
        if result.deceptionDetected {
            patterns.append(ProgrammingPattern(
                ...
                explanation: "Vague attribution ('experts say', 'sources confirm') prevents you from verifying claims."
            ))
        }
        ...
        if patterns.isEmpty {
            patterns.append(ProgrammingPattern(
                ...
                explanation: "Even low-level text can contain framing that shapes your perception without obvious triggers."
            ))
        }
    ...
    private func buildDecodedMessage(from result: PsyopScanResult) -> String {
        if result.psyopScore >= 7 {
            return "This text is heavily engineered to manipulate your emotional state. It wants you to react, not think. The programming is designed to bypass your rational mind and create an automatic response."
        } else if result.psyopScore >= 4 {
            return "Moderate manipulation detected. This text uses common programming techniques — emotional language, vague sourcing, or artificial urgency — to shape how you feel about the topic before you've had time to think."
        } else {
            return "Low-level programming. While not aggressively manipulative, the framing still guides your perception. Notice how the word choices subtly push you toward a specific emotional response."
        }
    }
    ...
    class Coordinator: NSObject, DataScannerViewControllerDelegate {
        ...
        private func processItems(_ items: [RecognizedItem]) {
            ...
            guard combined != lastProcessedText,
                  combined.split(separator: " ").count >= 5 else { return }
