...
struct YourPathView: View {
    ...
    private func generateInsight() -> String {
        ...
        if streak >= 7 {
            ...
            return "Based on your interest in \(interests.first ?? "learning"), we've curated recommendations for you."
        }
    }
    ...
    private var insightCards: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 12) {
                InsightCard(icon: "flame.fill", value: "\(progress.stats.currentStreak)", label: "Day Streak", color: .orange)
                InsightCard(icon: "checkmark.circle.fill", value: "\(progress.stats.lessonsCompleted)", label: "Lessons Done", color: .green)
                InsightCard(icon: "speedometer", value: analytics.getRecommendedDifficulty(), label: "Your Level", color: Palette.primary.cyan)
                InsightCard(icon: engagementIcon, value: analytics.getEngagementLevel(), label: "Engagement", color: engagementColor)
            }
        ...
        return Group {
            if !inProgress.isEmpty {
                VStack(alignment: .leading, spacing: 14) {
                    SectionHeader(icon: "arrow.right.circle.fill", title: "CONTINUE", color: Palette.accent.gold)
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 16).stroke(recommendation.color.opacity(0.3), lineWidth: 1))
            )
        }
        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 14).stroke(recommendation.color.opacity(0.2), lineWidth: 1))
            )
        }
        .buttonStyle(ScaleButtonStyle())
    }
