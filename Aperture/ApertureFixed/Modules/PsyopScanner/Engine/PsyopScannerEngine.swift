...
final class PsyopScannerEngine {
    ...
    func analyze(text: String) -> PsyopScanResult {
        ...
        let effects = generatePsychEffects(fearScore: fearScore, psyopScore: psyopScore, deception: deceptionDetected, patterns: patterns)
    ...
    private func calculatePsyopScore(text: String) -> Int {
        ...
        score += emotionalTriggers.reduce(0) { count, trigger in count + (text.contains(trigger) ? 1 : 0) }
        score += outrageBaitPatterns.reduce(0) { count, pattern in count + (text.contains(pattern) ? 2 : 0) }
        return min(score, 10)
    }
    ...
    private func detectPatterns(text: String) -> [ManipulationPatternResult] {
        ...
        if !fearMatches.isEmpty {
            patterns.append(ManipulationPatternResult(type: .fearMongering, matchedPhrases: fearMatches, explanation: "Uses fear-inducing words to trigger your fight-or-flight response."))
        }
        
        let sourceMatches = vagueSourcePatterns.filter { text.contains($0) }
        if !sourceMatches.isEmpty {
            patterns.append(ManipulationPatternResult(type: .vagueSources, matchedPhrases: sourceMatches, explanation: "Attributes claims to unverifiable sources."))
        }
        
        let divisionMatches = divisionPatterns.filter { text.contains($0) }
        if !divisionMatches.isEmpty {
            patterns.append(ManipulationPatternResult(type: .divisionTactics, matchedPhrases: divisionMatches, explanation: "Creates us-vs-them framing to trigger tribal instincts."))
        }
        
        let outrageMatches = outrageBaitPatterns.filter { text.contains($0) }
        if !outrageMatches.isEmpty {
            patterns.append(ManipulationPatternResult(type: .outrageBait, matchedPhrases: outrageMatches, explanation: "Classic outrage bait designed to make you angry and share."))
        }
        
        return patterns
    }
    
    private func generatePsychEffects(fearScore: Double, psyopScore: Int, deception: Bool, patterns: [ManipulationPatternResult]) -> [PsychEffect] {
        var effects: [PsychEffect] = []
        
        if fearScore > 0.3 {
            effects.append(PsychEffect(effect: "Anxiety activation — nervous system may shift into fight-or-flight", severity: fearScore > 0.6 ? .severe : .moderate, icon: "bolt.heart.fill", bodyResponse: "Elevated heart rate, shallow breathing"))
        }
        
        if patterns.contains(where: { $0.type == .outrageBait || $0.type == .divisionTactics }) {
            effects.append(PsychEffect(effect: "Rage trigger — designed to hijack your prefrontal cortex with anger", severity: .severe, icon: "flame.fill", bodyResponse: "Blood pressure spike, urge to react"))
        }
        
        if deception {
            effects.append(PsychEffect(effect: "Confusion induction — contradictory claims create cognitive dissonance", severity: .moderate, icon: "questionmark.circle.fill", bodyResponse: "Mental fog, difficulty focusing"))
        }
        
        if effects.isEmpty {
            effects.append(PsychEffect(effect: "Minimal emotional manipulation detected", severity: .mild, icon: "checkmark.circle.fill", bodyResponse: "Your nervous system appears unaffected"))
        }
    ...
    private func generateCalmOptions(threatLevel: ThreatLevel) -> [CalmOption] {
        ...
        if threatLevel == .high {
            options.append(CalmOption(type: .breathe, title: "Breathe First", description: "Your nervous system needs 90 seconds to reset.", duration: 90, isInteractive: true))
            options.append(CalmOption(type: .bodyCheck, title: "Body Scan", description: "Notice where you're holding tension.", duration: 30, isInteractive: true))
        }
        
        if threatLevel == .medium {
            options.append(CalmOption(type: .breathe, title: "3 Deep Breaths", description: "Activate your parasympathetic nervous system.", duration: 30, isInteractive: true))
        }
        
        if threatLevel != .low {
            options.append(CalmOption(type: .groundYourself, title: "5-4-3-2-1 Grounding", description: "Reconnect with the present moment.", duration: 60, isInteractive: true))
        }
        
        options.append(CalmOption(type: .waitAndRevisit, title: "Wait 24 Hours", description: "If it still matters tomorrow, engage with a clear head.", duration: nil, isInteractive: false))
        options.append(CalmOption(type: .verify, title: "Verify Before Believing", description: "Cross-check with 3+ independent sources.", duration: nil, isInteractive: false))
        options.append(CalmOption(type: .ignore, title: "Choose Peace", description: "You are not obligated to engage.", duration: nil, isInteractive: false))
        
        return options
    }
    
    private func calculateThreatLevel(_ fearScore: Double, _ psyopScore: Int, _ deception: Bool) -> ThreatLevel {
