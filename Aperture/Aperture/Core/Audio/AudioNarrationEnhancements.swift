...
class AudioNarrationController: ObservableObject {
  ...
  func updateNowPlaying() {
    ...
    info[MPMediaItemPropertyTitle] =
      currentSectionTitle.isEmpty ? "SunFlow: Aperture" : currentSectionTitle
    info[MPMediaItemPropertyArtist] =
      currentCourseTitle.isEmpty ? "SunFlow: Reignited" : currentCourseTitle
    info[MPMediaItemPropertyAlbumTitle] = "Aperture Audio"
    info[MPNowPlayingInfoPropertyPlaybackRate] =
      TextToSpeechManager.shared.isSpeaking && !TextToSpeechManager.shared.isPaused
      ? Double(playbackSpeed.rate) : 0.0
  ...
  func addBookmark(
    courseId: String, courseTitle: String, sectionTitle: String, paragraphIndex: Int,
    textSnippet: String
  ) {
...
struct AudioBookmark: Identifiable, Codable {
  ...
  init(
    courseId: String, courseTitle: String, sectionTitle: String, paragraphIndex: Int,
    textSnippet: String, timestamp: Date
  ) {
    id = UUID().uuidString
      ...
      VStack(spacing: 0) {
        ...
        GeometryReader { geo in
          ZStack(alignment: .leading) {
            ...
            Rectangle().fill(Color(hex: "#2ECC71")).frame(
              width: geo.size.width * tts.progress, height: 3)
          }
        }.frame(height: 3)
        ...
        HStack(spacing: 14) {
          ...
          Button {
            ...
            Image(systemName: "gobackward.15").font(.system(size: 18)).foregroundColor(
          ...
          Button {
            ...
            Image(systemName: "goforward.15").font(.system(size: 18)).foregroundColor(
              Palette.text.primary)
          }.buttonStyle(PlainButtonStyle())
          ...
          Button {
            ...
            Image(systemName: "xmark.circle").font(.system(size: 18)).foregroundColor(
              Palette.text.muted)
          }.buttonStyle(PlainButtonStyle())
        }
...
struct AudioBookmarksView: View {
    ...
    NavigationStack {
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 16) {
            VStack(spacing: 8) {
              Image(systemName: "bookmark.fill").font(.system(size: 36)).foregroundColor(
                Color(hex: "#FFD700"))
              Text("AUDIO BOOKMARKS").font(.system(size: 10, weight: .bold)).tracking(3)
                .foregroundColor(Color(hex: "#FFD700"))
              Text("\(controller.bookmarks.count) saved").font(.system(size: 13)).foregroundColor(
                Palette.text.muted)
            }.padding(.top, 16)

            if controller.bookmarks.isEmpty {
              VStack(spacing: 12) {
                Image(systemName: "bookmark.slash").font(.system(size: 40)).foregroundColor(
                  Palette.text.muted)
                Text("No bookmarks yet").font(.system(size: 16, weight: .bold)).foregroundColor(
                  Palette.text.muted)
      ...
      .toolbar {
        ...
        ToolbarItem(placement: .principal) {
          Text("BOOKMARKS").font(.system(size: 14, weight: .bold, design: .rounded)).tracking(2)
            .foregroundColor(Color(hex: "#FFD700"))
        }
      }
    }
  ...
  private func bookmarkCard(_ bookmark: AudioBookmark) -> some View {
    HStack(spacing: 12) {
      Image(systemName: "bookmark.fill").font(.system(size: 18)).foregroundColor(
        Color(hex: "#FFD700"))
      VStack(alignment: .leading, spacing: 4) {
        Text(bookmark.courseTitle).font(.system(size: 12, weight: .bold, design: .rounded))
          .foregroundColor(Color(hex: "#FFD700"))
        Text(bookmark.sectionTitle).font(.system(size: 13, weight: .medium)).foregroundColor(
          Palette.text.primary)
        Text(bookmark.textSnippet).font(.system(size: 11)).foregroundColor(Palette.text.muted)
          .lineLimit(2)
        Text(bookmark.timestamp, style: .relative).font(.system(size: 10)).foregroundColor(
          Palette.text.muted)
      }
      Spacer()
      VStack(spacing: 8) {
        Button {
          ...
          Image(systemName: "play.circle.fill").font(.system(size: 28)).foregroundColor(
            Color(hex: "#2ECC71"))
        }.buttonStyle(PlainButtonStyle())
    ...
    .padding(14)
    .background(
      RoundedRectangle(cornerRadius: 14).fill(Color(hex: "#FFD700").opacity(0.04))
        .overlay(
