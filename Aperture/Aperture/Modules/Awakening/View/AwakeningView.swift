...
struct AwakeningView: View {
  ...
  private var presenter: AwakeningPresenterType {
    ...
    return VStack(spacing: 16) {
      ZStack {
        Circle()
          .fill(
            RadialGradient(
              colors: [level.color.opacity(0.4), level.color.opacity(0.05)], center: .center,
      ...
      Text(level.title)
        ...
        .foregroundStyle(
          LinearGradient(
            colors: [level.color, level.color.opacity(0.7)], startPoint: .leading,
            endPoint: .trailing))
          ...
          GeometryReader { geo in
            ZStack(alignment: .leading) {
              ...
              RoundedRectangle(cornerRadius: 6)
                .fill(
                  LinearGradient(
                    colors: [level.color, level.color.opacity(0.6)], startPoint: .leading,
                    endPoint: .trailing)
                )
      ...
      RoundedRectangle(cornerRadius: 20, style: .continuous)
        ...
        .overlay(
          RoundedRectangle(cornerRadius: 20, style: .continuous).stroke(
            level.color.opacity(0.3), lineWidth: 1))
    )
  }

  private var matrixEducationCard: some View {
    Button {
      showMatrixEducation = true
    } label: {
      VStack(alignment: .leading, spacing: 12) {
        HStack {
          Text("🔴").font(.system(size: 22))
          Text("THE MATRIX DECODED").font(.system(size: 13, weight: .bold, design: .rounded))
            .tracking(2).foregroundColor(Palette.primary.red)
          Spacer()
          Text("\(matrixProgress.layersCompleted.count)/10").font(
            .system(size: 14, weight: .bold, design: .rounded)
          ).foregroundColor(
            matrixProgress.hasCompletedAllLayers ? Color.green : Palette.primary.red)
        }
        ...
        GeometryReader { geo in
          ZStack(alignment: .leading) {
            ...
            RoundedRectangle(cornerRadius: 4)
              .fill(
                LinearGradient(
                  colors: [Palette.primary.red, Palette.primary.orange, Palette.accent.gold],
        ...
        HStack {
          Image(
            systemName: matrixProgress.hasCompletedAllLayers ? "checkmark.circle.fill" : "book.fill"
          )
          .foregroundColor(matrixProgress.hasCompletedAllLayers ? Color.green : Palette.primary.red)
          Text(
            matrixProgress.hasCompletedAllLayers ? "All layers understood" : "Foundation required"
          )
          .font(.system(size: 12, weight: .medium, design: .rounded))
          .foregroundColor(matrixProgress.hasCompletedAllLayers ? Color.green : Palette.text.muted)
          Spacer()
          Image(systemName: "chevron.right.circle.fill").font(.system(size: 16)).foregroundColor(
            Palette.primary.red.opacity(0.7))
        }
      }
      .padding(16)
      .background(
        RoundedRectangle(cornerRadius: 16, style: .continuous)
          ...
          .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous).stroke(
              Palette.primary.red.opacity(0.3), lineWidth: 1))
      )
    }
    ...
    return Button {
      ...
      VStack(alignment: .leading, spacing: 12) {
        HStack {
          ...
          Text("PHD CONSCIOUSNESS").font(.system(size: 13, weight: .bold, design: .rounded))
            .tracking(2).foregroundColor(Palette.primary.cyan)
          Spacer()
          Text("\(completedLessons)/\(totalLessons)").font(
            .system(size: 14, weight: .bold, design: .rounded)
          ).foregroundColor(completedLessons == totalLessons ? Color.green : Palette.primary.cyan)
        }
        ...
        HStack {
          ...
          Spacer()
          Image(systemName: "chevron.right.circle.fill").font(.system(size: 16)).foregroundColor(
            Palette.primary.cyan.opacity(0.7))
        }
      }
      .padding(16)
      .background(
        RoundedRectangle(cornerRadius: 16, style: .continuous)
          .fill(
            LinearGradient(
              colors: [Palette.primary.cyan.opacity(0.1), Palette.primary.violet.opacity(0.1)],
      ...
      HStack(spacing: 16) {
        ZStack {
          RoundedRectangle(cornerRadius: 12)
            .fill(
              LinearGradient(
                colors: [Palette.accent.gold.opacity(0.3), Palette.primary.violet.opacity(0.3)],
                startPoint: .topLeading, endPoint: .bottomTrailing)
            )
  ...
  private var dailyCheckInCard: some View {
    Button {
      ...
      VStack(alignment: .leading, spacing: 12) {
        HStack {
          Image(systemName: "sun.max.fill").font(.system(size: 24, weight: .bold)).foregroundColor(
            Palette.accent.gold)
          Text("TODAY'S CHECK-IN").font(.system(size: 13, weight: .bold, design: .rounded))
            .tracking(2).foregroundColor(Palette.accent.gold)
          Spacer()
          Image(systemName: "chevron.right.circle.fill").font(.system(size: 20)).foregroundColor(
            Palette.accent.gold.opacity(0.7))
        }
        Text("Where did I go unconscious today?").font(
          .system(size: 18, weight: .semibold, design: .rounded)
        ).foregroundColor(Palette.text.primary)
        Text("Log a moment when you lost awareness and reacted automatically").font(
          .system(size: 13, weight: .medium, design: .rounded)
        ).foregroundColor(Palette.text.secondary)

        let todayCheckIns = presenterBox.profile.checkIns.filter {
          Calendar.current.isDateInToday($0.date)
        }.count
        HStack {
          Image(systemName: todayCheckIns > 0 ? "checkmark.circle.fill" : "circle").foregroundColor(
            todayCheckIns > 0 ? Color.green : Palette.text.muted)
          Text(todayCheckIns > 0 ? "Logged today" : "Not logged yet").font(
            .system(size: 12, weight: .medium, design: .rounded)
          ).foregroundColor(todayCheckIns > 0 ? Color.green : Palette.text.muted)
          Spacer()
          Text("\(presenterBox.profile.totalCheckIns) total").font(
            .system(size: 12, weight: .medium, design: .rounded)
          ).foregroundColor(Palette.text.muted)
        }
      }
  ...
  private var featureCardsSection: some View {
    VStack(spacing: 14) {
      featureCard(
        icon: "brain.head.profile", title: "Cognitive Dissonance",
        subtitle: "Log when reality ≠ what you were taught",
        stat: "\(presenterBox.profile.totalDissonanceEntries) entries",
        color: Palette.primary.violet
      ) { showDissonanceLog = true }
      featureCard(
        icon: "checklist", title: "Reality Audit", subtitle: "Inventory and examine your beliefs",
        stat:
          "\(presenterBox.profile.examinedBeliefs)/\(presenterBox.profile.totalBeliefs) examined",
        color: Palette.primary.cyan
      ) { showRealityAudit = true }
      featureCard(
        icon: "questionmark.circle.fill", title: "Question Bank",
        subtitle: "Socratic questions to challenge assumptions",
        stat: "\(presenterBox.profile.answeredQuestions) answered", color: Palette.primary.orange
      ) { showQuestionBank = true }
      featureCard(
        icon: "heart.circle.fill", title: "Emotional Processing",
        subtitle: "Guides for grief, anger, and betrayal", stat: "Dark Night Tools",
        color: Color.pink
      ) { showEmotionalProcessing = true }
    }
  }

  private func featureCard(
    icon: String, title: String, subtitle: String, stat: String, color: Color,
    ...
    Button(action: {
      ...
      HStack(spacing: 14) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          Text(title).font(.system(size: 16, weight: .bold, design: .rounded)).foregroundColor(
            Palette.text.primary)
          Text(subtitle).font(.system(size: 12, weight: .medium, design: .rounded)).foregroundColor(
            Palette.text.secondary
          ).lineLimit(1)
        }
        Spacer()
        VStack(alignment: .trailing, spacing: 2) {
          Text(stat).font(.system(size: 12, weight: .semibold, design: .rounded)).foregroundColor(
            color)
  ...
  private var cosmicBackground: some View {
    ZStack {
      ...
      ForEach(0..<40, id: \.self) { _ in
        Circle()
          .fill(
            [Palette.accent.gold, Palette.primary.violet, Palette.primary.cyan].randomElement()!
