...
final class CourseEngagementTracker: ObservableObject {
    ...
    private let abandonmentMessages = [
        ...
        ("📚 Books don't read themselves", "Well, technically I read them to you, but you still gotta show up!"),
        ...
        // Direct and fun
        ("🎯 Quick check-in", "Still awakening, or did you fall back asleep? No judgment... okay, a little."),
    ...
    func startCourse(_ courseId: String, courseName: String, totalLessons: Int) {
        ...
        if activeCourses.contains(where: { $0.courseId == courseId }) {
    ...
    func checkForAbandonedCourses() {
        ...
        for engagement in activeCourses {
            let daysSinceAccess = calendar.dateComponents([.day], from: engagement.lastAccessDate, to: now).day ?? 0
    ...
    func getEngagement(for courseId: String) -> CourseEngagement? {
        activeCourses.first { $0.courseId == courseId } ??
        abandonedCourses.first { $0.courseId == courseId } ??
...
struct MyCoursesView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        if !tracker.activeCourses.isEmpty {
                            courseSection(title: "IN PROGRESS", courses: tracker.activeCourses, icon: "play.fill", color: .green)
                        }
                        
                        // Abandoned courses
                        if !tracker.abandonedCourses.isEmpty {
                            courseSection(title: "PICK UP WHERE YOU LEFT OFF", courses: tracker.abandonedCourses, icon: "arrow.counterclockwise", color: .orange)
                        }
                        
                        // Completed courses
                        if !tracker.completedCourses.isEmpty {
                            courseSection(title: "COMPLETED", courses: tracker.completedCourses, icon: "checkmark.seal.fill", color: Color(hex: "#FFD700"))
                        }
            ...
            .alert("Reset Course?", isPresented: $showResetConfirmation) {
                ...
                Text("This will erase all progress and start from the beginning. Your earned badges will be kept.")
            }
        }
    }
    ...
    private func courseSection(title: String, courses: [CourseEngagement], icon: String, color: Color) -> some View {
