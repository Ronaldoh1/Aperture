...
enum ContentMode: String, CaseIterable, Codable {
    ...
    var description: String {
        switch self {
        case .full: return "Everything. All modules, all courses, all perspectives. The complete SunFlow journey."
        case .nutritionFocus: return "Quantum Nourish only. Scanner, Kill List, Fitness, Cleansing, Meal Plans. No spiritual or religious courses."
...
enum ContentModule: String, CaseIterable {
    ...
    var isNutrition: Bool {
        switch self {
        case .vibeSpectrum, .scanner, .barcodeScanner, .killList, .mealPlan, .fitness, .cleansing, .biometrics, .challenges, .badges, .budgetMode, .shoppingList, .marketFinder:
            return true
        default:
            return false
        }
        ...
        NavigationStack {
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        VStack(spacing: 10) {
                            Image(systemName: "slider.horizontal.3").font(.system(size: 44)).foregroundColor(Color(hex: "#3498DB"))
                            Text("CONTENT PREFERENCES").font(.system(size: 10, weight: .bold)).tracking(3).foregroundColor(Color(hex: "#3498DB"))
                            Text("Your App, Your Experience").font(.system(size: 20, weight: .bold, design: .rounded)).foregroundColor(Palette.text.primary)
                            Text("Choose what's visible. This isn't censorship — it's user empowerment. You can change this anytime.")
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            Text("CONTENT MODE").font(.system(size: 10, weight: .bold)).tracking(1.5).foregroundColor(Palette.text.muted)
                                ...
                                Button {
                                    ...
                                    HStack(spacing: 14) {
                                        ...
                                        VStack(alignment: .leading, spacing: 3) {
                                            Text(mode.rawValue).font(.system(size: 14, weight: .bold, design: .rounded)).foregroundColor(Palette.text.primary)
                                            Text(mode.description).font(.system(size: 11, weight: .medium)).foregroundColor(Palette.text.muted).lineLimit(2)
                                        }
                                        Spacer()
                                        Image(systemName: prefs.contentMode == mode ? "checkmark.circle.fill" : "circle")
                                            .font(.system(size: 22)).foregroundColor(prefs.contentMode == mode ? mode.color : Palette.text.muted)
                                    }
                                    .padding(14)
                                    .background(RoundedRectangle(cornerRadius: 14).fill(prefs.contentMode == mode ? mode.color.opacity(0.08) : Color.white.opacity(0.03))
                                        .overlay(RoundedRectangle(cornerRadius: 14).stroke(prefs.contentMode == mode ? mode.color.opacity(0.3) : Color.white.opacity(0.06), lineWidth: 1)))
                        ...
                        if prefs.contentMode == .custom {
                            let categories: [String: [ContentModule]] = Dictionary(grouping: ContentModule.allCases, by: { $0.category })
                            let sortedKeys: [String] = Array(categories.keys.sorted())
                            
                            ForEach(sortedKeys, id: \.self) { category in
                                VStack(alignment: .leading, spacing: 10) {
                                    Text(category.uppercased()).font(.system(size: 10, weight: .bold)).tracking(1.5).foregroundColor(Color(hex: "#3498DB"))
                                        ...
                                        Button {
                                            ...
                                            HStack(spacing: 12) {
                                                Image(systemName: module.icon).font(.system(size: 16))
                                                    .foregroundColor(prefs.isModuleVisible(module) ? Color(hex: "#2ECC71") : Palette.text.muted)
                                                    .frame(width: 24)
                                                Text(module.rawValue).font(.system(size: 14, weight: .medium))
                                                    .foregroundColor(prefs.isModuleVisible(module) ? Palette.text.primary : Palette.text.muted)
                                                Spacer()
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button { dismiss() } label: { HStack(spacing: 4) { Image(systemName: "chevron.left"); Text("Back") }.foregroundColor(Color(hex: "#3498DB")) }
                }
                ToolbarItem(placement: .principal) {
                    Text("PREFERENCES").font(.system(size: 14, weight: .bold, design: .rounded)).tracking(2).foregroundColor(Color(hex: "#3498DB"))
                }
            }
        }
    }
