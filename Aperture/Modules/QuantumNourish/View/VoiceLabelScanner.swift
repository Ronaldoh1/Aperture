...
class VoiceScannerManager: ObservableObject {
    ...
    private func processCommand(_ text: String) {
        if text.contains("scan this") || text.contains("read this") || text.contains("check this") {
            ...
            lastCommand = "query"
            let query = text.replacingOccurrences(of: "what is ", with: "").replacingOccurrences(of: "is this safe", with: "").trimmingCharacters(in: .whitespaces)
    ...
    func analyzeIngredient(_ name: String) {
        ...
        let killListItems: [String: String] = [
            "high fructose corn syrup": "🔴 AVOID — Liver damage, obesity, insulin resistance. In 80%+ of processed foods.",
            "aspartame": "🔴 AVOID — Neurotoxin. Headaches, mood disorders. Marketed as 'diet' but linked to weight gain.",
            "sodium nitrite": "🔴 AVOID — Converts to carcinogenic nitrosamines. Common in deli meats, hot dogs.",
            "red 40": "🔴 AVOID — Petroleum-based dye. Hyperactivity in children. Banned in EU for kids.",
            "yellow 5": "🔴 AVOID — Petroleum-based dye. Allergic reactions, hyperactivity. Banned in several countries.",
            ...
            "monosodium glutamate": "🟡 CAUTION — Excitotoxin. Hidden under 40+ names including 'natural flavors.'",
            "carrageenan": "🟡 CAUTION — Inflammatory. GI issues. Common in milk alternatives.",
            "sodium benzoate": "🔴 AVOID — Forms benzene (carcinogen) when combined with Vitamin C. In many sodas.",
            "potassium bromate": "🔴 AVOID — Carcinogen in bread. Banned in EU, Canada, Brazil. Still legal in US.",
            "titanium dioxide": "🔴 AVOID — Nanoparticle damaging gut lining. Banned in EU since 2022. Still in US candy.",
            ...
            "sucralose": "🟡 CAUTION — Artificial sweetener. May disrupt gut microbiome and insulin response.",
            "canola oil": "🟡 CAUTION — Highly processed seed oil. Inflammatory. Often from GMO rapeseed.",
            "soybean oil": "🟡 CAUTION — Most consumed oil in US. Highly processed, inflammatory, usually GMO.",
        ]
        
        // Check against kill list
        for (ingredient, result) in killListItems {
            if trimmed.contains(ingredient) || ingredient.contains(trimmed) {
                scanResult = VoiceScanResult(ingredient: ingredient.capitalized, verdict: result, isKillList: result.contains("AVOID"))
        ...
        scanResult = VoiceScanResult(ingredient: trimmed.capitalized, verdict: "✅ Not on the Kill List. Check the full label for other ingredients.", isKillList: false)
    ...
    func analyzeIngredientsText(_ text: String) {
        ...
        if found.isEmpty {
            ...
            scanResult = VoiceScanResult(ingredient: "Full Label", verdict: "✅ Scanned \(ingredients.count) ingredients — none on the Kill List. Still check for excess sugar, sodium, and seed oils.", isKillList: false)
        } else {
            statusMessage = "🔴 Found \(found.count) Kill List item(s)!"
            scanResult = VoiceScanResult(ingredient: "Full Label", verdict: "🔴 Found \(found.count) Kill List item(s): \(found.map(\.ingredient).joined(separator: ", "))", isKillList: true)
        }
    }
    
    private func speakResult(_ text: String) {
        let utterance = AVSpeechUtterance(string: text.replacingOccurrences(of: "🔴", with: "Warning.").replacingOccurrences(of: "🟡", with: "Caution.").replacingOccurrences(of: "✅", with: "Clear."))
        utterance.rate = 0.45
...
struct VoiceLabelScannerView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        ...
                        Button {
                            ...
                            ZStack {
                                Circle().fill(manager.isListening ? Color.red.opacity(0.15) : Color.cyan.opacity(0.08))
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text("VOICE COMMANDS").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.white.opacity(0.3))
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            Text("OR TYPE / PASTE INGREDIENTS").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.white.opacity(0.3))
                        ...
                        if let result = manager.scanResult {
                            VStack(alignment: .leading, spacing: 8) {
                                HStack(spacing: 8) {
                                    Image(systemName: result.isKillList ? "exclamationmark.triangle.fill" : "checkmark.circle.fill")
                                        .foregroundColor(result.isKillList ? .red : .green)
                                    Text(result.ingredient).font(.system(size: 15, weight: .bold)).foregroundColor(.white)
                                }
                                Text(result.verdict)
                            ...
                            .background(RoundedRectangle(cornerRadius: 14).fill(
                                ...
                                .overlay(RoundedRectangle(cornerRadius: 14).stroke(
                                    result.isKillList ? Color.red.opacity(0.15) : Color.green.opacity(0.15), lineWidth: 1)))
                        }
            ...
            .onAppear {
                ...
                withAnimation(.easeInOut(duration: 1.5).repeatForever(autoreverses: true)) { pulsePhase = .pi * 2 }
            }
    ...
    private func commandRow(_ command: String, _ desc: String) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Text(command).font(.system(size: 11, weight: .bold, design: .monospaced)).foregroundColor(.cyan).frame(width: 130, alignment: .leading)
