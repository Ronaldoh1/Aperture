...
struct DailyCheckInSheet: View {
  ...
  var body: some View {
    ZStack {
      ...
      VStack(spacing: 0) {
        HStack {
          Text("Daily Check-In").font(.system(size: 20, weight: .bold, design: .rounded))
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 24) {
            VStack(alignment: .leading, spacing: 8) {
              ...
              Text("Describe a moment when you lost awareness and reacted automatically.").font(
                .system(size: 14, weight: .medium, design: .rounded)
              ).foregroundColor(Palette.text.secondary).lineSpacing(4)
            }

            ZStack(alignment: .topLeading) {
              if unconsciousnessMoment.isEmpty {
                Text("e.g., I got triggered by a comment and snapped...").font(
                  .system(size: 15, weight: .regular, design: .rounded)
                ).foregroundColor(Palette.text.muted).padding(.top, 12).padding(.leading, 12)
              }
              TextEditor(text: $unconsciousnessMoment).font(
                .system(size: 15, weight: .regular, design: .rounded)
              ).foregroundColor(Palette.text.primary).scrollContentBackground(.hidden).frame(
                minHeight: 120)
            }
            .padding(8)
            .background(
            ...
            VStack(alignment: .leading, spacing: 12) {
              Text("CATEGORY").font(.system(size: 12, weight: .bold, design: .rounded)).tracking(1)
              ...
              Text(selectedCategory.description).font(
                .system(size: 12, weight: .medium, design: .rounded)
              ).foregroundColor(selectedCategory.color.opacity(0.9)).padding(12).background(
                RoundedRectangle(cornerRadius: 10, style: .continuous).fill(
            ...
            VStack(alignment: .leading, spacing: 12) {
              ...
              HStack {
                Text("Mild").font(.system(size: 11, weight: .medium, design: .rounded))
                  .foregroundColor(Palette.text.muted)
                Spacer()
                Text("Intense").font(.system(size: 11, weight: .medium, design: .rounded))
                  .foregroundColor(Palette.text.muted)
              }
            }

            VStack(alignment: .leading, spacing: 8) {
              Text("INSIGHT (OPTIONAL)").font(.system(size: 12, weight: .bold, design: .rounded))
                .tracking(1).foregroundColor(Palette.text.secondary)
              ZStack(alignment: .topLeading) {
                if insight.isEmpty {
                  Text("Any realization or pattern...").font(.system(size: 14)).foregroundColor(
                    Palette.text.muted
                  ).padding(.top, 12).padding(.leading, 12)
                }
                TextEditor(text: $insight).font(.system(size: 14)).foregroundColor(
            ...
            Button {
              ...
              HStack {
                ...
                Text("Save Check-In").font(.system(size: 16, weight: .bold, design: .rounded))
              }
            ...
            .opacity(
              unconsciousnessMoment.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                ? 0.5 : 1.0)
  ...
  private func saveCheckIn() {
    let checkIn = DailyCheckIn(
      ...
      insight: insight.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        ? nil : insight.trimmingCharacters(in: .whitespacesAndNewlines)
    )
