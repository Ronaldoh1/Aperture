...
class StreakManager: ObservableObject {
    ...
    func load() {
        guard let data = UserDefaults.standard.data(forKey: storageKey),
              let decoded = try? JSONDecoder().decode([PracticeStreak].self, from: data) else {
            return
        }
        streaks = decoded
    }
...
struct StreakCardView: View {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16)
                        .stroke(streak.isCompletedToday ? Color.green.opacity(0.3) : streak.color.opacity(0.2), lineWidth: 1)
                )
        )
        .onAppear {
            if streak.currentStreak > 0 {
    ...
    private func statItem(_ label: String, _ value: String, _ icon: String, _ color: Color) -> some View {
        VStack(spacing: 2) {
            HStack(spacing: 4) {
