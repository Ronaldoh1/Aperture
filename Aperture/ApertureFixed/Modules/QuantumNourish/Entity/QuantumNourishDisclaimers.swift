...
enum DisclaimerType: String, CaseIterable {
    ...
    var shortText: String {
        switch self {
        case .medical:
            return "Not medical advice. Consult a licensed healthcare provider before making changes to your diet or health regimen."
        case .nutritional:
            return "Nutritional information is for educational purposes only. Individual needs vary. Consult a registered dietitian for personalized guidance."
        case .fitness:
            return "Not a substitute for professional athletic training. Consult your physician before starting any exercise or nutrition program."
        case .supplement:
            return "Supplement information is educational only. Not evaluated by the FDA. Consult your healthcare provider before taking any supplement."
        case .fasting:
            return "Fasting protocols are not appropriate for everyone. Do not fast if pregnant, nursing, diabetic, or under 18. Consult your doctor first."
        case .frequency:
            return "Frequency/MHz values are based on alternative wellness models and are not recognized by mainstream medical science. Use as an educational framework."
        }
    }
            ...
            Button {
                ...
                .background(
                    RoundedRectangle(cornerRadius: 8)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 8).stroke(type.color.opacity(0.1), lineWidth: 0.5))
                )
            }
            .buttonStyle(.plain)
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 20) {
                    ...
                    Text("Quantum Nourish is an educational tool for exploring the relationship between food choices and consciousness. Please read and acknowledge the following:")
                    ...
                    ForEach([DisclaimerType.medical, .nutritional, .frequency], id: \.rawValue) { type in
                        VStack(alignment: .leading, spacing: 8) {
                            HStack(spacing: 6) {
                                ...
                                Text(type.rawValue.uppercased()).font(.system(size: 10, weight: .bold)).tracking(1).foregroundColor(type.color)
                            }
                            Text(type.shortText)
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 12)
                                .fill(type.color.opacity(0.04))
                                .overlay(RoundedRectangle(cornerRadius: 12).stroke(type.color.opacity(0.15), lineWidth: 1))
                        )
                    }
                    .padding(.horizontal, 20)
