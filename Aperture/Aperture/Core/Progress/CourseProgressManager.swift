...
final class CourseProgressManager: ObservableObject {
  ...
  private func loadProgress() {
    guard let data = defaults.data(forKey: progressKey),
      let decoded = try? JSONDecoder().decode([String: UserCourseProgress].self, from: data)
    else {
      return
    }
    allProgress = decoded
  ...
  private func recordActivity() {
    ...
    if let lastActivity = defaults.object(forKey: lastActivityKey) as? Date {
      ...
      let daysDiff =
        Calendar.current.dateComponents([.day], from: lastActivityDay, to: today).day ?? 0
  ...
  func moduleProgress(courseId: String, moduleId _: String, lessonIds: [String]) -> (
...
struct CourseProgressManagerKey: EnvironmentKey {
  nonisolated(unsafe) static let defaultValue = CourseProgressManager.shared
}
