...
struct CourseHubView: View {
  ...
  var body: some View {
    ...
    .sheet(item: $selectedCourse) { course in
      ...
      case "antichrist_decoded":
        if let c = AlexandriaCourseRegistry.course(for: "antichrist_decoded") {
          AlexandriaCourseView(course: c)
        }
      case "baphomet_mirror":
        if let c = AlexandriaCourseRegistry.course(for: "baphomet_mirror") {
          AlexandriaCourseView(course: c)
        }
      case "gospel_judas":
        if let c = AlexandriaCourseRegistry.course(for: "gospel_judas") {
          AlexandriaCourseView(course: c)
        }
      case "guilt_power":
        if let c = AlexandriaCourseRegistry.course(for: "guilt_power") {
          AlexandriaCourseView(course: c)
        }
      case "tonantzin":
        if let c = AlexandriaCourseRegistry.course(for: "tonantzin") {
          AlexandriaCourseView(course: c)
        }
      case "cross_solar":
        if let c = AlexandriaCourseRegistry.course(for: "cross_solar") {
          AlexandriaCourseView(course: c)
        }
      case "crucifixion_initiation":
        if let c = AlexandriaCourseRegistry.course(for: "crucifixion_initiation") {
          AlexandriaCourseView(course: c)
        }
      case "twelve_plus_one":
        if let c = AlexandriaCourseRegistry.course(for: "twelve_plus_one") {
          AlexandriaCourseView(course: c)
        }
      case "anunnaki":
        if let c = AlexandriaCourseRegistry.course(for: "anunnaki") {
          AlexandriaCourseView(course: c)
        }
      case "elohim":
        if let c = AlexandriaCourseRegistry.course(for: "elohim") {
          AlexandriaCourseView(course: c)
        }
      case "nephilim_watchers":
        if let c = AlexandriaCourseRegistry.course(for: "nephilim_watchers") {
          AlexandriaCourseView(course: c)
        }
      case "urantia_gaia":
        if let c = AlexandriaCourseRegistry.course(for: "urantia_gaia") {
          AlexandriaCourseView(course: c)
        }
      case "christ_consciousness":
        if let c = AlexandriaCourseRegistry.course(for: "christ_consciousness") {
          AlexandriaCourseView(course: c)
        }
      case "lemuria_atlantis":
        if let c = AlexandriaCourseRegistry.course(for: "lemuria_atlantis") {
          AlexandriaCourseView(course: c)
        }
      case "abracadabra":
        if let c = AlexandriaCourseRegistry.course(for: "abracadabra") {
          AlexandriaCourseView(course: c)
        }
      case "tarot_arcana":
        if let c = AlexandriaCourseRegistry.course(for: "tarot_arcana") {
          AlexandriaCourseView(course: c)
        }
      case "one_forty_four_k":
        if let c = AlexandriaCourseRegistry.course(for: "one_forty_four_k") {
          AlexandriaCourseView(course: c)
        }
      case "alchemy_great_work":
        if let c = AlexandriaCourseRegistry.course(for: "alchemy_great_work") {
          AlexandriaCourseView(course: c)
        }
      case "antarctica":
        if let c = AlexandriaCourseRegistry.course(for: "antarctica") {
          AlexandriaCourseView(course: c)
        }
      // QUANTUM NOURISH
  ...
  private var statsSection: some View {
    HStack(spacing: 12) {
      statCard(
        value: "\(CourseCatalog.courses.count)", label: "Courses", icon: "book.fill",
        color: Palette.primary.cyan)
      statCard(
        value: "\(CourseCatalog.totalModules)", label: "Modules", icon: "folder.fill",
        color: Palette.primary.violet)
      statCard(
        value: "\(Int(CourseCatalog.totalHours))h+", label: "Content", icon: "clock.fill",
...
struct CourseCard: View {
  ...
  var body: some View {
    ...
    .accessibilityLabel(
      "\(course.title). \(course.category.rawValue) course. \(course.difficulty.rawValue) difficulty."
    )
    .accessibilityHint(
      "\(course.moduleCount) modules, about \(String(format: "%.1f", course.estimatedHours)) hours. Double tap to start course."
    )
