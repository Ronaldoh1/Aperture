    ...
    private var meetDragonPage: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                ...
                HStack(spacing: 16) {
                    ForEach([SacredGeometryShape.vesicaPiscis, .seedOfLife, .flowerOfLife], id: \.self) { shape in
    ...
    private var firstMicroWinPage: some View {
        VStack(spacing: 24) {
            ...
            HStack(spacing: 8) {
                ...
                Text("Tip: Shake your phone anytime to send us feedback")
                    .font(.system(size: 12, weight: .medium))
                    .foregroundColor(.white.opacity(0.3))
            }
            .padding(.horizontal, 16).padding(.vertical, 8)
            .background(Capsule().fill(Color.cyan.opacity(0.04)))
