...
struct AnimatedFlowerOfLife: View {
  ...
  private var sunGlow: some View {
    let glowAlpha1 = 0.25 + centerGlow * 0.15
    let glowAlpha2 = 0.12 + centerGlow * 0.08
    let colors: [Color] = [
  ...
  private var flowerLayer2Inner: some View {
    ...
    return ZStack {
      ForEach(0..<6, id: \.self) { i in
        let angle = Double(i) * (.pi / 3) + rotRad
        let xOff = CGFloat(cos(angle)) * r
        let yOff = CGFloat(sin(angle)) * r
  ...
  private var flowerLayer3Outer: some View {
    ...
    return ZStack {
      ForEach(0..<6, id: \.self) { i in
        let angle = Double(i) * (.pi / 3) + rotRad
        let xOff = CGFloat(cos(angle)) * r * 2
        let yOff = CGFloat(sin(angle)) * r * 2
  ...
  private var flowerLayer4Interstitial: some View {
    ...
    return ZStack {
      ForEach(0..<6, id: \.self) { i in
        let angle = Double(i) * (.pi / 3) + (.pi / 6) + rotRad
        let xOff = CGFloat(cos(angle)) * dist
        let yOff = CGFloat(sin(angle)) * dist
  ...
  private func makeCircle(r: CGFloat, offsetX: CGFloat, offsetY: CGFloat, alpha: Double)
    -> some View
  {
    ...
    let c2 = Color.white.opacity(alpha * 0.5)
    let c3: Color = accentColor.opacity(alpha * 0.7)
    let grad = LinearGradient(
      colors: [c1, c2, c3], startPoint: .topLeading, endPoint: .bottomTrailing)
  ...
  private var sunRays: some View {
    ...
    let rayAlpha = 0.5 + centerGlow * 0.3
    return ZStack {
      ForEach(0..<6, id: \.self) { i in
  ...
  private var lightParticles: some View {
    Canvas { context, canvasSize in
      ...
      for i in 0..<18 {
        ...
        let rDist = Double(size) * (0.25 + sin(time * 2 + di * 0.8) * 0.18)
        ...
        let alpha = 0.3 + sin(time * 2.5 + di * 0.7) * 0.25
