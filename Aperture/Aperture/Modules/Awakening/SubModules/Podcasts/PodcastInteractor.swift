...
class FirebasePodcastStorage: PodcastStorageProtocol {
  ...
  func getListeningHistory() async throws -> [PodcastListenLog] {
    guard let data = UserDefaults.standard.data(forKey: historyKey),
      let history = try? JSONDecoder().decode([PodcastListenLog].self, from: data)
    else {
      return []
    }
  ...
  func getAllNotes() async throws -> [PodcastNote] {
    guard let data = UserDefaults.standard.data(forKey: notesKey),
      let notes = try? JSONDecoder().decode([PodcastNote].self, from: data)
    else {
      return []
    }
  ...
  private func updateStats(minutesAdded: Int, podcastId _: UUID) async {
    var stats = getStats()
  ...
  func getStats() -> PodcastStats {
    guard let data = UserDefaults.standard.data(forKey: statsKey),
      let stats = try? JSONDecoder().decode(PodcastStats.self, from: data)
    else {
      return .empty
    }
    return stats
...
class APIPodcastStorage: PodcastStorageProtocol {
  ...
  func saveFavorite(podcastId _: UUID) async throws {
    // POST /api/podcasts/favorites
  }

  func removeFavorite(podcastId _: UUID) async throws {
  ...
  func logListen(podcastId _: UUID, episodeTitle _: String, minutesListened _: Int) async throws {
  ...
  func saveNote(podcastId _: UUID, episodeTitle _: String, note _: String) async throws {
    // POST /api/podcasts/notes
  }

  func getNotes(for _: UUID) async throws -> [PodcastNote] {
  ...
  func getRecommendations(basedOn _: [UUID]) async throws -> [UUID] {
...
class PodcastInteractor: ObservableObject {
  ...
  func switchToFirebase() {
    storage = FirebasePodcastStorage()
  }

  func switchToAPI(baseURL: String, apiKey: String) {
    storage = APIPodcastStorage(baseURL: baseURL, apiKey: apiKey)
  }
  ...
  func loadData() async {
    do {
      ...
      recentHistory = try Array((await storage.getListeningHistory()).prefix(10))
  ...
  func logListening(podcast: AwakeningPodcast, episode: String, minutes: Int) async {
    do {
      try await storage.logListen(
        podcastId: podcast.id, episodeTitle: episode, minutesListened: minutes)
      await loadData()
    } catch {
...
enum PodcastAssembly {
  @MainActor static func build() -> AwakeningPodcastsView {
    return AwakeningPodcastsView()
  }

  @MainActor static func buildWithCustomBackend(baseURL _: String, apiKey _: String)
