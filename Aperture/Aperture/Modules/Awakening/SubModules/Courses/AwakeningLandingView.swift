...
struct AwakeningLandingView: View {
    ...
    GeometryReader { geo in
      ForEach(0..<30, id: \.self) { i in
        Circle()
          .fill(
            [
              Color(hex: "#7C5CFF"), Color(hex: "#00D9FF"), Color(hex: "#FFD700"),
              Color(hex: "#E040FB"),
            ][i % 4]
          )
  ...
  private var sunflowStateSection: some View {
    NavigationLink(
      destination: PremiumGateView(featureName: "SunFlow State") { SunFlowStateView() }
    ) {
      VStack(spacing: 16) {
        HStack {
          HStack(spacing: 8) {
          ...
          destination: AnyView(
            GenericCourseView(
              ...
              modules: CatholicChurchExposeCourse.shared.modules.map { m in
                GenericModule(
                  id: m.id, number: m.number, title: m.title, subtitle: m.subtitle,
                  lessons: m.lessons.map { l in
                    GenericLesson(
                      id: l.id, title: l.title, content: l.content, keyPoints: l.keyPoints)
                  })
              }
            ))
        ),
    ...
    CourseSectionView(
      ...
      courses: [
        CourseRowData(
          ...
          destination: AnyView(
            PlaceholderCourseView(title: "The Great Scams", subtitle: "Coming soon"))
        ),
        CourseRowData(
          id: "systems_control",
          ...
          destination: AnyView(
            PlaceholderCourseView(title: "Systems of Control", subtitle: "Coming soon"))
        ),
      ]
    )
  }
    ...
    CourseSectionView(
      ...
      courses: [
        CourseRowData(
          ...
          color: Color(hex: "#9C27B0"),
          destination: AnyView(
            PlaceholderCourseView(title: "PhD Consciousness", subtitle: "Coming soon"))
        ),
        CourseRowData(
          id: "quantum",
          ...
          destination: AnyView(
            PlaceholderCourseView(title: "Quantum Reality", subtitle: "Coming soon"))
        ),
      ]
    )
  }
    ...
    CourseSectionView(
      ...
      courses: [
        CourseRowData(
          ...
          destination: AnyView(
            PlaceholderCourseView(title: "Buddha's Path", subtitle: "Coming soon"))
        ),
        CourseRowData(
          id: "sacred_geometry",
          ...
          color: Color(hex: "#7C5CFF"),
          destination: AnyView(
            PlaceholderCourseView(title: "Sacred Geometry", subtitle: "Coming soon"))
        ),
      ]
    )
  }
...
struct AwakeningMetatronsCubeShape: Shape {
  func path(in rect: CGRect) -> Path {
    ...
    for point in points {
      path.addEllipse(
        in: CGRect(x: point.x - r / 3, y: point.y - r / 3, width: r * 2 / 3, height: r * 2 / 3))
    }
...
struct GenericCourseView: View {
  ...
  var body: some View {
    ...
    .sheet(item: $selectedLesson) { lesson in
      GenericLessonDetailView(
        lesson: lesson, color: color, onComplete: { completedLessons.insert(lesson.id) })
    }
  }
    ...
    VStack(spacing: 16) {
      ZStack {
        Circle()
          .fill(
            RadialGradient(
              colors: [color.opacity(0.4), Color.clear], center: .center, startRadius: 20,
              endRadius: 60)
          )
          .frame(width: 120, height: 120)
  ...
  private var progressSection: some View {
    VStack(spacing: 12) {
      HStack {
        Text("PROGRESS").font(.system(size: 12, weight: .bold, design: .rounded)).foregroundColor(
          Palette.text.muted
        ).tracking(2)
        Spacer()
        Text("\(Int(progressPercentage * 100))%").font(
      ...
      GeometryReader { geo in
        ZStack(alignment: .leading) {
          ...
          RoundedRectangle(cornerRadius: 4).fill(color).frame(
            width: geo.size.width * progressPercentage, height: 8)
        }
      }.frame(height: 8)
    }
    .padding(16)
  ...
  private var modulesSection: some View {
    VStack(spacing: 16) {
      ForEach(modules) { module in
        GenericModuleCard(
          module: module, color: color, isExpanded: expandedModules.contains(module.id),
...
struct GenericModuleCard: View {
  ...
  let onToggle: () -> Void
  let onSelectLesson: (GenericLesson) -> Void
  ...
  var body: some View {
    VStack(spacing: 0) {
      Button(action: onToggle) {
        HStack(spacing: 16) {
          Text("\(module.number)").font(.system(size: 16, weight: .black, design: .rounded))
            .foregroundColor(color).frame(width: 36, height: 36).background(color.opacity(0.15))
            .clipShape(Circle())
          VStack(alignment: .leading, spacing: 2) {
            Text(module.title).font(.system(size: 16, weight: .bold, design: .rounded))
              .foregroundColor(.white)
            Text("\(completedCount)/\(module.lessons.count) lessons").font(
              .system(size: 12, weight: .medium)
            ).foregroundColor(Palette.text.secondary)
          }
          Spacer()
          Image(systemName: isExpanded ? "chevron.up" : "chevron.down").font(
      ...
      if isExpanded {
        VStack(spacing: 8) {
          ForEach(module.lessons) { lesson in
            GenericLessonRow(
              lesson: lesson, color: color, isCompleted: completedLessons.contains(lesson.id),
    ...
    .background(
      RoundedRectangle(cornerRadius: 16).fill(Color.white.opacity(0.05)).overlay(
        RoundedRectangle(cornerRadius: 16).stroke(
          color.opacity(isExpanded ? 0.3 : 0.1), lineWidth: 1)))
  }
}

struct GenericLessonRow: View {
  let lesson: GenericLesson
  let color: Color
  ...
  var body: some View {
    Button(action: onTap) {
      HStack(spacing: 12) {
        ZStack {
          Circle().stroke(isCompleted ? color : Color.white.opacity(0.3), lineWidth: 2).frame(
            width: 24, height: 24)
          if isCompleted {
            Image(systemName: "checkmark").font(.system(size: 12, weight: .bold)).foregroundColor(
              color)
          }
        }
        Text(lesson.title).font(.system(size: 14, weight: .semibold, design: .rounded))
          .foregroundColor(isCompleted ? color : .white).lineLimit(2).multilineTextAlignment(
            .leading)
        Spacer()
        Image(systemName: "play.circle.fill").font(.system(size: 24)).foregroundColor(
          color.opacity(0.6))
    ...
    ZStack {
      ...
      ScrollView(showsIndicators: false) {
        VStack(alignment: .leading, spacing: 24) {
          VStack(alignment: .leading, spacing: 8) {
            Text("LESSON").font(.system(size: 12, weight: .bold, design: .rounded)).foregroundColor(
              color
            ).tracking(2)
            Text(lesson.title).font(.system(size: 24, weight: .black, design: .rounded))
              .foregroundColor(.white)
          }
          Text(lesson.content).font(.system(size: 16, weight: .regular, design: .rounded))
            .foregroundColor(Palette.text.primary).lineSpacing(6)
          if !lesson.keyPoints.isEmpty {
            VStack(alignment: .leading, spacing: 12) {
              Text("🔑 KEY POINTS").font(.system(size: 12, weight: .bold, design: .rounded))
              ...
              ForEach(lesson.keyPoints, id: \.self) { point in
                HStack(alignment: .top, spacing: 12) {
                  ...
                  Text(point).font(.system(size: 14, weight: .medium, design: .rounded))
                    .foregroundColor(Palette.text.secondary)
                }
              }
          ...
          Button(action: {
            ...
            .font(.system(size: 16, weight: .bold, design: .rounded)).foregroundColor(.black).frame(
              maxWidth: .infinity
            ).padding(.vertical, 16).background(color).clipShape(RoundedRectangle(cornerRadius: 14))
          }
          Spacer(minLength: 50)
      ...
      VStack {
        HStack {
          Spacer()
          Button(action: { dismiss() }) {
            Image(systemName: "xmark").font(.system(size: 16, weight: .semibold)).foregroundColor(
              .white
