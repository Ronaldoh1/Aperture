...
struct CandidateDataEntryView: View {
  ...
  private func sourceCard(
    name: String, description: String, url: String, icon: String, color: Color
  ) -> some View {
    Button(action: {
      HapticManager.shared.light()
...
struct CandidateComparisonView: View {
  ...
  private var emptyState: some View {
    VStack(spacing: 16) {
      ...
      Text(
        "We'll show you a side-by-side breakdown of their funding, voting records, and red flags."
      )
      .font(.system(size: 13))
...
struct ElectionTrackerView: View {
  ...
  private func electionCard(type: String, date: String, location: String, daysUntil: Int)
    -> some View
  {
    HStack {
  ...
  private var resourcesSection: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      Text(
        "Primaries are where real change happens. General elections often give us two pre-selected candidates. Vote in your primary!"
      )
      .font(.system(size: 13))
      .foregroundColor(.orange)
      .padding()
