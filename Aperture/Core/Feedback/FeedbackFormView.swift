...
@MainActor
class ShakeFeedbackManager: ObservableObject {
    static let shared = ShakeFeedbackManager()
    ...
    func captureScreenshot() {
        ...
        capturedScreenshot = renderer.image { ctx in
...
struct FeedbackFormView: View {
    ...
    @StateObject private var manager = ShakeFeedbackManager.shared
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button { dismiss() } label: {
                        Image(systemName: "xmark.circle.fill").foregroundColor(.white.opacity(0.3)).font(.system(size: 22))
                    }
                }
                ToolbarItem(placement: .principal) {
                    HStack(spacing: 6) {
                        Image(systemName: "bubble.left.and.bubble.right.fill").foregroundColor(.cyan).font(.system(size: 14))
    ...
    private var formView: some View {
        ScrollView(showsIndicators: false) {
            VStack(spacing: 18) {
                ...
                // Shake hint
                HStack(spacing: 8) {
                    Image(systemName: "iphone.radiowaves.left.and.right").font(.system(size: 14)).foregroundColor(.cyan.opacity(0.5))
                    Text("Shake your phone anytime to open this form").font(.system(size: 11, weight: .medium)).foregroundColor(.white.opacity(0.3))
                }
                .padding(10).background(Capsule().fill(Color.cyan.opacity(0.04)))
                
                // Category
                VStack(alignment: .leading, spacing: 8) {
                    Text("WHAT'S THIS ABOUT?").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(.white.opacity(0.3))
                        ...
                        HStack(spacing: 8) {
                            ForEach(FeedbackCategory.allCases) { cat in
                                Button {
                                    ...
                                    .background(Capsule().fill(selectedCategory == cat ? cat.color : cat.color.opacity(0.08)))
                                    .overlay(Capsule().stroke(cat.color.opacity(0.2), lineWidth: selectedCategory == cat ? 0 : 1))
                                }
                            }
                        }
                    }
                ...
                if selectedCategory == .love || selectedCategory == .general {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("HOW'S YOUR EXPERIENCE?").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(.white.opacity(0.3))
                        HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("TELL US MORE").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(.white.opacity(0.3))
                    ...
                    .background(RoundedRectangle(cornerRadius: 14).fill(Color.white.opacity(0.04))
                        .overlay(RoundedRectangle(cornerRadius: 14).stroke(selectedCategory.color.opacity(0.1), lineWidth: 1)))
                }
                ...
                if manager.capturedScreenshot != nil {
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Include Screenshot").font(.system(size: 13, weight: .bold)).foregroundColor(.white)
                ...
                // Device info
                VStack(alignment: .leading, spacing: 4) {
                    Text("DEVICE INFO (auto-attached)").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.white.opacity(0.2))
                    Text(manager.deviceInfo).font(.system(size: 10, weight: .medium, design: .monospaced)).foregroundColor(.white.opacity(0.25)).lineSpacing(3)
                }
...
struct ShakeFeedbackModifier: ViewModifier {
    @StateObject private var feedbackManager = ShakeFeedbackManager.shared
