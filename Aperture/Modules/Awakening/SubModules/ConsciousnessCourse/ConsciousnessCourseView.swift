...
struct ConsciousnessCourseView: View {
    ...
    private var courseHeader: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text("Consciousness").font(.system(size: 20, weight: .bold, design: .rounded)).foregroundColor(Palette.text.primary)
                Text("PhD-Level Course").font(.system(size: 13, weight: .medium, design: .rounded)).foregroundColor(Palette.primary.cyan)
            }
            Spacer()
    ...
    private var heroSection: some View {
        VStack(spacing: 16) {
            ZStack {
                Circle()
                    .fill(RadialGradient(colors: [Palette.primary.cyan.opacity(0.4), Color.clear], center: .center, startRadius: 0, endRadius: 60))
                    .frame(width: 120, height: 120)
                
                Image(systemName: "brain.head.profile")
                    .font(.system(size: 50, weight: .semibold))
                    .foregroundStyle(
                        LinearGradient(colors: [Palette.primary.cyan, Palette.primary.violet], startPoint: .topLeading, endPoint: .bottomTrailing)
                    )
            }
    ...
    private var statsSection: some View {
        VStack(spacing: 16) {
            ...
            // Stats row
            HStack(spacing: 12) {
                statCard(value: "\(db.modules.count)", label: "Modules", icon: "folder.fill", color: Palette.primary.violet)
                statCard(value: "\(db.totalLessons)", label: "Lessons", icon: "doc.text.fill", color: Palette.primary.cyan)
                statCard(value: "\(db.totalMinutes / 60)h+", label: "Content", icon: "clock.fill", color: Palette.accent.gold)
            }
        }
        .padding(16)
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                ...
                .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).stroke(Palette.primary.cyan.opacity(0.2), lineWidth: 1))
        )
    }
    ...
    private func statCard(value: String, label: String, icon: String, color: Color) -> some View {
        VStack(spacing: 6) {
            ...
            Text(label).font(.system(size: 10, weight: .medium, design: .rounded)).foregroundColor(Palette.text.muted)
        }
    ...
    private var footerSection: some View {
        VStack(spacing: 12) {
            HStack(spacing: 8) {
                ...
                Text("DRAGON'S WISDOM").font(.system(size: 12, weight: .bold, design: .rounded)).tracking(1).foregroundColor(Palette.accent.gold)
            }
            
            Text("\"This course won't give you answers—it will show you that you ARE the answer. The one who questions consciousness is consciousness questioning itself.\"")
...
struct ModuleDetailView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(colors: [Color.black, module.color.opacity(0.1), Color.black], startPoint: .top, endPoint: .bottom)
                .ignoresSafeArea()
            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("Module \(module.number)").font(.system(size: 14, weight: .bold, design: .rounded)).foregroundColor(Palette.text.muted)
                    Spacer()
    ...
    private var moduleHeader: some View {
        VStack(spacing: 12) {
            ZStack {
                Circle()
                    .fill(RadialGradient(colors: [module.color.opacity(0.4), module.color.opacity(0.05)], center: .center, startRadius: 0, endRadius: 45))
            ...
            HStack(spacing: 16) {
                HStack(spacing: 4) {
                    ...
                    Text("\(module.estimatedMinutes) min").font(.system(size: 12, weight: .semibold, design: .rounded))
                }
                .foregroundColor(Palette.text.muted)
                
                HStack(spacing: 4) {
                    Image(systemName: "doc.text").font(.system(size: 12))
                    Text("\(module.lessons.count) lessons").font(.system(size: 12, weight: .semibold, design: .rounded))
                }
                .foregroundColor(Palette.text.muted)
                
                HStack(spacing: 4) {
                    Circle().fill(module.difficulty.color).frame(width: 8, height: 8)
                    Text(module.difficulty.rawValue).font(.system(size: 12, weight: .semibold, design: .rounded))
                }
                .foregroundColor(module.difficulty.color)
            }
        }
    ...
    private var overviewSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("OVERVIEW").font(.system(size: 11, weight: .bold, design: .rounded)).tracking(1).foregroundColor(Palette.text.secondary)
    ...
    private var lessonsSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("LESSONS").font(.system(size: 11, weight: .bold, design: .rounded)).tracking(1).foregroundColor(Palette.text.secondary)
        ...
        HStack(spacing: 14) {
            ...
            ZStack {
                ...
                if isComplete {
                    ...
                    Text("\(lesson.number)").font(.system(size: 16, weight: .bold, design: .rounded)).foregroundColor(.white)
                }
            }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 8) {
                    HStack(spacing: 4) {
                        ...
                        Text("\(lesson.duration) min").font(.system(size: 11, weight: .medium, design: .rounded))
                    }
                    .foregroundColor(Palette.text.muted)
                    ...
                    if !lesson.reflectionPrompts.isEmpty {
                        HStack(spacing: 4) {
                            ...
                            Text("\(lesson.reflectionPrompts.count) prompts").font(.system(size: 11, weight: .medium, design: .rounded))
                        }
                        .foregroundColor(Palette.text.muted)
                    }
                }
