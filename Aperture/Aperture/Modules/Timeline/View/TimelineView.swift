...
struct TimelineView: View {
  ...
  // P1: Era navigation + avatar
  @StateObject private var avatarManager = TimelineAvatarManager.shared
  @State private var selectedCategory: EraCategory? = nil
  @State private var showAvatarSetup: Bool = false
  @State private var showProgrammingDetector: Bool = false
    ...
    NavigationStack {
      ZStack {
        ...
        if !hasChosenInterval {
          ...
          VStack(spacing: 0) {
            // Era category chips (tap-to-jump)
            TimelineEraCategoryBar(
              categories: EraCategory.allCases,
            ...
            HStack(spacing: 0) {
              ...
              // Mini map (right rail — Cosmos pattern)
              TimelineMiniMap(
                eras: presenterBox.eras,
                currentEraIndex: tappedEraIndex ?? 0,
          ...
          // Avatar "You Are Here" overlay (replaces old yearArrow)
          if !isDragonAbsorbed {
            avatarYouAreHereOverlay
          }
        }
      ...
      .sheet(isPresented: $showAvatarSetup) {
        TimelineAvatarSetupView(manager: avatarManager)
      }
      .fullScreenCover(isPresented: $showProgrammingDetector) {
        if #available(iOS 16.0, *) {
          ProgrammingDetectorView()
        }
      }
    }
    .onAppear {
          ...
          if showMenu {
            VStack(spacing: 0) {
              ...
              menuItem(
                ...
                label: "Programming Detector",
                delay: 0.1
              ) {
                showProgrammingDetector = true
              }
                ...
                .overlay(
                  RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(
                      LinearGradient(
                        colors: [
                          Palette.accent.gold.opacity(0.4), Palette.primary.cyan.opacity(0.2),
                        ],
  ...
  private func menuItem(icon: String, label: String, delay _: Double, action: @escaping () -> Void)
    -> some View
  {
    Button {
      withAnimation(.spring(response: 0.35, dampingFraction: 0.75)) {
  ...
  private func scrollToTop() {
    withAnimation(.spring(response: 0.6, dampingFraction: 0.8)) {
      scrollProxy?.scrollTo("timelineTop", anchor: .top)
      scrollOffset = 0
      tappedEraIndex = nil
  ...
  private func beginJourney() {
    ...
    withAnimation {
      dragonMessage =
        "The journey begins... \(presenterBox.eras.count) moments where truth was hidden."
    }
  ...
  // MARK: - Avatar "You Are Here" Overlay

  private var avatarYouAreHereOverlay: some View {
  ...
  private func jumpToCategory(_ category: EraCategory) {
    let eras = presenterBox.eras
    guard let index = eras.firstIndex(where: { $0.category == category }) else { return }
    jumpToEra(index)
  }

  private func jumpToEra(_ index: Int) {
    tappedEraIndex = index
    withAnimation(.spring(response: 0.5, dampingFraction: 0.8)) {
      scrollProxy?.scrollTo("era_\(index)", anchor: .center)
    }
    HapticManager.shared.light()
  }

  private func updateTimeComment() {
    let maxScroll = CGFloat(presenterBox.eras.count) * 140
          ...
          Rectangle()
            .fill(
              LinearGradient(
                colors: [
                  era.isRevealed
                    ? Palette.accent.gold.opacity(0.6) : Palette.primary.red.opacity(0.5),
      ...
      HStack(spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 1) {
          ...
          Text(era.subtitle)
            ...
            .foregroundColor(
              era.isRevealed ? Palette.accent.gold.opacity(0.9) : Palette.primary.red.opacity(0.8)
            )
            .lineLimit(1)
        }
            ...
            VStack(alignment: .center, spacing: 14) {
              ForEach(era.description.components(separatedBy: "\n\n"), id: \.self) { para in
                Text(para)
                  .font(
                    .system(
                      size: para.hasPrefix("☀️") ? 16 : 15,
                      weight: para.hasPrefix("☀️") ? .bold : .medium, design: .rounded)
                  )
