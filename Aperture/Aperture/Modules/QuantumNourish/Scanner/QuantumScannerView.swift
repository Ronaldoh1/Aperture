...
struct QuantumScannerView: View {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 10) {
              ...
              Text(
                "Copy the ingredients from any food label and paste them here. We'll decode every item."
              )
                  ...
                  RoundedRectangle(cornerRadius: 12)
                    ...
                    .overlay(
                      RoundedRectangle(cornerRadius: 12).stroke(
                        Color(hex: "#E74C3C").opacity(0.2), lineWidth: 1))
                )
              ...
              HStack(spacing: 8) {
                ...
                sampleButton(
                  "Doritos",
                  text:
                    "Corn, Vegetable Oil, Maltodextrin, Monosodium Glutamate, Cheddar Cheese, Artificial Color, Natural and Artificial Flavors"
                )
              }
  ...
  private func analyzeIngredients() {
    ...
    scanResults = ingredients.map { ingredient in
      ...
      let dangerKeywords = [
        "high fructose", "hydrogenated", "artificial", "monosodium glutamate", "msg", "aspartame",
        "sucralose", "caramel color", "red 40", "yellow 5", "blue 1", "sodium benzoate", "bht",
        "bha", "tbhq", "phosphoric acid", "natural flavors", "maltodextrin",
      ]
      ...
      if dangerKeywords.contains(where: { lower.contains($0) }) {
        return IngredientScanResult(
          ...
          explanation:
            "This ingredient is a known consciousness suppressant. Check the Kill List for details.",
          alternatives: ["Look for organic alternatives without this ingredient"]
        )
      }

      let cautionKeywords = [
        "sugar", "corn syrup", "palm oil", "soybean oil", "canola", "dextrose", "maltose",
        "modified",
      ]
      if cautionKeywords.contains(where: { lower.contains($0) }) {
        return IngredientScanResult(
          ...
          dangerLevel: .acidic,
          explanation:
            "This ingredient is low-frequency and should be minimized for consciousness elevation.",
          alternatives: []
        )
      }
    ...
    VStack(alignment: .leading, spacing: 16) {
      ...
      HStack(spacing: 12) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          Text("SCAN VERDICT").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(
            overallColor)
          Text(overallScore).font(.system(size: 14, weight: .bold, design: .rounded))
