...
struct ConsciousnessFieldAnimation: View {
    ...
    private func generateParticles() {
        for i in 0..<12 {
            ...
            let radius: CGFloat = CGFloat.random(in: 50...80)
            particles.append(FieldParticle(
                id: i,
                x: cos(angle) * radius,
                y: sin(angle) * radius
            ))
                ...
                ForEach(InnerState.allCases, id: \.self) { state in
                    Button {
                        ...
                        Text(state.rawValue)
                            ...
                            .padding(.horizontal, 12)
                            .padding(.vertical, 6)
                            .background(Capsule().fill(innerState == state ? state.color : Color.white.opacity(0.1)))
                    }
                }
            }
        }
...
struct BookChoosesReaderAnimation: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text(readerReady ? "Consciousness aligned → Teaching appears" : "Not ready → Teaching remains hidden")
                .font(.system(size: 12, weight: .medium))
...
struct ThreeRangesAnimation: View {
    ...
    private func frequencyRow(name: String, color: Color, icon: String, attributes: [String], experience: String) -> some View {
        HStack(spacing: 12) {
            ZStack {
                Circle()
            ...
            ForEach(["Dissolution", "Humility", "Stillness", "Receiving"], id: \.self) { stage in
                VStack(spacing: 6) {
                    Circle()
                        ...
                        .frame(width: 40, height: 40)
                        .overlay(
                            Text("\(["Dissolution", "Humility", "Stillness", "Receiving"].firstIndex(of: stage)! + 1)")
                                .font(.system(size: 14, weight: .bold))
                                .foregroundColor(.cyan)
                        )
...
struct DailyWisdomAnimation: View {
    ...
    var body: some View {
        ZStack {
            ...
            ForEach(0..<8, id: \.self) { i in
                Rectangle()
                    .fill(LinearGradient(colors: [.yellow.opacity(0.8), .orange.opacity(0.3)], startPoint: .bottom, endPoint: .top))
            ...
            // Central sun
            Circle()
                .fill(RadialGradient(colors: [.yellow, .orange], center: .center, startRadius: 0, endRadius: 30))
                .frame(width: 60, height: 60)
