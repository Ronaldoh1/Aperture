...
final class GoogleCalendarManager: ObservableObject {
  ...
  private func fetchEvents(calendarId: String, token: String, timeMin: String, timeMax: String)
    async throws -> [GoogleCalendarEvent]
  {
    ...
    if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 401 {
      ...
      return try await fetchEvents(
        calendarId: calendarId, token: accessToken!, timeMin: timeMin, timeMax: timeMax)
    }
