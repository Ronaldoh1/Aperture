...
class NotificationManager: ObservableObject {
    ...
    for i in 0..<7 {
      ...
      if let futureDate = Calendar.current.date(byAdding: .day, value: i, to: Date()) {
        let dayComponents = Calendar.current.dateComponents(
          [.year, .month, .day], from: futureDate)
  ...
  private func loadSettings() {
    ...
    streakWarningsEnabled =
      UserDefaults.standard.object(forKey: keys.streakWarnings) as? Bool ?? true
    ...
    if hour > 0 || minute > 0 {
      reminderTime =
        Calendar.current.date(from: DateComponents(hour: hour, minute: minute)) ?? reminderTime
    }
  }
...
struct NotificationSettingsView: View {
  ...
  var body: some View {
    List {
      ...
      Section(
        header: Text("Dragon Wisdom"),
        footer: Text("Receive daily wisdom from your Dragon companion at noon.")
      ) {
        Toggle(
          "Daily Dragon Message",
          isOn: Binding(
      ...
      Section(
        header: Text("Practice Prompts"),
        footer: Text("Get brief practice prompts throughout the day to stay present.")
      ) {
        Toggle(
      ...
      Section(
        header: Text("Streak Protection"),
        footer: Text("Get reminded at 8 PM if you haven't practiced that day.")
      ) {
