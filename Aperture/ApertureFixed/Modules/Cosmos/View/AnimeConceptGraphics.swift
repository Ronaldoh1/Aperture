...
struct TwelveHousesView: View {
    ...
    let goldSaints = ["Mu", "Aldebaran", "Saga", "Deathmask", "Aiolia", "Shaka", "Dohko", "Milo", "Aiolos", "Shura", "Camus", "Aphrodite"]
...
struct AnimeConceptsGalleryView: View {
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 30) {
                ...
                // Imposter Pope
                conceptCard(title: "THE HIJACKED INSTITUTION", subtitle: "Truth murdered, replaced by fraud") {
                    ImposterPopeView()
                }
                
                // Sagittarius Saves Athena
                conceptCard(title: "TRUTH-SEEKER SAVES DIVINE", subtitle: "Sagittarius ♐️ — Called 'traitor' for knowing truth") {
    ...
    private func conceptCard<Content: View>(title: String, subtitle: String, @ViewBuilder content: () -> Content) -> some View {
        VStack(spacing: 12) {
            VStack(spacing: 4) {
                Text(title)
                    .font(.system(size: 12, weight: .black, design: .rounded))
