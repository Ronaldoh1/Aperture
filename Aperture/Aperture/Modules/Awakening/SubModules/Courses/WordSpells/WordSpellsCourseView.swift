...
struct WordSpellsCourseView: View {
      ...
      ForEach(0..<15, id: \.self) { i in
        Text(ancientGlyphs[i % ancientGlyphs.count])
          ...
          .position(
            ...
            y: (CGFloat(i) * 60 + particleOffset).truncatingRemainder(
  ...
  private let ancientGlyphs = [
    "𓇳", "𓂀", "א", "ש", "Ω", "☀", "𓆣", "ע", "𓊨", "☥", "𓏏", "ה", "י", "ו", "𓅃",
  ]
  ...
  private var headerSection: some View {
    VStack(spacing: 20) {
      ...
      HStack(spacing: 16) {
        ...
        WordSpellStatPill(
          icon: "text.book.closed", value: "18", label: "Decodes", color: course.color)
  ...
  private var etymologyPreview: some View {
    VStack(spacing: 16) {
      ...
      VStack(spacing: 12) {
        EtymologyRow(
          word: "IS•RA•EL", decode: "ISIS + RA + EL", meaning: "Feminine Divine + Sun + God",
          color: course.color)
        EtymologyRow(
          word: "ISRAELITE", decode: "IS REAL LIGHT", meaning: "One who embodies the light",
          color: course.color)
        EtymologyRow(
          word: "SOUL", decode: "SOL", meaning: "The sun within you", color: course.color)
        EtymologyRow(word: "HOLY", decode: "HELIOS", meaning: "Of the sun god", color: course.color)
        EtymologyRow(
          word: "AMEN", decode: "AMUN", meaning: "Egyptian hidden god", color: course.color)
      }
    }
    .padding(18)
    .background(
  ...
  private var introSection: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      Text(
        "They couldn't destroy the truth. So they hid it in the words themselves—hoping you'd never look."
      )
      ...
      Text(
        "Every prayer, every sermon, every 'holy' word carries the buried light of what they suppressed. The sun worship never ended. It just went underground—into language."
      )
