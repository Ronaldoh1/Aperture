...
enum BeliefBackground: String, CaseIterable, Identifiable {
    ...
    var pathIntro: String {
        switch self {
        case .christian:
            return "You already know the story of Yeshua. But there are gospels they didn't put in the Bible — and they change everything. Your path starts with what was hidden from you."
        case .catholic:
            return "You were raised in the tradition of Rome. The rituals are beautiful — but they're older than Rome itself. Your path reveals what the Church built upon and what it buried."
        case .muslim:
            return "Islam carries deep mystical wisdom through Sufism — the heart of the tradition that institutions sometimes suppress. Your path connects the dots between Fana and Christ Consciousness."
        case .jewish:
            return "Kabbalah is already in your tradition — the mystical current that runs beneath the surface. Your path explores the Sefirot, Ein Sof, and how Gnostic teachings echo Jewish mysticism."
        case .hindu:
            return "You're already closer than most. Kundalini, Atman, Brahman — these are the mechanisms other traditions rediscovered. Your path shows how every awakening tradition maps back to Vedic truth."
        case .buddhist:
            return "Satori, Kensho, Sunyata — you know the territory. Your path explores how Buddhist insight connects to Gnostic direct knowing and the Christ consciousness that transcends religion."
        case .agnostic:
            return "You haven't committed to a story yet — that's actually an advantage. No programming to undo. Your path is pure pattern recognition: what every tradition agrees on, stripped of dogma."
        case .atheist:
            return "You value evidence and reason. Good — you'll need both. Your path is the scientific case: documented history, primary sources, and the measurable patterns that institutions don't want examined."
        case .spiritual:
            return "You feel something but haven't found it in any building. Your path connects the threads you've already been pulling — Kundalini, meditation, energy work — to their original source."
        case .indigenous:
            return "Your ancestors knew the truth before anyone wrote it down. Your path honors that knowing and shows how every 'mystery school' was attempting to rediscover what your people never forgot."
        case .mexican:
            return "Guadalupano/a — before we begin, there's something you need to see about La Virgen. Something beautiful and something stolen. Tap the image when you're ready."
        case .none:
            return "No assumptions. No labels. Just truth. Your path starts with the Timeline — the patterns that emerge when you see history without the filter."
        }
    }
    ...
    var recommendedPath: [PathRecommendation] {
        ...
        case .christian, .catholic:
            return [
                PathRecommendation(module: "Timeline", reason: "See what happened to the original teachings", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Alexandria", reason: "Read the gospels they removed", icon: "books.vertical.fill", priority: 2),
                PathRecommendation(module: "Tonantzin Course", reason: "How they replaced your local gods", icon: "figure.stand.dress", priority: 3),
                PathRecommendation(module: "Gnostic Library", reason: "The Nag Hammadi texts — Yeshua's hidden words", icon: "book.closed.fill", priority: 4),
            ]
        case .muslim:
            return [
                PathRecommendation(module: "Timeline", reason: "Trace the Abrahamic line to its source", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Awakening Paths", reason: "Fana, Sufism, and the mystic heart of Islam", icon: "arrow.triangle.branch", priority: 2),
                PathRecommendation(module: "Alexandria", reason: "Where Judaism, Christianity, and Islam converge", icon: "books.vertical.fill", priority: 3),
            ]
        case .jewish:
            return [
                PathRecommendation(module: "Timeline", reason: "From Sumer to the Sefirot — the hidden thread", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Gnostic Library", reason: "Gnostic Judaism and the Demiurge concept", icon: "book.closed.fill", priority: 2),
                PathRecommendation(module: "Alexandria", reason: "Kabbalah, Ein Sof, and the tree of life", icon: "books.vertical.fill", priority: 3),
            ]
        case .hindu:
            return [
                PathRecommendation(module: "Timeline", reason: "How Vedic truth survived while others burned", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Awakening Paths", reason: "Kundalini mechanics mapped across traditions", icon: "arrow.triangle.branch", priority: 2),
                PathRecommendation(module: "Cosmos", reason: "Sacred geometry encoded in temple architecture", icon: "globe.americas.fill", priority: 3),
            ]
        case .buddhist:
            return [
                PathRecommendation(module: "Timeline", reason: "Buddhism's suppression and survival patterns", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Awakening Paths", reason: "Satori ↔ Gnosis ↔ Moksha — the same thing", icon: "arrow.triangle.branch", priority: 2),
                PathRecommendation(module: "Unlearn", reason: "Emptiness as the ultimate deprogramming", icon: "brain.head.profile", priority: 3),
            ]
        case .agnostic, .atheist:
            return [
                PathRecommendation(module: "Timeline", reason: "The documented pattern of knowledge suppression", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Signal vs Noise", reason: "How media manipulates perception right now", icon: "antenna.radiowaves.left.and.right", priority: 2),
                PathRecommendation(module: "Myths vs Reality", reason: "10 things they taught wrong — with sources", icon: "book.closed.fill", priority: 3),
                PathRecommendation(module: "Alexandria", reason: "Primary texts — judge for yourself", icon: "books.vertical.fill", priority: 4),
            ]
        case .spiritual:
            return [
                PathRecommendation(module: "Timeline", reason: "Where your feelings connect to documented history", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Awakening Paths", reason: "Name what you're experiencing across traditions", icon: "arrow.triangle.branch", priority: 2),
                PathRecommendation(module: "Cosmos", reason: "Sacred geometry — the math behind the feeling", icon: "globe.americas.fill", priority: 3),
                PathRecommendation(module: "We Are One", reason: "The consciousness framework that unifies everything", icon: "sun.max.fill", priority: 4),
            ]
        case .indigenous:
            return [
                PathRecommendation(module: "Timeline", reason: "How they systematically erased your ancestors' truth", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Sun God Erasure", reason: "Inti, Tonatiuh, Kinich Ahau — all targeted", icon: "sun.max.fill", priority: 2),
                PathRecommendation(module: "Tonantzin Course", reason: "The original mother they tried to replace", icon: "figure.stand.dress", priority: 3),
            ]
        case .mexican:
            return [
                PathRecommendation(module: "Guadalupe Reveal", reason: "See what's behind the image", icon: "sun.max.fill", priority: 0),
                PathRecommendation(module: "Tonantzin Course", reason: "The full story of the mother they erased", icon: "figure.stand.dress", priority: 1),
                PathRecommendation(module: "Timeline", reason: "From Tenochtitlan to now — what was taken", icon: "clock.arrow.circlepath", priority: 2),
                PathRecommendation(module: "Alexandria", reason: "The codices they didn't burn", icon: "books.vertical.fill", priority: 3),
            ]
        case .none:
            return [
                PathRecommendation(module: "Timeline", reason: "Start here — let the patterns speak for themselves", icon: "clock.arrow.circlepath", priority: 1),
                PathRecommendation(module: "Signal vs Noise", reason: "Learn to filter truth from manipulation", icon: "antenna.radiowaves.left.and.right", priority: 2),
                PathRecommendation(module: "Alexandria", reason: "Explore at your own pace", icon: "books.vertical.fill", priority: 3),
            ]
        }
    }
}
...
struct GnosticPathfinderModal: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            withAnimation(.easeInOut(duration: 4).repeatForever(autoreverses: true)) { pulsePhase = .pi * 2 }
        }
                        ...
                        Button {
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: 12)
                                    ...
                                    .overlay(RoundedRectangle(cornerRadius: 12).stroke(belief.color.opacity(0.15), lineWidth: 1))
                            )
                        }
                    }
                }
            ...
            VStack(spacing: 24) {
                if let belief = selectedBelief {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        ForEach(Array(belief.recommendedPath.sorted(by: { $0.priority < $1.priority }).enumerated()), id: \.element.id) { index, rec in
    ...
    private func pathStepCard(index: Int, recommendation: PathRecommendation, belief: BeliefBackground) -> some View {
        HStack(spacing: 14) {
            // Step number
            ZStack {
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: 6) {
                    Image(systemName: recommendation.icon).font(.system(size: 12)).foregroundColor(belief.color)
    ...
    private var timelineAlwaysCard: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text("Always explore the Timeline").font(.system(size: 13, weight: .bold)).foregroundColor(.white)
    ...
    private var guadalupeTeaser: some View {
        Button { showGuadalupeReveal = true } label: {
            VStack(spacing: 12) {
...
struct GuadalupeRevealView: View {
    ...
    private var actionArea: some View {
        VStack(spacing: 12) {
            switch phase {
            case .showing:
                Button { startDissolve() } label: {
                    Text("Touch the image")
                        .font(.system(size: 14, weight: .bold))
                        .foregroundColor(Color(red: 1.0, green: 0.85, blue: 0.3))
                        .padding(.horizontal, 24).padding(.vertical, 10)
                        .background(Capsule().fill(Color(red: 1.0, green: 0.85, blue: 0.3).opacity(0.15))
                            .overlay(Capsule().stroke(Color(red: 1.0, green: 0.85, blue: 0.3).opacity(0.3), lineWidth: 1)))
                }
            case .glowing, .dissolving:
                EmptyView()
            case .revealed:
