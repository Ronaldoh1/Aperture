...
final class FlowNotificationManager: ObservableObject {
  ...
  func scheduleFocusBlockReminders(blocks: [FocusBlock]) async {
    ...
    for block in blocks where block.type == .deepWork {
      ...
      startContent.body =
        "\(block.title) starts in \(focusReminderMinutes) min. \(focusMessages.randomElement()!)"
      ...
      if let blockStart = calendar.date(from: triggerComponents),
        let reminderTime = calendar.date(
          byAdding: .minute, value: -focusReminderMinutes, to: blockStart),
        reminderTime > Date()
      {
        let triggerDate = calendar.dateComponents(
          [.year, .month, .day, .hour, .minute], from: reminderTime)
      ...
      if let blockEnd = calendar.date(from: endComponents), blockEnd > Date() {
        let endTriggerDate = calendar.dateComponents(
          [.year, .month, .day, .hour, .minute], from: blockEnd)
