    ...
    VStack(spacing: 16) {
      HStack {
        Image(systemName: "shield.fill")
          .foregroundColor(
            mindShield.shieldStrength > 70
              ? .green : mindShield.shieldStrength > 40 ? .yellow : .red)
      ...
      ZStack(alignment: .leading) {
        ...
        Capsule()
          .fill(
            LinearGradient(
              colors: mindShield.shieldStrength > 70
                ? [.green, .cyan]
                : mindShield.shieldStrength > 40 ? [.yellow, .orange] : [.red, .orange],
              startPoint: .leading,
              endPoint: .trailing
            )
          )
          .frame(
            width: CGFloat(mindShield.shieldStrength) / 100 * (UIScreen.main.bounds.width - 64),
            height: 20)
      ...
      if mindShield.mediaSources.isEmpty {
        ...
        HStack {
          ...
          Text("\(String(format: "%.1f", mindShield.totalHours)) hours")
            ...
            .foregroundColor(
              mindShield.totalHours > 40 ? .red : mindShield.totalHours > 20 ? .yellow : .green)
        }
        .padding(.top, 8)
      }
    }
...
class MindShieldManager: ObservableObject {
  ...
  private func calculateShieldStrength() {
    ...
    let avgManipulation =
      Double(mediaSources.reduce(0) { $0 + $1.manipulationScore }) / Double(mediaSources.count)
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 100))], spacing: 8) {
              ForEach(MediaCategory.allCases, id: \.self) { category in
                Button(action: { selectedCategory = category }) {
                  ...
                  .padding(.vertical, 8)
                  .background(
                    selectedCategory == category ? category.color : Color.white.opacity(0.1)
                  )
                  .cornerRadius(8)
                }
              }
          ...
          VStack(alignment: .leading, spacing: 8) {
            HStack {
              ...
              Text("\(manipulationScore)/10")
                .font(.system(size: 12, weight: .bold))
                .foregroundColor(
                  manipulationScore > 7 ? .red : manipulationScore > 4 ? .orange : .green)
            }
            Slider(
              value: Binding(
