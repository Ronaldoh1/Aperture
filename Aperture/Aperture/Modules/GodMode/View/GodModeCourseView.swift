...
struct GodModeCourseView: View {
  ...
  private func moduleCard(_ module: GodModeModule, index _: Int) -> some View {
          ...
          RoundedRectangle(cornerRadius: 16)
            .stroke(
              isComplete
                ...
                : isUnlocked ? Color(hex: "#FFD700").opacity(0.15) : Color.white.opacity(0.05),
              lineWidth: 1
            )
        )
    )
...
class GodModeProgressManager {
  ...
  func load() -> GodModeProgress {
    guard let data = UserDefaults.standard.data(forKey: key),
      let progress = try? JSONDecoder().decode(GodModeProgress.self, from: data)
    else {
