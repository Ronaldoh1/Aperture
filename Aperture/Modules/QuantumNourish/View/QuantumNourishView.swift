...
struct QuantumNourishView: View {
                    ...
                    VStack(spacing: 24) {
                        ...
                        VStack(spacing: 10) {
                            HStack(spacing: 6) {
                                ...
                                Text("TEST THIS MEAL?").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(Color(hex: "#FF6B35"))
                            }
                            ...
                            HStack {
                                ...
                                Button {
                                    ...
                                    Image(systemName: "arrow.right.circle.fill").font(.system(size: 26)).foregroundColor(Color(hex: "#FF6B35"))
                                }
                            }
                            if !probeResult.isEmpty {
                                Text(probeResult).font(.system(size: 12, weight: .medium, design: .rounded))
                                    ...
                                    .background(RoundedRectangle(cornerRadius: 8).fill(Color(hex: "#FF6B35").opacity(0.06)))
                            }
                        }
                        .padding(14)
                        .background(RoundedRectangle(cornerRadius: 14).fill(Color(hex: "#FF6B35").opacity(0.04))
                            .overlay(RoundedRectangle(cornerRadius: 14).stroke(Color(hex: "#FF6B35").opacity(0.15), lineWidth: 1)))
    ...
    private var heroSection: some View {
        VStack(spacing: 16) {
            ...
            // Stats
            HStack(spacing: 12) {
                miniStat(value: "\(db.allFoods.count)+", label: "Foods Mapped", icon: "leaf.fill", color: Color(hex: "#2ECC71"))
                miniStat(value: "8", label: "Frequency Tiers", icon: "waveform", color: Color(hex: "#9B59B6"))
    ...
    private func miniStat(value: String, label: String, icon: String, color: Color) -> some View {
        VStack(spacing: 4) {
            ...
            Text(value).font(.system(size: 16, weight: .bold, design: .rounded)).foregroundColor(Palette.text.primary)
    ...
    private var searchBar: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14)
                ...
                .overlay(RoundedRectangle(cornerRadius: 14).stroke(Color(hex: "#2ECC71").opacity(0.2), lineWidth: 1))
        )
    }
    ...
    private var searchResultsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if searchResults.isEmpty && searchText.count >= 2 {
                VStack(spacing: 8) {
                    Image(systemName: "magnifyingglass").font(.system(size: 30)).foregroundColor(Palette.text.muted)
    ...
    private var quickActionsGrid: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                actionCard(title: "Vibe Spectrum", subtitle: "Full frequency map", icon: "waveform.path.ecg", color: Color(hex: "#9B59B6"), section: .spectrum)
                actionCard(title: "Scan Ingredients", subtitle: "Camera + OCR", icon: "barcode.viewfinder", color: Color(hex: "#E74C3C"), section: .scanner)
                actionCard(title: "Vibe Tracker", subtitle: "Log daily MHz", icon: "chart.line.uptrend.xyaxis", color: Color(hex: "#3498DB"), section: .tracker)
                actionCard(title: "Six Pillars", subtitle: "Quantum nutrition", icon: "atom", color: Color(hex: "#F1C40F"), section: .pillars)
                actionCard(title: "Pineal Protocol", subtitle: "Decalcify & activate", icon: "eye.fill", color: Color(hex: "#9B59B6"), section: .pineal)
                actionCard(title: "Kill List", subtitle: "Avoid these forever", icon: "xmark.octagon.fill", color: Color(hex: "#E74C3C"), section: .destroyers)
                actionCard(title: "Meal Plan", subtitle: "7-day protocol", icon: "calendar.badge.clock", color: Color(hex: "#2ECC71"), section: .mealPlan)
                actionCard(title: "Cosmic Badges", subtitle: "Earn achievements", icon: "star.circle.fill", color: Color(hex: "#FFD700"), section: .badges)
                actionCard(title: "Challenges", subtitle: "Level up frequency", icon: "flame.fill", color: Color(hex: "#FF6B35"), section: .challenges)
                actionCard(title: "Biometrics", subtitle: "Health data sync", icon: "heart.text.square.fill", color: Color(hex: "#FF6B6B"), section: .biometrics)
                actionCard(title: "Quantum Fitness", subtitle: "Athletic nutrition", icon: "figure.strengthtraining.traditional", color: Color(hex: "#FF6B35"), section: .fitness)
                actionCard(title: "Quantum Cleansing", subtitle: "Fast, detox, juice", icon: "drop.triangle.fill", color: Color(hex: "#1ABC9C"), section: .cleansing)
                // P1 Features
                actionCard(title: "Scan History", subtitle: "Past scans + favorites", icon: "clock.arrow.circlepath", color: Color(hex: "#E74C3C"), section: .scanHistory)
                actionCard(title: "Budget Mode", subtitle: "Eat clean on any $", icon: "dollarsign.circle.fill", color: Color(hex: "#2ECC71"), section: .budgetMode)
                actionCard(title: "Shopping List", subtitle: "Conscious grocery run", icon: "cart.fill", color: Color(hex: "#2ECC71"), section: .shoppingList)
                actionCard(title: "Market Finder", subtitle: "Clean food near you", icon: "map.fill", color: Color(hex: "#3498DB"), section: .marketFinder)
                actionCard(title: "Alternatives", subtitle: "Replace, don't remove", icon: "arrow.triangle.swap", color: Color(hex: "#27AE60"), section: .supplementAlts)
                actionCard(title: "Refeeding Guide", subtitle: "Break fasts safely", icon: "exclamationmark.triangle.fill", color: Color(hex: "#E74C3C"), section: .refeedingGuide)
                // Baby Nutrition
                actionCard(title: "Tiny Humans", subtitle: "Baby nutrition 0-24mo", icon: "figure.and.child.holdinghands", color: Color(hex: "#FFB6C1"), section: .babyNutrition)
            }
        }
    }
    
    private func actionCard(title: String, subtitle: String, icon: String, color: Color, section: NourishSection) -> some View {
        Button {
        ...
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                Text("FREQUENCY SPECTRUM")
                    ...
                    .foregroundColor(Color(hex: "#9B59B6"))
                Spacer()
                Button { selectedSection = .spectrum } label: {
                    Text("VIEW ALL →")
            ...
            HStack {
                ...
                Spacer()
                Text("⚡ TRANSCENDENCE").font(.system(size: 9, weight: .bold)).foregroundColor(Color(hex: "#9933FF"))
            }
        }
        .padding(16)
        .background(
            RoundedRectangle(cornerRadius: 16)
                .fill(Color.white.opacity(0.04))
                .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color(hex: "#9B59B6").opacity(0.15), lineWidth: 1))
        )
    }
        ...
        Button {
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 12).stroke(pillar.color.opacity(0.2), lineWidth: 1))
            )
        }
        .buttonStyle(PlainButtonStyle())
    }
    ...
    private var dailyAffirmation: some View {
        VStack(spacing: 10) {
            Text("🐉")
                .font(.system(size: 24))
            Text("\"Your body is a temple. But what frequency is that temple vibrating at? Every meal is a vote — for consciousness or for sleep. Choose wisely.\"")
        ...
        Button {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        if !food.effects.isEmpty {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("EFFECTS")
                                    .font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(food.tier.color)
                                ForEach(food.effects, id: \.self) { effect in
                                    HStack(spacing: 6) {
                                        Circle().fill(food.tier.color).frame(width: 4, height: 4)
                                        Text(effect).font(.system(size: 12, weight: .medium)).foregroundColor(Palette.text.secondary)
                                    }
                                }
                            }
                        }
                        ...
                        if !food.avoidReasons.isEmpty {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("FOUND IN").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.red)
                                ForEach(food.avoidReasons, id: \.self) { reason in
                                    HStack(spacing: 6) {
                                        Image(systemName: "exclamationmark.triangle.fill").font(.system(size: 8)).foregroundColor(.red)
                                        Text(reason).font(.system(size: 12, weight: .medium)).foregroundColor(Palette.text.secondary)
                                    }
                                }
                            }
                        }
                        ...
                        if !food.alternatives.isEmpty {
                            VStack(alignment: .leading, spacing: 6) {
                                Text("SWAP FOR").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.green)
                                HStack(spacing: 6) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 14).stroke(food.tier.color.opacity(expanded ? 0.2 : 0.1), lineWidth: 1))
            )
        }
        .buttonStyle(PlainButtonStyle())
    }
