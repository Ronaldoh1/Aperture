...
enum HygieneLevel: Int, CaseIterable, Codable, Identifiable {
  ...
  var morningMessage: String {
    ...
    case .level1:
      return
        "Good morning. Today's goal: just NOTICE how many times you open social media. Don't judge it. Just count. Awareness is step one."
    case .level2:
      return
        "Good morning. Your budget today: 2.5 hours. Before each session, ask: am I seeking information or stimulation? Set a timer when you open an app."
    case .level3:
      return
        "Good morning. Your budget: 1 hour. Only open social media with a specific purpose. When that purpose is complete, close it. You're training sovereignty."
    case .level4:
      return
        "Good morning. Your check-in windows: morning (15 min) and evening (15 min). Outside those windows, your attention belongs to your REAL life."
    case .level5:
      return
        "Good morning. You own your attention. If social media serves a purpose today, use it briefly and intentionally. If not, don't open it. You are free."
    }
  }
...
struct ReactiveContentAnalysis: Identifiable, Codable {
  ...
  init(
    id: String = UUID().uuidString, content: String, analyzedAt: Date = Date(),
    ...
    self.reactivityScore = reactivityScore
    self.triggers = triggers
    self.recommendation = recommendation
    self.breathingAdvice = breathingAdvice
  }
}
...
struct ReactivityTrigger: Identifiable, Codable {
  ...
  init(
    id: String = UUID().uuidString, name: String, description: String, nervousSystemEffect: String
  ) {
    self.id = id
    self.name = name
...
struct HygieneCheckIn: Identifiable, Codable {
  ...
  init(
    id: String = UUID().uuidString, date: Date = Date(), reportedMinutes: Int,
...
final class ReactiveContentEngine {
  ...
  func analyze(content: String) -> ReactiveContentAnalysis {
    ...
    if rageScore > 0 {
      triggers.append(
        ReactivityTrigger(
          name: "Rage Bait",
          description:
            "This content uses emotionally loaded language designed to make you angry before you think. Loaded words: the linguistic equivalent of a sucker punch.",
          nervousSystemEffect:
            "Cortisol spike → fight-or-flight activation → prefrontal cortex (rational brain) goes offline → you react instead of respond"
        ))
    ...
    if tribalScore > 0 {
      triggers.append(
        ReactivityTrigger(
          ...
          description:
            "This content activates your 'us vs them' wiring. It wants you to identify with a group and see another group as the enemy. This is how the outrage economy works.",
          nervousSystemEffect:
            "Amygdala activation → tribal identity override → nuanced thinking becomes impossible → you share to signal which 'team' you're on"
        ))
    ...
    if fearScore > 0 {
      triggers.append(
        ReactivityTrigger(
          ...
          description:
            "Scarcity language, secrecy framing, urgency cues. This content says 'ACT NOW before it's too late.' Real information doesn't expire in 24 hours.",
          nervousSystemEffect:
            "Anxiety spike → hypervigilance → compulsive checking/sharing → dopamine hit from 'protecting' your group"
        ))
    ...
    if strawScore > 0 {
      triggers.append(
        ReactivityTrigger(
          ...
          description:
            "This takes something someone said and reframes it in the worst possible way. 'So you're saying...' almost never reflects what was actually said. It's interpretation disguised as quotation.",
          nervousSystemEffect:
            "Moral outrage → feeling of righteousness → dopamine reward for 'catching' someone → sharing to perform moral superiority"
        ))
    ...
    if capsWords.count >= 3 {
      triggers.append(
        ReactivityTrigger(
          ...
          description:
            "ALL CAPS is the text equivalent of someone grabbing your shoulders and shaking you. It bypasses reading comprehension and hits your nervous system directly.",
          nervousSystemEffect:
            "Heightened alertness → perceived urgency → reduced critical evaluation → impulse to act/share immediately"
        ))
    ...
    if exclamations >= 3 || questions >= 3 {
      triggers.append(
        ReactivityTrigger(
          ...
          description:
            "Excessive punctuation (!!! or ???) is designed to make you feel the 'energy' of outrage or disbelief. It's emotional contagion through typography.",
          nervousSystemEffect:
            "Mirror neurons activate → you 'feel' the poster's emotion → emotional contagion spreads without any actual new information"
        ))
    ...
    if normalizedScore >= 70 {
      recommendation =
        "This content is HIGHLY reactive. It's designed to make you feel, not think. Do NOT engage, share, or reply right now. Come back in 30 minutes if you still care."
    } else if normalizedScore >= 40 {
      recommendation =
        "This content has reactive elements. Before engaging, ask: what is the FACTUAL claim here? Can I verify it? Is my reaction being manufactured?"
    } else if normalizedScore >= 20 {
      recommendation =
        "Mild reactive elements detected. This may be worth engaging with — just pause and breathe first. Respond, don't react."
    } else {
      recommendation =
        "Low reactivity detected. This content appears relatively neutral. Still worth verifying any claims before sharing."
    }
    ...
    if normalizedScore >= 50 {
      breathingAdvice =
        "Before doing ANYTHING with this content: 4 breaths. In for 4 counts, hold for 4, out for 4. This re-engages your prefrontal cortex. React after breathing, not before."
    } else {
      breathingAdvice =
        "Quick check: notice your body. Jaw clenched? Shoulders tight? That's your nervous system responding to content designed to trigger it. Relax, then decide."
    }
...
class SocialHygieneManager: ObservableObject {
  ...
  var isUnderBudget: Bool {
    guard let today = checkIns.first, Calendar.current.isDateInToday(today.date) else {
      return true
    }
  ...
  func scheduleMorningReminder() {
    ...
    center.requestAuthorization(options: [.alert, .sound]) { granted, _ in
      ...
      let request = UNNotificationRequest(
        identifier: "aperture.hygiene.morning", content: content, trigger: trigger)
      center.add(request)
    }
  ...
  private func load() {
    if let data = UserDefaults.standard.dictionary(forKey: dataKey) {
      if let lvl = data["level"] as? Int, let level = HygieneLevel(rawValue: lvl) {
        currentLevel = level
      }
...
struct SocialMediaHygieneView: View {
  ...
  var body: some View {
    NavigationStack {
      ...
      .sheet(isPresented: $showCheckIn) { DailyCheckInView(manager: manager) }
  ...
  private var heroSection: some View {
    VStack(spacing: 10) {
      ...
      Text("DIGITAL HYGIENE").font(.system(size: 10, weight: .bold)).tracking(4).foregroundColor(
        .cyan.opacity(0.4))
    ...
    Button {
      ...
      HStack(spacing: 14) {
        ZStack {
          ...
          Image(systemName: manager.currentLevel.icon).font(.system(size: 20)).foregroundColor(
            manager.currentLevel.color)
        }
        VStack(alignment: .leading, spacing: 3) {
          HStack(spacing: 6) {
            Text("Level \(manager.currentLevel.rawValue)").font(.system(size: 10, weight: .bold))
              .tracking(1).foregroundColor(manager.currentLevel.color)
            Text(manager.currentLevel.name).font(.system(size: 14, weight: .bold)).foregroundColor(
              .white)
          }
          Text("Budget: \(manager.dailyGoalMinutes) min/day").font(.system(size: 11))
      ...
      .padding(14)
      .background(
        RoundedRectangle(cornerRadius: 14).fill(manager.currentLevel.color.opacity(0.06))
          .overlay(
            RoundedRectangle(cornerRadius: 14).stroke(
              manager.currentLevel.color.opacity(0.15), lineWidth: 1)))
    }
  }
  ...
  private var dailyStatusCard: some View {
    HStack(spacing: 0) {
      ...
      statCell(
        "\(manager.averageMinutesLast7)m", "Avg/Day",
        manager.averageMinutesLast7 <= manager.dailyGoalMinutes ? .green : .red)
  ...
  private var modulesGrid: some View {
    VStack(spacing: 8) {
      HStack {
        Text("TOOLS").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(
          .white.opacity(0.3))
        Spacer()
      }
      ...
      // Row 1
      HStack(spacing: 10) {
        moduleButton("Scan Content", "antenna.radiowaves.left.and.right", .red) {
          showScanner = true
        }
      ...
      HStack(spacing: 10) {
        ...
        moduleButton("Change Level", "slider.horizontal.3", manager.currentLevel.color) {
          showLevelPicker = true
        }
      }
    }
  ...
  private func moduleButton(
    ...
    Button(action: action) {
      ...
      .frame(maxWidth: .infinity)
      .background(
        RoundedRectangle(cornerRadius: 12).fill(color.opacity(0.06))
  ...
  private var morningReminderCard: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack(spacing: 8) {
        ...
        Text("MORNING REMINDER").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(
          .orange.opacity(0.6))
        Spacer()
        Toggle(
          "",
          isOn: Binding(
            ...
            set: { on in
              if on { manager.scheduleMorningReminder() } else { manager.cancelMorningReminder() }
            }
          )
        )
      ...
      if manager.morningReminderEnabled {
        Text(
          "Every morning at \(String(format: "%d:%02d", manager.morningReminderHour, manager.morningReminderMinute)) AM:"
        )
    ...
    .padding(14)
    .background(
      RoundedRectangle(cornerRadius: 14).fill(Color.orange.opacity(0.04))
      ...
      VStack(spacing: 0) {
        ...
        TabView(selection: $currentPage) {
          ForEach(Array(lessons.enumerated()), id: \.offset) { index, lesson in
            ScrollView(showsIndicators: false) {
              VStack(spacing: 20) {
                Image(systemName: lesson.icon).font(.system(size: 36)).foregroundColor(.cyan)
                  .padding(.top, 30)
        ...
        HStack(spacing: 16) {
          if currentPage > 0 {
            Button {
              ...
              Text("Back").font(.system(size: 14, weight: .medium)).foregroundColor(
                .white.opacity(0.4))
            }
          }
          Spacer()
    ...
    NavigationStack {
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 16) {
            Text(
              "Paste any social media post, tweet, headline, or comment.\nWe'll tell you what it's doing to your nervous system."
            )
    ...
    VStack(alignment: .leading, spacing: 14) {
      ...
      HStack(spacing: 12) {
        ZStack {
          ...
          Circle().trim(from: 0, to: CGFloat(a.reactivityScore) / 100)
            .stroke(
              scoreColor(a.reactivityScore), style: StrokeStyle(lineWidth: 4, lineCap: .round)
            )
        ...
        VStack(alignment: .leading, spacing: 2) {
          ...
          Text(scoreLabel(a.reactivityScore)).font(.system(size: 11)).foregroundColor(
            scoreColor(a.reactivityScore))
        }
        Spacer()
      }
      ...
      VStack(alignment: .leading, spacing: 4) {
        HStack(spacing: 6) {
          ...
          Text("BEFORE YOU REACT").font(.system(size: 9, weight: .bold)).tracking(1)
            .foregroundColor(.cyan.opacity(0.6))
        }
        Text(a.breathingAdvice).font(.system(size: 12, weight: .medium)).foregroundColor(
          .cyan.opacity(0.8)
        ).lineSpacing(3)
      }
      ...
      if !a.triggers.isEmpty {
        VStack(alignment: .leading, spacing: 8) {
          Text("MANIPULATION TRIGGERS DETECTED").font(.system(size: 9, weight: .bold)).tracking(1)
            .foregroundColor(.red.opacity(0.6))
          ForEach(a.triggers) { trigger in
            VStack(alignment: .leading, spacing: 6) {
              Text(trigger.name).font(.system(size: 13, weight: .bold)).foregroundColor(.white)
              Text(trigger.description).font(.system(size: 11)).foregroundColor(.white.opacity(0.6))
                .lineSpacing(2)
              HStack(spacing: 4) {
                Image(systemName: "brain.head.profile").font(.system(size: 10)).foregroundColor(
                  .orange)
      ...
      // Recommendation
      VStack(alignment: .leading, spacing: 4) {
        Text("RECOMMENDATION").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(
          .green.opacity(0.6))
        Text(a.recommendation).font(.system(size: 12, weight: .medium)).foregroundColor(
          .green.opacity(0.8)
        ).lineSpacing(3)
      }
      ...
      RoundedRectangle(cornerRadius: 16).fill(Color.white.opacity(0.03))
        .overlay(
          RoundedRectangle(cornerRadius: 16).stroke(
            scoreColor(a.reactivityScore).opacity(0.15), lineWidth: 1)))
  }
      ...
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 8) {
              HStack {
                Text("Social Media Time").font(.system(size: 13, weight: .bold)).foregroundColor(
                  .white)
                Spacer()
            ...
            VStack(alignment: .leading, spacing: 8) {
              HStack {
                Text("Times You Got Triggered").font(.system(size: 13, weight: .bold))
                  .foregroundColor(.white)
                Spacer()
                Text("\(Int(triggerCount))").font(.system(size: 15, weight: .bold)).foregroundColor(
                  .orange)
              }
            ...
            // Mood
            VStack(alignment: .leading, spacing: 8) {
              Text("Overall Mood After Social Media").font(.system(size: 13, weight: .bold))
                .foregroundColor(.white)
              HStack(spacing: 12) {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 16) {
            ...
            HStack(spacing: 10) {
              ...
              Text("Accounts Unfollowed:").font(.system(size: 13, weight: .medium)).foregroundColor(
                .white.opacity(0.5))
              Spacer()
              Text("\(manager.unfollowedCount)").font(.system(size: 20, weight: .bold))
            ...
            .padding(12).background(
              RoundedRectangle(cornerRadius: 12).fill(Color.orange.opacity(0.06)))

            ForEach(Array(EchoChamberDatabase.declutterChecklist.enumerated()), id: \.offset) {
              _, category in
              VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 8) {
                  ...
                  Button {
                    ...
                    HStack(alignment: .top, spacing: 10) {
                      ...
                      Text(item)
                        ...
                        .foregroundColor(
                          completedItems.contains(item) ? .white.opacity(0.3) : .white.opacity(0.7)
                        )
                  ...
                  .overlay(
                    RoundedRectangle(cornerRadius: 14).stroke(
                      category.icon == "sparkles"
                        ? Color.green.opacity(0.1) : Color.white.opacity(0.05), lineWidth: 1
                    )))
            }
              ...
              VStack(alignment: .leading, spacing: 0) {
                Button {
                  ...
                  HStack(spacing: 12) {
                    ZStack {
                      ...
                      Image(systemName: level.icon).font(.system(size: 16)).foregroundColor(
                        level.color)
                    }
                    VStack(alignment: .leading, spacing: 3) {
                      HStack(spacing: 6) {
                        Text("Level \(level.rawValue)").font(.system(size: 10, weight: .bold))
                          .foregroundColor(level.color)
                        Text(level.name).font(.system(size: 14, weight: .bold)).foregroundColor(
                          .white)
                      ...
                      Text(level.description).font(.system(size: 11)).foregroundColor(
                        .white.opacity(0.4))
                      Text("Budget: \(level.dailyBudgetMinutes) min/day").font(
                ...
                if expandedLevel == level {
                  VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("DAILY PROTOCOL").font(.system(size: 9, weight: .bold)).tracking(1)
                  ...
                  .overlay(
                    RoundedRectangle(cornerRadius: 14).stroke(
                      manager.currentLevel == level
                        ? level.color.opacity(0.2) : Color.white.opacity(0.04), lineWidth: 1
                    )))
            }
      ...
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 16) {
            ...
            HStack(spacing: 20) {
              VStack(spacing: 4) {
                Text("Level").font(.system(size: 9, weight: .bold)).foregroundColor(
                  .white.opacity(0.3))
                ZStack {
                  Circle().fill(manager.currentLevel.color.opacity(0.15)).frame(
                    width: 56, height: 56)
                  Text("\(manager.currentLevel.rawValue)").font(.system(size: 22, weight: .bold))
                    .foregroundColor(manager.currentLevel.color)
                }
                Text(manager.currentLevel.name).font(.system(size: 10, weight: .bold))
                  .foregroundColor(manager.currentLevel.color)
              }
              VStack(spacing: 4) {
                Text("Streak").font(.system(size: 9, weight: .bold)).foregroundColor(
                  .white.opacity(0.3))
                Text("\(manager.streakDays)").font(.system(size: 28, weight: .bold))
                  .foregroundColor(.green)
                Text("days on budget").font(.system(size: 10)).foregroundColor(.white.opacity(0.3))
              }
              VStack(spacing: 4) {
                Text("Avg 7d").font(.system(size: 9, weight: .bold)).foregroundColor(
                  .white.opacity(0.3))
            ...
            if !manager.checkIns.isEmpty {
              VStack(alignment: .leading, spacing: 8) {
                Text("RECENT CHECK-INS").font(.system(size: 10, weight: .bold)).tracking(2)
                ...
                ForEach(manager.checkIns.prefix(14)) { checkIn in
                  HStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                      HStack(spacing: 6) {
                        Text("\(checkIn.reportedMinutes) min").font(
                          .system(size: 12, weight: .bold)
                        )
                        .foregroundColor(
                          checkIn.reportedMinutes <= manager.dailyGoalMinutes ? .green : .red)
                        Text("\(checkIn.triggerCount) triggers").font(.system(size: 10))
                          .foregroundColor(.orange)
                      }
                      if let note = checkIn.note {
                        Text(note).font(.system(size: 10)).foregroundColor(.white.opacity(0.3))
                          .lineLimit(1)
                      }
                    }
                    Spacer()
                    Text(checkIn.date, style: .date).font(.system(size: 9)).foregroundColor(
                      .white.opacity(0.2))
                  }
                  .padding(8).background(
                    RoundedRectangle(cornerRadius: 8).fill(Color.white.opacity(0.02)))
                }
              }
            }
            ...
            if !manager.scanHistory.isEmpty {
              VStack(alignment: .leading, spacing: 8) {
                Text("RECENT SCANS").font(.system(size: 10, weight: .bold)).tracking(2)
                  .foregroundColor(.white.opacity(0.3))
                ForEach(manager.scanHistory.prefix(5)) { scan in
                  HStack(spacing: 8) {
                    Text("\(scan.reactivityScore)")
                      .font(.system(size: 14, weight: .bold))
                      .foregroundColor(
                        scan.reactivityScore >= 70
                          ? .red : scan.reactivityScore >= 40 ? .orange : .green
                      )
                      .frame(width: 30)
                    Text(scan.content).font(.system(size: 11)).foregroundColor(.white.opacity(0.5))
                      .lineLimit(2)
                    Spacer()
                  }
                  .padding(8).background(
            ...
            // Stats
            VStack(alignment: .leading, spacing: 8) {
              Text("STATS").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(
                .white.opacity(0.3))
