...
struct FinancialLiteracyView: View {
  ...
  private func moduleHeader(icon: String, title: String, lessonCount: Int, completedCount: Int)
    -> some View
  {
    HStack(spacing: 12) {
  ...
  private func lessonRow(
    id: String, title: String, duration: Int, isCompleted: Bool, onTap _: @escaping () -> Void
  ) -> some View {
    Button(action: {
