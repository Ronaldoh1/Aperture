...
struct OfferAnalyzerView: View {
    ...
    HStack(alignment: .top, spacing: 10) {
      ...
      VStack(alignment: .leading, spacing: 4) {
        Text("Your Privacy is Protected")
          ...
          .foregroundColor(.white)
        Text(
          "Analysis happens on-device using local AI. Your offer letter is NEVER uploaded to any server. Data is deleted after analysis."
        )
        .font(.system(size: 11))
        .foregroundColor(.gray)
      }
  ...
  private func extractOfferDetails(from text: String) -> OfferAnalysis {
    ...
    var salary: Double = 100_000  // Default
    if let match = salaryRegex?.firstMatch(
      ...
      let range = Range(match.range(at: 1), in: text)
    {
      ...
      salary = Double(salaryString) ?? 100_000
    }
    ...
    if lowercaseText.contains("non-compete") {
      redFlags.append(
        EquityRedFlag(
          ...
          description: "Restricts your future employment options. Consider negotiating removal.",
          severity: 4))
    }
    if lowercaseText.contains("at-will") {
      redFlags.append(
        EquityRedFlag(
          ...
          description: "Either party can terminate without cause. Standard but worth noting.",
          severity: 2))
    }
    if lowercaseText.contains("unlimited pto") || lowercaseText.contains("unlimited vacation") {
      redFlags.append(
        EquityRedFlag(
          title: "Unlimited PTO",
          description: "Often means people take less vacation. Ask about average days taken.",
          severity: 3))
    }

    // Detect green flags
    var greenFlags: [GreenFlag] = []
    if lowercaseText.contains("equity") || lowercaseText.contains("stock")
      || lowercaseText.contains("rsu")
    {
      greenFlags.append(
        GreenFlag(
          title: "Equity Compensation",
          description: "Potential for additional upside if company performs well"))
    }
    ...
    if lowercaseText.contains("remote") || lowercaseText.contains("work from home") {
      greenFlags.append(
        GreenFlag(title: "Remote Work", description: "Flexibility and savings on commute"))
    }

    // Market comparison (simplified - would use real data in production)
    let marketComparison = MarketComparison(
      percentile: min(95, max(5, Int((salary / 150_000) * 100))),
    ...
    let suggestedCounter =
      salary < marketComparison.marketMedian ? marketComparison.marketMedian : salary * 1.15
...
struct WealthSimulatorView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      // Income Slider
      sliderSection("Annual Income", value: $income, range: 30000...300_000, format: "$%.0f")
