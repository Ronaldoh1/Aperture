...
struct CosmosView: View {
  ...
  private var realmsListSection: some View {
    let realmsList: [Realm] = Array(realms)
    VStack(spacing: 0) {
  ...
  @ViewBuilder
  private func realmCard(for realmsList: [Realm], at index: Int) -> some View {
    if index < realmsList.count {
      let realm: Realm = realmsList[index]
      RealmCard(
        realm: realm,
...
struct EntityDetailView: View {
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 24) {
            ...
            if !entity.symbolism.isEmpty {
              detailSection(
                title: "Symbolism", content: entity.symbolism, color: Palette.primary.violet)
            }
            ...
            if !entity.gnosticTeaching.isEmpty {
              detailSection(
                title: "Gnostic Teaching", content: entity.gnosticTeaching,
                color: Palette.accent.gold, icon: "book.fill")
            }
  ...
  private func detailSection(
    title: String, content: String, color: Color = Palette.text.secondary,
        ...
        ScrollView {
          VStack(spacing: 20) {
            ...
            HStack(alignment: .top, spacing: 10) {
              ...
              Text(
                "\"They wrote the book. They edited the book. They translated the book. They taught you to never question the book. Maybe start there.\""
              )
              .font(.system(size: 13, weight: .medium, design: .rounded))
              .italic()
              .foregroundColor(Palette.accent.gold)
