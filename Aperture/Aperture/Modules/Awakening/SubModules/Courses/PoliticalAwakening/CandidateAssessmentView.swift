...
struct CandidateAssessmentView: View {
  ...
  private var inputPromptSection: some View {
    VStack(spacing: 20) {
      ...
      Text(
        "We'll calculate their Integrity Score based on publicly available data from FEC filings, voting records, and organizational affiliations."
      )
      .font(.system(size: 14))
      .foregroundColor(.gray)
    ...
    VStack(spacing: 16) {
      ...
      HStack {
        ...
        ZStack {
          ...
          Image(
            systemName: candidate.integrityScore >= 70
  ...
  private func recommendedActions(_: CandidateData) -> some View {
    VStack(alignment: .leading, spacing: 16) {
...
class CandidateAssessmentViewModel: ObservableObject {
  ...
  func loadCandidate(name _: String, position _: String, party _: String) {
    ...
    CandidateData(
      ...
      redFlags: [
        RedFlag(
          ...
          description:
            "Received $2.3M from America First PAC, primarily funded by fossil fuel interests.",
          severity: .high,
        ...
        RedFlag(
          title: "ALEC Member",
          description:
            "Listed as member of American Legislative Exchange Council, which drafts corporate-friendly legislation.",
          severity: .high,
          source: "ALEC Exposed"
        ),
        RedFlag(
