...
struct CameraVibeScannerView: View {
      ...
      VStack(spacing: 12) {
        ...
        ScrollView(.horizontal, showsIndicators: false) {
          HStack(spacing: 8) {
            ...
            testLabel(
              "🍕 Hot Pocket",
              "Enriched Flour, Water, Reduced Fat Mozzarella Cheese, Pepperoni, Soybean Oil, Modified Food Starch, Sodium Phosphates, BHT, TBHQ, Artificial Flavor"
            )
            testLabel(
              "🍫 Snickers",
              "Milk Chocolate, Peanuts, Corn Syrup, Sugar, Palm Oil, Skim Milk, Lactose, Salt, Egg Whites, Artificial Flavor, Soy Lecithin"
            )
            testLabel(
              "🥗 Clean Salad",
              "Organic Mixed Greens, Avocado, Cherry Tomatoes, Hemp Seeds, Extra Virgin Olive Oil, Lemon Juice, Pink Himalayan Salt"
            )
            testLabel(
              "🧃 Naked Juice",
              "Apple Juice, Mango Puree, Orange Juice, Banana Puree, Natural Flavors")
          }
          .padding(.horizontal, 20)
        }
      }
  ...
  private var verdictBanner: some View {
    ...
    let highCount = scanResults.filter { $0.dangerLevel == .high || $0.dangerLevel == .superHigh }
      .count
    ...
    return VStack(spacing: 14) {
      ...
      HStack(spacing: 20) {
        ...
        verdictStat(
          count: scanResults.filter { $0.dangerLevel == .neutral || $0.dangerLevel == .mediumLow }
            .count, label: "NEUTRAL", color: .yellow)
    ...
    .background(
      RoundedRectangle(cornerRadius: 18)
        ...
        .overlay(
          RoundedRectangle(cornerRadius: 18).stroke(overallColor.opacity(0.15), lineWidth: 1))
    )
  }

  private func verdictStat(count: Int, label: String, color: Color) -> some View {
    VStack(spacing: 2) {
      Text("\(count)").font(.system(size: 20, weight: .bold, design: .rounded)).foregroundColor(
        color)
  ...
  private var vibeBreakdownChart: some View {
    ...
    let maxCount =
      tiers.map { tier in scanResults.filter { $0.dangerLevel == tier }.count }.max() ?? 1
      ...
      RoundedRectangle(cornerRadius: 12)
        ...
        .overlay(
          RoundedRectangle(cornerRadius: 12).stroke(
            result.dangerLevel.color.opacity(0.1), lineWidth: 1))
    )
  }
  ...
  private func analyzeScannedIngredients() {
    ...
    scanResults = ingredients.map { ingredient in
      matchIngredientAdvanced(ingredient)
    }

    let destructiveCount = scanResults.filter { $0.dangerLevel == .destructive }.count
    let acidicCount = scanResults.filter { $0.dangerLevel == .acidic }.count
    let highCount = scanResults.filter {
      $0.dangerLevel == .high || $0.dangerLevel == .superHigh || $0.dangerLevel == .mediumHigh
    }.count

    if destructiveCount >= 3 {
      overallVerdict =
        "☠️ CONSCIOUSNESS KILLER\n\(destructiveCount) toxic ingredients — avoid completely"
      overallColor = .red
  ...
  private func matchIngredientAdvanced(_ ingredient: String) -> IngredientScanResult {
    ...
    let destructiveKeywords: [(String, String, [String])] = [
      (
        ...
        "HFCS — mercury-contaminated sweetener that bypasses satiety signals. Metabolic weapon found in most processed foods.",
        ["Raw honey", "Maple syrup"]
      ),
      (
        "caramel color",
        "Contains 4-MEI, a known carcinogen. Added purely for appearance — provides zero nutritional value.",
        ["Avoid — purely cosmetic"]
      ),
      (
        "phosphoric acid",
        "Strips calcium from bones. pH 2.5 — more acidic than vinegar. Found in cola drinks.",
        ["Sparkling water with lemon"]
      ),
      (
        "natural flavors",
        "Catch-all term hiding 100+ chemical compounds including MSG. 'Natural' means nothing legally.",
        ["Real herbs and spices"]
      ),
      (
        "artificial flavor",
        "Synthetic chemicals engineered to trick your brain. No nutritional value, pure deception.",
        ["Real food flavoring"]
      ),
      (
        "sodium phosphate",
        "Disrupts mineral balance. Accelerates aging at cellular level. Kidney stress.",
        ["Sea salt"]
      ),
      (
        "bht",
        "Butylated Hydroxytoluene — petroleum-derived preservative. Banned in several countries. Endocrine disruptor.",
        ["Vitamin E (natural preservative)"]
      ),
      (
        "bha",
        "Butylated Hydroxyanisole — 'reasonably anticipated to be a human carcinogen' per NIH. Still FDA approved.",
        ["Rosemary extract"]
      ),
      (
        "tbhq",
        "Tertiary Butylhydroquinone — derived from petroleum. Linked to vision disturbances, liver effects.",
        ["Natural antioxidants"]
      ),
      (
        "modified food starch",
        "Chemically altered starch — your body doesn't recognize it. Creates inflammatory response.",
        ["Tapioca, arrowroot"]
      ),
      (
        "soy lecithin",
        "95% of US soy is GMO. Highly processed emulsifier. Endocrine disrupting properties.",
        ["Sunflower lecithin"]
      ),
      (
        "enriched flour",
        "Stripped of all nutrition, then synthetic vitamins added back. Dead food wearing a costume.",
        ["Sprouted grain flour", "Almond flour"]
      ),
      (
        "corn syrup",
        "Processed corn sugar. Spikes insulin. Often from GMO corn. Feeds candida overgrowth.",
        ["Raw honey", "Dates"]
      ),
      (
        "palm oil",
        "Inflammatory omega-6 heavy. Environmentally destructive. Often contains processing contaminants.",
        ["Coconut oil", "Olive oil"]
      ),
      (
        "soybean oil",
        "Highly processed, GMO, omega-6 inflammatory. In nearly every processed food.",
        ["Avocado oil", "Olive oil"]
      ),
      (
        "red 40",
        "Petroleum-derived dye. Linked to hyperactivity, behavioral issues. Banned in several EU countries.",
        ["Beet juice (natural color)"]
      ),
      (
        "yellow 5", "Tartrazine — coal tar derivative. Allergenic. Behavioral effects in children.",
        ["Turmeric (natural color)"]
      ),
      (
        "blue 1",
        "Brilliant Blue — synthetic dye that crosses the blood-brain barrier. Why is this in food?",
        ["Spirulina (natural blue)"]
      ),
      (
        "sodium benzoate",
        "When combined with vitamin C, forms benzene (carcinogen). Common in acidic drinks.",
        ["Citric acid preservation"]
      ),
      (
        "sucralose",
        "Splenda — chlorinated sugar molecule. Destroys gut bacteria. 50% stays in body.",
        ["Stevia", "Monk fruit"]
      ),
      (
        "maltodextrin",
        "Higher glycemic index than table sugar (GI: 110 vs 65). Spikes blood sugar violently.",
        ["Coconut sugar"]
      ),
      (
        "artificial color",
        "Petroleum-derived compounds added to make dead food look alive. Pure deception.",
        ["Natural plant-based colors"]
      ),
      (
        "aspartame",
        "Breaks down to methanol → formaldehyde in your body. Excitotoxin that kills neurons.",
        ["Stevia leaf", "Raw honey"]
      ),
    ]
    ...
    let acidicKeywords: [(String, String)] = [
      (
        "sugar",
        "Refined sugar feeds candida, inflammation, and addiction. Consciousness suppressant."
      ),
      ...
      (
        "egg whites",
        "Isolated from whole food context. Avidin blocks biotin absorption when processed."
      ),
      ("canola", "Rapeseed oil — requires hexane extraction. Inflammatory omega-6 dominant."),
      (
        "peanut",
        "High in inflammatory omega-6. Often contaminated with aflatoxin (carcinogenic mold)."
      ),
    ...
    let highVibeKeywords: [(String, String)] = [
      (
        "organic",
        "Organic certification means no synthetic pesticides or GMOs. Higher biophotonic activity."
      ),
      ("hemp", "Complete protein with perfect omega ratio. Anti-inflammatory. Brain food."),
      (
        "avocado",
        "Heart-healthy monounsaturated fats. Hormone building blocks. Nutrient absorption booster."
      ),
      (
        "olive oil",
        "Extra virgin = liquid gold. Anti-inflammatory oleocanthal. Mediterranean life force."
      ),
      (
        "lemon",
        "Despite being acidic, lemons are alkaline-forming in the body. Vitamin C powerhouse."
      ),
      (
        "himalayan salt",
        "84 trace minerals your body needs. Unprocessed. Energetically coherent crystal structure."
      ),
      ...
      (
        "ginger",
        "Universal medicine in Ayurveda. Increases digestive fire. Clears energy channels."
      ),
    ]
    ...
    let neutralKeywords: [(String, String)] = [
      ...
      (
        "milk chocolate",
        "Processed cacao with sugar and dairy. The cacao is good; the rest is not."
      ),
    ]
    ...
    return IngredientScanResult(
      ...
      dangerLevel: .neutral,
      explanation:
        "Not in our database. Research this ingredient independently — if you can't pronounce it, your body probably can't process it.",
      alternatives: []
    )
  }
