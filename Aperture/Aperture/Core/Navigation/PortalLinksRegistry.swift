...
enum PortalLinks {
  ...
  static func toDragon() -> some View {
    PortalLinkButton("Badge Collection", icon: "seal.fill", destination: .badges, color: .purple)
  }
}
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        RelatedContentSection(
          ...
          links: [
            (
              "Apocryphon of John", "The secret teaching about the Demiurge",
              .alexandria(textId: "apocryphon_john"), .green
            ),
            (
              "Kundalini Awakening", "Energy rising through chakras",
              .awakeningPath(pathId: "kundalini"), .orange
            ),
            (
              "Nag Hammadi Discovery", "When the texts were found",
              .timeline(eventId: "nag_hammadi"), .purple
            ),
          ]
        )
