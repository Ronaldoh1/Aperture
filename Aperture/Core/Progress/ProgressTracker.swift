...
struct JourneyProgressView: View {
            ...
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 24) {
                        ...
                        // Header with Sacred Geometry progress
                        headerSection
    ...
    private var headerSection: some View {
        VStack(spacing: 16) {
            // Sacred Geometry + Badge Count
            HStack(spacing: 16) {
                // Badge avatar - Flower of Life
                ZStack {
                    Circle()
                        .fill(
                            RadialGradient(
                    ...
                    FlowerOfLifeBubbleShape()
                        .stroke(Palette.accent.gold, lineWidth: 1.5)
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(badgeManager.totalBadgeCount) Badges Earned")
                        .font(.system(size: 14))
                    ...
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            ...
                            Capsule()
                                ...
                                .frame(width: geo.size.width * badgeManager.collectionProgress, height: 6)
                        }
                    }
                    .frame(height: 6)
                    .frame(maxWidth: 150)
                    
                    Text("\(badgeManager.uniqueShapesEarned().count)/\(SacredGeometryShape.allCases.count) unique shapes")
                        .font(.system(size: 10))
    ...
    private var statsGrid: some View {
        LazyVGrid(columns: [
            ...
            ProgressStatCard(title: "Courses Started", value: "\(tracker.stats.coursesStarted)", icon: "book.fill", color: Palette.accent.gold)
            ProgressStatCard(title: "Courses Completed", value: "\(tracker.stats.coursesCompleted)", icon: "checkmark.seal.fill", color: .green)
            ProgressStatCard(title: "Lessons Done", value: "\(tracker.stats.lessonsCompleted)", icon: "list.bullet", color: Palette.primary.cyan)
            ProgressStatCard(title: "Practices", value: "\(tracker.stats.practicesCompleted)", icon: "figure.mind.and.body", color: .purple)
            ProgressStatCard(title: "Cosmos Explored", value: "\(tracker.stats.cosmosExplored)", icon: "globe.americas.fill", color: Palette.primary.violet)
            ProgressStatCard(title: "Dragon Talks", value: "\(tracker.stats.dragonInteractions)", icon: "flame.fill", color: .red)
        }
    }
