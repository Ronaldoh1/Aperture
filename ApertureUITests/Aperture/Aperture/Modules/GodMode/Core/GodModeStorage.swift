...
class GodModeStorage: ObservableObject {
  ...
  private func saveRealityShifts() {
    save(realityShifts, key: keys.shifts)
  }

  private func saveSATSSessions() {
    save(satsSessions, key: keys.sats)
  }

  private func saveThreeSixNineSessions() {
    save(threeSixNineSessions, key: keys.threeSixNine)
  }

  private func saveRevisionEntries() {
    save(revisionEntries, key: keys.revision)
  }

  private func saveMentalDietEntries() {
    save(mentalDietEntries, key: keys.mentalDiet)
  }

  private func saveSolarSyncSessions() {
    save(solarSyncSessions, key: keys.solarSync)
  }

  private func saveBardoPractices() {
    save(bardoPractices, key: keys.bardo)
  }

  private func saveSynchronicities() {
    save(synchronicities, key: keys.synchronicities)
  }

  private func saveMonadCheckIns() {
    save(monadCheckIns, key: keys.checkIns)
  }

  private func saveVibrationShields() {
    save(vibrationShields, key: keys.shields)
  }
