...
enum AwakeningLevel: Int, CaseIterable, Codable {
    ...
    var dragonMessage: String {
        switch self {
        case .npc: return "You're still running their programs. That's okay - awareness is the first step."
        case .glitch: return "You felt it. Something doesn't add up. Trust that feeling."
        case .patternRecognition: return "Now you're seeing it everywhere. The patterns. The manipulation. The loops."
...
struct DailyCheckIn: Codable, Identifiable {
    ...
    init(id: UUID = UUID(), date: Date = Date(), unconsciousnessMoment: String, emotionalIntensity: Int, category: UnconsciousnessCategory, insight: String? = nil) {
        self.id = id
        self.date = date
...
struct CognitiveDissonanceEntry: Codable, Identifiable {
    ...
    init(id: UUID = UUID(), date: Date = Date(), trigger: String, whatIWasTaught: String, whatIObserved: String, emotionalIntensity: Int, resolved: Bool = false, resolution: String? = nil, domain: DissonanceDomain) {
        self.id = id
        self.date = date
...
struct RealityAuditEntry: Codable, Identifiable {
    ...
    init(id: UUID = UUID(), domain: AuditDomain, belief: String, status: BeliefStatus = .inherited, origin: String? = nil, evidence: String? = nil, notes: String? = nil, lastExamined: Date = Date()) {
        self.id = id
        self.domain = domain
        self.belief = belief
        self.status = status
...
struct QuestionEntry: Codable, Identifiable {
    ...
    init(id: UUID = UUID(), question: String, category: QuestionCategory, reflection: String? = nil, askedDate: Date = Date(), answeredDate: Date? = nil) {
        self.id = id
...
struct AwakeningProfile: Codable {
    ...
    var totalDissonanceEntries: Int { dissonanceLog.count }
    var resolvedDissonances: Int { dissonanceLog.filter { $0.resolved }.count }
    
    var examinedBeliefs: Int {
        auditEntries.filter { $0.status == .integrated || $0.status == .discarded }.count
    }
    var totalBeliefs: Int { auditEntries.count }
    
    var answeredQuestions: Int {
        questionHistory.filter { $0.isAnswered }.count
    }
    var totalQuestions: Int { questionHistory.count }
