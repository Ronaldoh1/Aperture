...
struct PatternRecognitionView: View {
    ...
    private var palantirSection: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 4) {
                ...
                Text("A solved border = Palantir loses billions.\nA permanent crisis = Palantir prints money.")
                    .font(.system(size: 10, weight: .medium))
    ...
    private func asymmetryRow(_ category: String, _ argentina: String, _ venezuela: String) -> some View {
        HStack(spacing: 4) {
            Text(argentina)
                .font(.system(size: 8, weight: .medium))
                .foregroundColor(.green)
    ...
    private var venezuelaSection: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 4) {
                ...
                Text("Venezuela has the LARGEST proven oil reserves\nin the world. Larger than Saudi Arabia.")
                    .font(.system(size: 9, weight: .medium))
                    .foregroundColor(.white.opacity(0.8))
                    .multilineTextAlignment(.center)
    ...
    private var signOfTheCrossSection: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 10) {
                crossStepRow("👆", "\"In the name of the Father...\"", "Touch FOREHEAD", "OBEY MORALITY", "Mind Control")
                crossStepRow("👇", "\"...and of the Son...\"", "Touch CHEST", "OBEY LAW", "Body Control")
                crossStepRow("👈👉", "\"...and of the Holy Spirit.\"", "Touch SHOULDERS", "OBEY MONEY", "Labor Control")
            ...
            Text("Every time you cross yourself, you activate\nthe control program and seal it with Egypt. 😂")
                .font(.system(size: 10, weight: .bold, design: .rounded))
                .foregroundColor(.orange)
                .multilineTextAlignment(.center)
        }
    ...
    private func crossStepRow(_ emoji: String, _ prayer: String, _ action: String, _ obey: String, _ control: String) -> some View {
        HStack(spacing: 10) {
            Text(emoji)
                .font(.system(size: 16))
                .frame(width: 30)
    ...
    private var breakingFreeSection: some View {
        VStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                breakFreeRow("2", "Stop worshipping the STRUCTURE", "The divine is WITHIN you, not institutions")
    ...
    private func triangleNode(icon: String, label: String, sublabel: String, color: Color) -> some View {
        VStack(spacing: 4) {
            ZStack {
                Circle()
                    .fill(color.opacity(0.2))
    ...
    private func administrationRow(_ event: SelectiveMoralityEvent) -> some View {
            ...
            HStack {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(event.years)
                        ...
                        .foregroundColor(event.administration.contains("EPSTEIN") ? .red.opacity(0.8) : .white.opacity(0.5))
                }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ForEach(event.whatHappened, id: \.self) { item in
                    HStack(alignment: .top, spacing: 6) {
                        ...
                        Text(item)
                            .font(.system(size: 11, weight: .medium, design: .rounded))
                            .foregroundColor(event.administration.contains("EPSTEIN") ? .red.opacity(0.9) : .white.opacity(0.7))
                    }
                }
            }
            .padding(.leading, 24)
        ...
        .background(
            RoundedRectangle(cornerRadius: 10, style: .continuous)
                .fill(event.administration.contains("EPSTEIN") ? Color.red.opacity(0.15) : Color.white.opacity(0.03))
                .overlay(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .stroke(event.administration.contains("EPSTEIN") ? Color.red.opacity(0.4) : Color.clear, lineWidth: 1)
                )
        )
    }
    ...
    private func twelveRow(_ emoji: String, _ text: String) -> some View {
        HStack(spacing: 10) {
            ...
            Text(text)
                .font(.system(size: 12, weight: text.contains("VATICAN") ? .bold : .medium, design: .rounded))
    ...
    private func christianMoralityRow(_ item: SelectiveChristianMorality) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(alignment: .top, spacing: 6) {
                Image(systemName: "megaphone.fill")
                    .font(.system(size: 10))
                    .foregroundColor(item.resources.contains("None") ? .red.opacity(0.7) : .green.opacity(0.7))
        ...
        .background(
            RoundedRectangle(cornerRadius: 10, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .stroke(item.resources.contains("None") ? Color.red.opacity(0.2) : Color.green.opacity(0.2), lineWidth: 1)
                )
        )
    }
    ...
    private var israelHamasSection: some View {
        VStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("Israeli officials admitted supporting Hamas to divide Palestinians and weaken the secular PLO.")
                    .font(.system(size: 11, weight: .medium, design: .rounded))
                    .foregroundColor(.white.opacity(0.9))
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\"Anyone who wants to thwart the establishment of a Palestinian state has to support bolstering Hamas.\"")
    ...
    private func definitionRow(_ emoji: String, _ term: String, _ definition: String, _ age: String) -> some View {
        HStack(spacing: 10) {
            Text(emoji)
                .font(.system(size: 18))
                .frame(width: 28)
    ...
    private var silencingMechanismSection: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 4) {
                ...
                Text("What are they so afraid of people seeing\nthat they need this much machinery to hide it?")
                    .font(.system(size: 12, weight: .bold, design: .rounded))
                    .foregroundColor(Palette.accent.gold)
                    .multilineTextAlignment(.center)
                    .lineSpacing(4)
    ...
    private func equationBox(_ title: String, _ subtitle: String, _ emoji: String, _ color: Color) -> some View {
        VStack(spacing: 4) {
            Text(emoji)
                .font(.system(size: 20))
    ...
    private func resourceRow(_ country: String, _ response: String, _ resources: String, _ hasResources: Bool) -> some View {
        HStack {
            Text(country)
                .font(.system(size: 12, weight: .bold, design: .rounded))
                .foregroundColor(.white)
    ...
    private func commandmentRow(_ number: String, _ commandment: String, _ meaning: String) -> some View {
        HStack(spacing: 8) {
            Text(number)
    ...
    private var animeTruthSection: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    otherAnimeRow("🐉", "DRAGON BALL Z", "7 Dragon Balls = 7 chakras, Super Saiyan = Kundalini")
                    otherAnimeRow("🍥", "NARUTO", "Chakra system, Third Eye (Sharingan), Inner demon integration")
    ...
    private var timelineContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("When the Epstein files dropped, the Vatican went SILENT. Then suddenly... they had something to say.")
                .font(.system(size: 13, weight: .medium, design: .rounded))
                .foregroundColor(.white.opacity(0.8))
            
            // Timeline events
            VStack(spacing: 0) {
                ForEach(Array(PatternRecognitionEvent.epsteinVaticanTimeline.enumerated()), id: \.element.id) { index, event in
                    timelineEvent(event, isLast: index == PatternRecognitionEvent.epsteinVaticanTimeline.count - 1)
                }
            }
    ...
    private var theQuestionContent: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    questionLine("If they said nothing when Columbus was committing genocide in God's name...")
