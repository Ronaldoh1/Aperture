                ...
                if selectedOptions[questions[currentQuestion].id] != nil {
                    Button {
                        ...
                        HStack {
                            ...
                            Image(systemName: currentQuestion < questions.count - 1 ? "chevron.right" : "sparkles")
                        }
            ...
            Text(option.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 12)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 12)
                                .stroke(isSelected ? Color(hex: "#9B59B6") : Color.white.opacity(0.1), lineWidth: isSelected ? 2 : 1)
                        )
                )
        }
    }
...
class PersonaStore: ObservableObject {
    ...
    private func loadPersona() {
        guard let raw = UserDefaults.standard.string(forKey: key),
              let persona = AwakeningPersona(rawValue: raw) else { return }
        currentPersona = persona
    }
        ...
        Button {
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .stroke(store.currentPersona?.color.opacity(0.3) ?? Color.white.opacity(0.1), lineWidth: 1)
                    )
            )
        }
