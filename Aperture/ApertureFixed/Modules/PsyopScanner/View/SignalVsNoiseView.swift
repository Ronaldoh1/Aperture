...
struct HeadlineAnalysis: Identifiable, Codable {
    ...
    init(id: String = UUID().uuidString, headline: String, source: String = "",
        ...
        self.analyzedAt = analyzedAt; self.noiseScore = noiseScore
        self.signalScore = signalScore; self.tactics = tactics
        self.verdict = verdict; self.whyItMatters = whyItMatters
        self.whatToWatchInstead = whatToWatchInstead; self.emotionalTarget = emotionalTarget
    }
}
...
struct ManipulationTactic: Identifiable, Codable {
    ...
    init(id: String = UUID().uuidString, name: String, description: String, severity: Int) {
        self.id = id; self.name = name; self.description = description; self.severity = severity
    }
}
...
struct SignalItem: Identifiable, Codable {
    ...
    init(id: String = UUID().uuidString, title: String, summary: String, category: SignalCategory,
        ...
        self.impactLevel = impactLevel; self.whyItMatters = whyItMatters
        self.whileYouWereDistracted = whileYouWereDistracted; self.dateRelevant = dateRelevant
    }
}
...
struct DistractionPattern: Identifiable, Codable {
    ...
    init(id: String = UUID().uuidString, distraction: String, distractionType: String,
         coverageDuration: String, whileYouWatched: [String], whoBenefited: String,
         lessonsLearned: String) {
        self.id = id; self.distraction = distraction; self.distractionType = distractionType
        self.coverageDuration = coverageDuration; self.whileYouWatched = whileYouWatched
        self.whoBenefited = whoBenefited; self.lessonsLearned = lessonsLearned
    }
}
...
final class NoiseDetectorEngine {
    ...
    func analyze(headline: String, source: String = "") -> HeadlineAnalysis {
        ...
        if emotionScore > 0 {
            detectedTactics.append(ManipulationTactic(
                name: "Emotional Bait",
                description: "Uses loaded language to trigger emotional reaction before rational thought. Detected: \(emotionalTargets.prefix(3).joined(separator: ", "))",
        ...
        if vagueScore > 0 {
            detectedTactics.append(ManipulationTactic(
                name: "Vague Sourcing",
                description: "Claims authority without naming specific sources. Ask: WHO said this? Where's the data?",
        ...
        if divisionScore > 0 {
            detectedTactics.append(ManipulationTactic(
                ...
                description: "Frames issue as us-vs-them to prevent nuanced thinking. Ask: who benefits from this division?",
        ...
        if scarcityScore > 0 {
            detectedTactics.append(ManipulationTactic(
                ...
                description: "Creates false scarcity or time pressure to bypass critical thinking. Real emergencies don't need clickbait.",
        ...
        if spectacleScore > 0 {
            detectedTactics.append(ManipulationTactic(
                ...
                description: "Celebrity/drama content designed to consume attention that could go toward things that affect your life.",
        ...
        if questionMarkCount > 0 {
            detectedTactics.append(ManipulationTactic(
                ...
                description: "Headlines ending in a question mark can usually be answered 'No.' This format lets outlets publish speculation as news.",
                severity: 2
            ))
        }
        ...
        if capsWords.count >= 2 {
            detectedTactics.append(ManipulationTactic(
                ...
                description: "All-caps words are visual shouting designed to trigger urgency. Actual breaking news doesn't need to yell.",
                severity: 3
            ))
        }
        
        // Calculate final scores
        let rawNoise = emotionScore + vagueScore + divisionScore + scarcityScore + spectacleScore + (questionMarkCount * 2) + (capsWords.count * 3)
        ...
        // Determine verdict
        let verdict: HeadlineVerdict
        if normalizedNoise >= 80 && detectedTactics.count >= 3 { verdict = .psyop }
        else if normalizedNoise >= 70 { verdict = .pureNoise }
        else if normalizedNoise >= 50 { verdict = .mostlyNoise }
        else if normalizedSignal >= 70 { verdict = .pureSignal }
        else if normalizedSignal >= 50 { verdict = .mostlySignal }
        else { verdict = .mixed }
        
        // Determine emotional target
        let primaryEmotion: String
        if divisionScore > emotionScore { primaryEmotion = "tribal identity / anger" }
        else if scarcityScore > emotionScore { primaryEmotion = "fear / anxiety" }
        else if spectacleScore > emotionScore { primaryEmotion = "curiosity / gossip" }
        else if emotionScore > 5 { primaryEmotion = "outrage / moral panic" }
        else { primaryEmotion = "general attention" }
        
        // Generate what to watch instead
        let watchInstead: String?
        if normalizedNoise > 50 {
            watchInstead = "Instead of reacting to this headline, ask: What policy changes, financial moves, or regulatory decisions happened today that got zero coverage?"
        } else { watchInstead = nil }
        ...
        return HeadlineAnalysis(
            ...
            whyItMatters: normalizedSignal > 50 ? "This contains actual information that could affect your life, finances, or rights." : nil,
...
class SignalNoiseManager: ObservableObject {
    ...
    private func persist() {
        ...
        let stats = SignalNoiseStats(totalScans: totalScans, noiseBlocked: noiseBlocked, signalFound: signalFound)
...
private struct SignalNoiseStats: Codable {
    let totalScans: Int; let noiseBlocked: Int; let signalFound: Int
}
...
struct SignalNoiseContentDatabase {
    ...
    static let patterns: [DistractionPattern] = [
        DistractionPattern(
            ...
            whoBenefited: "Every politician and corporation that needed public attention elsewhere. The slap consumed the entire national conversation for a week.",
            lessonsLearned: "Celebrity spectacle is the most reliable distraction tool because it triggers both tribal identity (picking sides) and emotional engagement (outrage/humor). No policy analysis can compete with two famous people in conflict."
        ),
        DistractionPattern(
            ...
            whoBenefited: "Both governments used the incident for political posturing. Defense contractors saw stock increases. Meanwhile, residents in Ohio were literally being poisoned with vinyl chloride.",
            lessonsLearned: "When something becomes 24/7 coverage and it involves a foreign 'threat,' check what domestic story just got buried. The pattern is reliable: foreign spectacle hides domestic failure."
        ),
        DistractionPattern(
            ...
            whoBenefited: "Social media platforms (engagement = ad revenue). Politicians (outrage = donations). Media companies (clicks = survival). The outrage economy is a $200B+ industry.",
            lessonsLearned: "The outrage cycle isn't a bug — it's the product. Every second you spend angry about a tweet is a second you're not reading a policy document, attending a city council meeting, or organizing in your community."
        ),
        DistractionPattern(
            ...
            whoBenefited: "Entertainment media (ad revenue), celebrities (relevance), and every institution that benefits from an uninformed public. Parasocial investment in celebrity lives is the modern bread and circuses.",
            lessonsLearned: "If you know more about a celebrity's relationship than your city council's voting record, the system is working exactly as designed. Your attention was stolen, not given."
        ),
        DistractionPattern(
            ...
            whoBenefited: "Both political parties benefit from culture wars because they keep voters emotionally engaged and tribally loyal without requiring either party to deliver material improvements. Corporations benefit because divided people don't organize against shared economic interests.",
            lessonsLearned: "Culture wars are cheaper than policy. It costs nothing to tweet about a social issue. It costs billions to fix infrastructure, healthcare, or education. Follow the money to find the motive."
        ),
    ]
    ...
    static let exampleSignals: [SignalItem] = [
        SignalItem(
            ...
            summary: "The FDA set action levels for lead in baby food at 10-20 ppb but made compliance voluntary, not mandatory. Health experts say this will only reduce exposure by 20-30%.",
            category: .food, impactLevel: 5,
            whyItMatters: "Your baby's food can legally contain lead and arsenic above levels other countries ban. Voluntary means no enforcement. No enforcement means no change.",
        ...
        SignalItem(
            ...
            summary: "Major social platforms changed algorithms to favor entertainment and engagement content over news. Independent journalism pages saw 30-60% reach drops.",
            category: .tech, impactLevel: 4,
            whyItMatters: "The platforms that control how you see the world quietly made it harder to see real journalism. This isn't censorship — it's algorithmic burial, which is worse because it's invisible.",
            whileYouWereDistracted: "You probably noticed your feed getting 'lighter' but didn't know it was by design."
        ),
        SignalItem(
            title: "Corporate Consolidation in Healthcare Accelerates",
            summary: "Hospital systems continue merging, reducing competition. In many regions, one or two systems control all healthcare options, enabling price increases.",
            category: .health, impactLevel: 5,
            whyItMatters: "When one company owns every hospital in your area, they set the price. Your 'choice' of healthcare is an illusion. This affects your bills directly.",
            whileYouWereDistracted: nil
        ),
        SignalItem(
            title: "Digital Privacy Bills Advance Quietly",
            summary: "Multiple bills affecting how companies collect, store, and sell your personal data are moving through committee with minimal public awareness.",
            category: .tech, impactLevel: 4,
            whyItMatters: "These bills determine whether your location data, health data, browsing history, and biometric data can be sold. This is about your digital autonomy.",
            whileYouWereDistracted: "The tech lobby spent millions ensuring this didn't trend on social media."
        ),
        SignalItem(
            title: "Water Infrastructure Aging Across Major Cities",
            summary: "EPA reports indicate that water infrastructure in dozens of major cities is past its designed lifespan, with lead pipes still servicing millions of homes.",
            category: .environment, impactLevel: 5,
            whyItMatters: "Flint wasn't an anomaly — it was a preview. The water pipes in your city may be older than your grandparents. This is a slow-motion crisis that doesn't trend because it's not dramatic.",
            whileYouWereDistracted: nil
        ),
        SignalItem(
            title: "Lobbying Expenditures Hit Record Highs",
            summary: "Corporate lobbying spending reached new records. Top spenders include pharmaceutical, tech, and defense companies.",
            category: .policy, impactLevel: 4,
            whyItMatters: "For every dollar a corporation spends on lobbying, they see $200+ in return through favorable legislation. This is legal corruption and it determines more about your life than any election.",
            whileYouWereDistracted: "Lobbying spending reports don't go viral. Celebrity feuds do. That's the system working."
        ),
    ]
...
struct SignalVsNoiseView: View {
    ...
    private var signalBackground: some View {
        LinearGradient(
            colors: [Color(red: 0.03, green: 0.05, blue: 0.10), Color(red: 0.05, green: 0.06, blue: 0.14), Color(red: 0.02, green: 0.03, blue: 0.08)],
            startPoint: .top, endPoint: .bottom
        ).ignoresSafeArea()
        .overlay(
            Circle().fill(Color.cyan.opacity(0.03)).frame(width: 300).blur(radius: 60).offset(x: 80, y: -80)
        )
    }
    ...
    private var sectionGrid: some View {
        VStack(spacing: 8) {
            HStack {
                Text("MODULES").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(.white.opacity(0.3))
                Spacer()
            }
    ...
    private var quickFilterPreview: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 6) {
                Image(systemName: "shield.lefthalf.filled").foregroundColor(Color(red: 1.0, green: 0.4, blue: 0.3))
            ...
            ForEach(Array(NoiseDetectorEngine.filterQuestions.prefix(4).enumerated()), id: \.offset) { i, q in
                HStack(alignment: .top, spacing: 10) {
                    ...
                    Text(q.question).font(.system(size: 12, weight: .medium)).foregroundColor(.white.opacity(0.7)).lineSpacing(2)
                }
            }
            
            Button { selectedSection = .noiseDetector } label: {
                Text("Try the Noise Detector →").font(.system(size: 12, weight: .bold))
                    .foregroundColor(.cyan)
            }
        }
        .padding(16)
        .background(RoundedRectangle(cornerRadius: 16).fill(Color(red: 1.0, green: 0.4, blue: 0.3).opacity(0.04))
            .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color(red: 1.0, green: 0.4, blue: 0.3).opacity(0.12), lineWidth: 1)))
    }
    ...
    private var patternPreview: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 6) {
                ...
                Text("PATTERN MAP PREVIEW").font(.system(size: 10, weight: .bold)).tracking(2).foregroundColor(.purple.opacity(0.6))
                Spacer()
            }
            let p = SignalNoiseContentDatabase.patterns[0]
            VStack(alignment: .leading, spacing: 6) {
                Text("DISTRACTION").font(.system(size: 8, weight: .bold)).tracking(1).foregroundColor(.red.opacity(0.5))
                ...
                Text("WHILE YOU WATCHED").font(.system(size: 8, weight: .bold)).tracking(1).foregroundColor(.green.opacity(0.5)).padding(.top, 4)
                Text(p.whileYouWatched[0]).font(.system(size: 11, weight: .medium)).foregroundColor(.green.opacity(0.7)).lineLimit(2)
                Text("+ \(p.whileYouWatched.count - 1) more...").font(.system(size: 10, weight: .medium)).foregroundColor(.white.opacity(0.3))
            }
            Button { selectedSection = .patternMap } label: {
                Text("See All Patterns →").font(.system(size: 12, weight: .bold)).foregroundColor(.purple)
            }
        }
        .padding(16)
        .background(RoundedRectangle(cornerRadius: 16).fill(Color.purple.opacity(0.04))
            .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color.purple.opacity(0.12), lineWidth: 1)))
    }
    ...
    private func sectionDestination(_ section: SignalSection) -> some View {
        ...
        case .history: NoiseScanHistoryView(manager: manager)
        }
    }
}
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            Text("Paste or type any headline:").font(.system(size: 12, weight: .bold)).foregroundColor(.white.opacity(0.5))
                        ...
                        // Filter Guide
                        Button { showFilterGuide.toggle() } label: {
                            HStack(spacing: 6) {
    ...
    private func analysisCard(_ a: HeadlineAnalysis) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(a.verdict.rawValue).font(.system(size: 16, weight: .bold)).foregroundColor(a.verdict.color)
                    Text("Targeting: \(a.emotionalTarget)").font(.system(size: 10, weight: .medium)).foregroundColor(.white.opacity(0.4))
                }
                Spacer()
            }
            ...
            if !a.tactics.isEmpty {
                VStack(alignment: .leading, spacing: 8) {
                    Text("MANIPULATION TACTICS DETECTED").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.orange.opacity(0.6))
                    ForEach(a.tactics) { tactic in
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(tactic.description).font(.system(size: 11, weight: .regular)).foregroundColor(.white.opacity(0.6)).lineSpacing(2)
                        }
            ...
            if let watch = a.whatToWatchInstead {
                VStack(alignment: .leading, spacing: 4) {
                    Text("REDIRECT YOUR ATTENTION").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.cyan.opacity(0.6))
                    Text(watch).font(.system(size: 12, weight: .medium)).foregroundColor(.cyan.opacity(0.8)).lineSpacing(3)
                }
            ...
            if let why = a.whyItMatters {
                VStack(alignment: .leading, spacing: 4) {
                    Text("WHY THIS MATTERS").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.green.opacity(0.6))
                    Text(why).font(.system(size: 12, weight: .medium)).foregroundColor(.green.opacity(0.8)).lineSpacing(3)
                }
        ...
        .background(RoundedRectangle(cornerRadius: 16).fill(Color.white.opacity(0.03))
            .overlay(RoundedRectangle(cornerRadius: 16).stroke(a.verdict.color.opacity(0.2), lineWidth: 1)))
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ForEach(Array(NoiseDetectorEngine.filterQuestions.enumerated()), id: \.offset) { _, q in
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(q.explanation).font(.system(size: 11, weight: .regular)).foregroundColor(.white.opacity(0.5)).lineSpacing(3)
                }
...
struct SignalTrackerView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 16) {
                        Text("What's actually happening while the noise machine runs.\nThese are the stories that affect your life.")
                            ...
                            HStack(spacing: 8) {
                                Button { selectedCategory = nil } label: {
                                    Text("All").font(.system(size: 11, weight: .medium))
                                        ...
                                        .background(Capsule().fill(selectedCategory == nil ? Color.cyan.opacity(0.3) : Color.white.opacity(0.04)))
                                }
                                ForEach(SignalCategory.allCases, id: \.self) { cat in
                                    Button { selectedCategory = cat } label: {
                                        ...
                                        .background(Capsule().fill(selectedCategory == cat ? cat.color.opacity(0.3) : Color.white.opacity(0.04)))
                                    }
                                }
                            }
                        }
                        
                        let filtered = selectedCategory == nil ? signals : signals.filter { $0.category == selectedCategory }
    ...
    private func signalCard(_ signal: SignalItem) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                Image(systemName: signal.category.icon).font(.system(size: 14)).foregroundColor(signal.category.color)
                Text(signal.category.rawValue).font(.system(size: 10, weight: .bold)).foregroundColor(signal.category.color)
                Spacer()
                HStack(spacing: 2) {
                    ForEach(0..<signal.impactLevel, id: \.self) { _ in
                        Image(systemName: "exclamationmark").font(.system(size: 8, weight: .bold)).foregroundColor(.red)
                    }
                }
            }
            
            Text(signal.title).font(.system(size: 14, weight: .bold)).foregroundColor(.white).lineSpacing(2)
            Text(signal.summary).font(.system(size: 12, weight: .regular)).foregroundColor(.white.opacity(0.6)).lineSpacing(3)
            
            VStack(alignment: .leading, spacing: 4) {
                Text("WHY THIS MATTERS TO YOU").font(.system(size: 8, weight: .bold)).tracking(1).foregroundColor(.green.opacity(0.5))
                Text(signal.whyItMatters).font(.system(size: 11, weight: .medium)).foregroundColor(.green.opacity(0.7)).lineSpacing(3)
            }
            .padding(10).background(RoundedRectangle(cornerRadius: 8).fill(Color.green.opacity(0.04)))
            
            if let distracted = signal.whileYouWereDistracted {
                VStack(alignment: .leading, spacing: 4) {
                    Text("WHY YOU DIDN'T HEAR ABOUT THIS").font(.system(size: 8, weight: .bold)).tracking(1).foregroundColor(.orange.opacity(0.5))
                    Text(distracted).font(.system(size: 11, weight: .medium)).foregroundColor(.orange.opacity(0.7)).lineSpacing(3)
                }
                .padding(10).background(RoundedRectangle(cornerRadius: 8).fill(Color.orange.opacity(0.04)))
            }
        ...
        .background(RoundedRectangle(cornerRadius: 14).fill(Color.white.opacity(0.03))
            .overlay(RoundedRectangle(cornerRadius: 14).stroke(signal.category.color.opacity(0.1), lineWidth: 1)))
    }
}
...
struct PatternMapView: View {
    ...
    private func patternCard(_ pattern: DistractionPattern) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            Button {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 8) {
                        ...
                        Text("DISTRACTION").font(.system(size: 8, weight: .bold)).tracking(1).foregroundColor(.red.opacity(0.6))
                        Spacer()
                        Text(pattern.distractionType).font(.system(size: 9, weight: .medium)).foregroundColor(.white.opacity(0.3))
                    ...
                    Text(pattern.coverageDuration).font(.system(size: 10, weight: .medium)).foregroundColor(.white.opacity(0.3))
                }
                .padding(14)
            }
            ...
            if isExpanded {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("WHILE YOU WATCHED").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.green.opacity(0.6))
                        ForEach(Array(pattern.whileYouWatched.enumerated()), id: \.offset) { _, item in
                            HStack(alignment: .top, spacing: 8) {
                                Image(systemName: "target").font(.system(size: 10)).foregroundColor(.green).padding(.top, 2)
                                Text(item).font(.system(size: 12, weight: .medium)).foregroundColor(.green.opacity(0.8)).lineSpacing(2)
                            }
                        }
                    }
                    
                    VStack(alignment: .leading, spacing: 4) {
                        Text("WHO BENEFITED").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.purple.opacity(0.6))
                        Text(pattern.whoBenefited).font(.system(size: 12, weight: .regular)).foregroundColor(.purple.opacity(0.7)).lineSpacing(3)
                    }
                    .padding(10).background(RoundedRectangle(cornerRadius: 8).fill(Color.purple.opacity(0.04)))
                    
                    VStack(alignment: .leading, spacing: 4) {
                        Text("LESSON").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(.cyan.opacity(0.6))
                        Text(pattern.lessonsLearned).font(.system(size: 12, weight: .medium, design: .rounded)).foregroundColor(.cyan.opacity(0.7)).lineSpacing(3)
                    }
                    .padding(10).background(RoundedRectangle(cornerRadius: 8).fill(Color.cyan.opacity(0.04)))
                }
        ...
        .background(RoundedRectangle(cornerRadius: 14).fill(Color.white.opacity(0.03))
            .overlay(RoundedRectangle(cornerRadius: 14).stroke(isExpanded ? Color.red.opacity(0.15) : Color.white.opacity(0.05), lineWidth: 1)))
    }
}
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 14) {
                        ForEach(Array(SignalNoiseContentDatabase.mediaLessons.enumerated()), id: \.offset) { _, lesson in
                            VStack(alignment: .leading, spacing: 10) {
                                HStack(spacing: 8) {
                                ...
                                Text(lesson.content).font(.system(size: 13, weight: .regular)).foregroundColor(.white.opacity(0.7)).lineSpacing(5)
                            }
                            .padding(16)
                            .background(RoundedRectangle(cornerRadius: 16).fill(Color(red: 0.4, green: 0.7, blue: 1.0).opacity(0.03))
                                .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color(red: 0.4, green: 0.7, blue: 1.0).opacity(0.1), lineWidth: 1)))
                        }
                        Spacer(minLength: 40)
                    }
                    .padding(20)
        ...
        NavigationStack {
            ZStack {
                ...
                if manager.savedSignals.isEmpty {
                    VStack(spacing: 12) {
                        Image(systemName: "line.3.horizontal.decrease.circle").font(.system(size: 40)).foregroundColor(.white.opacity(0.1))
                        Text("No saved signals yet").font(.system(size: 16, weight: .bold)).foregroundColor(.white.opacity(0.3))
                        Text("Save stories from the Signal Tracker\nto build your custom feed.")
                            .font(.system(size: 12)).foregroundColor(.white.opacity(0.2)).multilineTextAlignment(.center)
                    }
                } else {
                    ScrollView(showsIndicators: false) {
                        VStack(spacing: 10) {
                            ForEach(manager.savedSignals) { signal in
                                HStack(spacing: 10) {
                                    Image(systemName: signal.category.icon).foregroundColor(signal.category.color).frame(width: 24)
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text(signal.title).font(.system(size: 13, weight: .bold)).foregroundColor(.white).lineLimit(2)
                                        Text(signal.category.rawValue).font(.system(size: 10)).foregroundColor(signal.category.color)
                                    }
                                    Spacer()
                                }
                                .padding(12).background(RoundedRectangle(cornerRadius: 10).fill(Color.white.opacity(0.03)))
...
struct NoiseScanHistoryView: View {
    @Environment(\.dismiss) var dismiss
    @ObservedObject var manager: SignalNoiseManager
        ...
        NavigationStack {
            ZStack {
                ...
                if manager.scanHistory.isEmpty {
                    VStack(spacing: 12) {
                        Image(systemName: "clock.arrow.circlepath").font(.system(size: 40)).foregroundColor(.white.opacity(0.1))
                        Text("No scans yet").font(.system(size: 16, weight: .bold)).foregroundColor(.white.opacity(0.3))
                        Text("Use the Noise Detector to analyze headlines.").font(.system(size: 12)).foregroundColor(.white.opacity(0.2))
                    }
                } else {
                    ScrollView(showsIndicators: false) {
                        VStack(spacing: 10) {
                            ForEach(manager.scanHistory) { scan in
                                HStack(spacing: 10) {
                                    Image(systemName: scan.verdict.icon).foregroundColor(scan.verdict.color).frame(width: 24)
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text(scan.headline).font(.system(size: 12, weight: .medium)).foregroundColor(.white).lineLimit(2)
                                        HStack(spacing: 8) {
                                            Text(scan.verdict.rawValue).font(.system(size: 10, weight: .bold)).foregroundColor(scan.verdict.color)
                                            Text("Noise: \(scan.noiseScore)%").font(.system(size: 9)).foregroundColor(.red.opacity(0.6))
                                            Text("Signal: \(scan.signalScore)%").font(.system(size: 9)).foregroundColor(.green.opacity(0.6))
                                        }
                                    }
                                    Spacer()
                                }
