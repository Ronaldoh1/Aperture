...
enum ChronokeeperPersona {
  ...
  private static var courseListPrompt: String {
    let courses: [(String, String)] = [
      ...
      (
        "Antarctica",
        "Hidden history beneath the ice. Ancient civilizations and the truth they buried."
      ),
      (
        "Anti-Christ Decoded",
        "What 'antichrist' really means — not a person, but a system that replaces inner knowing with external authority."
      ),
      ...
      (
        ...
        "Yeshua's actual teaching: the divine spark within. The kingdom of God is inside you."
      ),
      (
        "Cross Solar Portal",
        "The cross as solar symbol across every civilization. Not execution — illumination."
      ),
      (
        "Crucifixion as Initiation",
        "Mystery school interpretation. Death of ego, resurrection of consciousness."
      ),
      ("Elohim", "The plural gods of Genesis. What 'Let US make man in OUR image' really means."),
      (
        "Gospel of Judas",
        "The most controversial gnostic text. Judas as the one who truly understood."
      ),
      (
        "Guilt and Power",
        "How guilt was weaponized by institutions to control populations for millennia."
      ),
      (
        "Lemuria & Atlantis",
        "The lost civilizations. Evidence, theories, and why mainstream rejects them."
      ),
      (
        "Nephilim & Watchers",
        "The Book of Enoch and the beings who came down. Why it was removed from the Bible."
      ),
      ("144,000", "Revelation's most misunderstood number. Not literal — it's a frequency code."),
      (
        "Tarot Arcana",
        "The hidden wisdom in the Major Arcana. A map of consciousness, not fortune telling."
      ),
      ("Tonantzin", "The Aztec mother goddess Rome buried under the Virgin of Guadalupe."),
      (
        "Twelve Plus One",
        "The universal pattern: 12 disciples, 12 zodiac signs, 12 months. The 13th is you."
      ),
      (
        "Urantia & Gaia",
        "Alternative cosmologies. The Urantia Book and Gaia hypothesis as frameworks for understanding."
      ),
    ]
  ...
  struct UserContext {
    ...
    var prompt: String {
      ...
      if let current = currentCourseId {
        lines.append(
          "- Currently on: \(current), Module \(currentModuleIndex + 1), Lesson \(currentLessonIndex + 1)"
        )
      }
      ...
      if !recentScans.isEmpty {
        lines.append(
          "- Recent food scans found these chemicals: \(recentScans.joined(separator: ", "))")
      }

      return lines.isEmpty
        ? "- No progress data yet. Treat as a curious newcomer." : lines.joined(separator: "\n")
    }
  }
  ...
  static let suggestedStarters: [ChronokeeperAction] = [
    ChronokeeperAction(
      label: "What should I learn first?", icon: "sparkles",
      type: .sendMessage("What course should I start with? I'm new to all this.")),
    ChronokeeperAction(
      label: "Tell me about the Kill List", icon: "exclamationmark.triangle.fill",
      type: .sendMessage("What chemicals are on the Kill List and why should I avoid them?")),
    ChronokeeperAction(
      label: "Who was Yeshua really?", icon: "sun.max.fill",
      type: .sendMessage("Tell me about the real Yeshua — before they changed his name.")),
    ChronokeeperAction(
      label: "Pick up where I left off", icon: "arrow.right.circle.fill", type: .continueJourney),
    ChronokeeperAction(
      label: "Read me something", icon: "speaker.wave.2.fill",
      type: .sendMessage(
        "Read me the first lesson of whichever course you think I should start with.")),
    ChronokeeperAction(
      label: "What's the Cosmos module?", icon: "globe.americas.fill",
      type: .sendMessage("Walk me through what the Cosmos module teaches. What are the spheres?")),
  ]
