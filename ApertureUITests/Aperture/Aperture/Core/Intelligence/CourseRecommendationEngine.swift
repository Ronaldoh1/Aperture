...
class CourseRecommendationEngine: ObservableObject {
    ...
    @Published private(set) var strategyRecommendations: [RecommendationStrategy: [CourseRecommendation]] = [:]
    ...
    func getRecommendations(for strategy: RecommendationStrategy, limit: Int = 5) -> [CourseRecommendation] {
    ...
    private func generateRecommendations() async {
        ...
        await generateStrategyRecommendations(allCourses: allCourses, profile: profile, progress: progress)
    }
    ...
    private func calculateScore(
        ...
        if let courseProgress = progress.first(where: { $0.courseId == course.id }),
           !courseProgress.isCompleted {
            ...
            reasons.insert(RecommendationReason(
    ...
    private func generateStrategyRecommendations(
        ...
        let exploredCategories = Set(progress.compactMap { prog in
            allCourses.first { $0.id == prog.courseId }?.category.rawValue
        })
        let unexplored = allCourses
        ...
        if let topCategory = profile.preferredCategory {
            let deepDive = allCourses
                .filter { $0.category.rawValue == topCategory }
                .filter { course in !progress.contains { prog in prog.courseId == course.id && prog.isCompleted } }
                .prefix(5)
                .map { course in
                    CourseRecommendation(
                        id: course.id,
...
struct RecommendationCard: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(recommendation.title). \(recommendation.subtitle). \(Int(recommendation.score * 100)) percent match.")
