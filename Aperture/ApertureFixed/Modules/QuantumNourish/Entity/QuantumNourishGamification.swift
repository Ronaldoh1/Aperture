...
enum CosmicBadge: String, CaseIterable, Identifiable, Codable {
    ...
    var tier: BadgeTier {
        ...
        case .twentyOneDayStreak, .pinealProtector, .vibeArchitect, .mushroomShaman, .cosmicChef: return .gold
...
class FrequencyChallengeEngine {
    static let challenges: [FrequencyChallenge] = [
        FrequencyChallenge(
            ...
            duration: 7,
            targetMhz: 50,
            rules: ["No foods from Destructive or Acidic tiers", "Minimum 3 raw meals per week", "Spring water or filtered only", "Log every meal"],
        ...
        FrequencyChallenge(
            ...
            duration: 14,
            targetMhz: 60,
            rules: ["Zero fluoride exposure (water, toothpaste)", "Daily turmeric + black pepper", "Chlorella or spirulina daily", "No alcohol or processed foods", "Sun gaze at sunrise/sunset (safely)"],
        ...
        FrequencyChallenge(
            ...
            duration: 3,
            targetMhz: 75,
            rules: ["Nothing cooked above 118°F / 48°C", "Fresh juices within 15 min of pressing", "Living sprouts with every meal", "Hydrate with spring water only"],
            reward: .rawWarrior,
            icon: "bolt.fill",
        ...
        FrequencyChallenge(
            ...
            duration: 5,
            targetMhz: 55,
            rules: ["Lion's Mane daily (neurogenesis)", "Chaga tea in the morning", "Reishi before bed", "Turkey Tail with meals", "Journal cognitive changes"],
        ...
        FrequencyChallenge(
            ...
            duration: 7,
            targetMhz: 72,
            rules: ["Follow each day's specific protocol", "Log all meals in Vibe Tracker", "No cheating — consciousness doesn't negotiate", "Complete daily reflection"],
            reward: .cosmicChef,
...
struct CosmicBadgeGalleryView: View {
        ...
        NavigationStack {
            ZStack {
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 20) {
                        ...
                        VStack(spacing: 10) {
                            Text("YOUR LEVEL")
                                .font(.system(size: 10, weight: .bold)).tracking(3).foregroundColor(Palette.text.muted)
                            Text(progress.level)
                            ...
                            GeometryReader { geo in
                                ZStack(alignment: .leading) {
                                    RoundedRectangle(cornerRadius: 6)
                                        ...
                                        .frame(height: 8)
                                    RoundedRectangle(cornerRadius: 6)
                                        .fill(LinearGradient(colors: [.green, Color(hex: "#FFD700"), Color(hex: "#9933FF")], startPoint: .leading, endPoint: .trailing))
                        ...
                        HStack(spacing: 12) {
                            ...
                            statBox(value: "\(progress.totalScans)", label: "Scans", icon: "barcode.viewfinder", color: Color(hex: "#E74C3C"))
                            statBox(value: "\(progress.totalMealsLogged)", label: "Meals", icon: "fork.knife", color: Color(hex: "#2ECC71"))
                        }
            ...
            .toolbar {
                ToolbarItem(placement: .principal) {
                    Text("COSMIC BADGES").font(.system(size: 12, weight: .bold, design: .rounded)).tracking(2).foregroundColor(Palette.accent.gold)
                }
    ...
    private func statBox(value: String, label: String, icon: String, color: Color) -> some View {
        VStack(spacing: 6) {
            ...
            Text(value).font(.system(size: 20, weight: .bold, design: .rounded)).foregroundColor(Palette.text.primary)
                    ...
                    VStack(spacing: 16) {
                        VStack(spacing: 8) {
                            Text("FREQUENCY CHALLENGES")
                                .font(.system(size: 10, weight: .bold)).tracking(4).foregroundColor(Color(hex: "#FF6B35"))
    ...
    private func challengeCard(_ challenge: FrequencyChallenge) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(challenge.title.uppercased()).font(.system(size: 12, weight: .bold, design: .rounded)).foregroundColor(challenge.color)
                    Text("\(challenge.duration) days • Target \(challenge.targetMhz)+ MHz").font(.system(size: 10, weight: .medium)).foregroundColor(Palette.text.muted)
                }
                Spacer()
            ...
            Text(challenge.description).font(.system(size: 13, weight: .medium)).foregroundColor(Palette.text.secondary)
            
            VStack(alignment: .leading, spacing: 6) {
                Text("RULES").font(.system(size: 9, weight: .bold)).tracking(1).foregroundColor(challenge.color)
                ForEach(challenge.rules, id: \.self) { rule in
                    HStack(spacing: 6) {
                        Image(systemName: "checkmark.circle").font(.system(size: 10)).foregroundColor(challenge.color.opacity(0.6))
                        Text(rule).font(.system(size: 12, weight: .medium)).foregroundColor(Palette.text.secondary)
                    }
                }
            }
            ...
            Button {
                ...
                Text(progress.activeChallenge == challenge.id ? "DAY \(progress.challengeDay) OF \(challenge.duration)" : "START CHALLENGE")
        ...
        .background(
            RoundedRectangle(cornerRadius: 14)
                ...
                .overlay(RoundedRectangle(cornerRadius: 14).stroke(challenge.color.opacity(0.15), lineWidth: 1))
        )
    }
}
