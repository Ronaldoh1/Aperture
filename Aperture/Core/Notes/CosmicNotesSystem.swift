...
struct CosmicNote: Identifiable, Codable {
    ...
    init(id: String = UUID().uuidString, title: String = "", body: String = "", type: CosmicNoteType = .note,
        ...
        self.id = id; self.title = title; self.body = body; self.type = type
        self.labels = labels; self.color = color; self.priority = priority
        self.checklistItems = checklistItems; self.isPinned = isPinned
        self.createdAt = createdAt; self.updatedAt = updatedAt
        self.sourceModule = sourceModule; self.voiceMemoPath = voiceMemoPath
    }
}
...
struct ChecklistItem: Identifiable, Codable {
    ...
    init(id: String = UUID().uuidString, text: String, isCompleted: Bool = false, createdAt: Date = Date()) {
        self.id = id; self.text = text; self.isCompleted = isCompleted; self.createdAt = createdAt
    }
}
...
class CosmicNotesManager: ObservableObject {
    ...
    func quickCapture(title: String, body: String = "", type: CosmicNoteType = .note,
    ...
    var totalTasks: Int { notes.filter { $0.type == .task }.count }
    var completedTasks: Int {
        notes.filter { $0.type == .task && $0.checklistItems.allSatisfy { $0.isCompleted } && !$0.checklistItems.isEmpty }.count
    }
        ...
        ScrollView(showsIndicators: false) {
            LazyVStack(spacing: 10) {
                ...
                ForEach(manager.filteredNotes) { note in
                    noteCard(note)
                        .contextMenu {
                            Button { manager.togglePin(note) } label: {
                                Label(note.isPinned ? "Unpin" : "Pin", systemImage: note.isPinned ? "pin.slash" : "pin")
                            }
                            Button { editingNote = note } label: {
                                Label("Edit", systemImage: "pencil")
                            }
                            Button(role: .destructive) { manager.delete(note) } label: {
        ...
        Button { editingNote = note } label: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 14)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 14)
                            .stroke(note.isPinned ? Color.yellow.opacity(0.2) : note.color.accentColor.opacity(0.1), lineWidth: 1)
                    )
            )
        }
    }
...
struct CosmicNoteEditorView: View {
    ...
    var body: some View {
        NavigationStack {
                        ...
                        HStack(spacing: 12) {
                            Button { showColorPicker.toggle() } label: {
                                HStack(spacing: 6) {
                                    ...
                                    Text("Color").font(.system(size: 12, weight: .medium)).foregroundColor(.white.opacity(0.5))
                                }
            ...
            .onAppear {
                if let n = note {
                    title = n.title; noteBody = n.body; noteType = n.type
                    selectedLabels = Set(n.labels); noteColor = n.color
                    priority = n.priority; checklistItems = n.checklistItems
                }
            }
        }
    }
        ...
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
            ForEach(NoteLabel.allCases, id: \.self) { label in
                Button {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 8)
                            .fill(selectedLabels.contains(label) ? label.color.opacity(0.25) : Color.white.opacity(0.04))
                    )
                }
            }
        }
    ...
    private func saveNote() {
        if isEditing, var existing = note {
            ...
            let newNote = CosmicNote(
                title: title, body: noteBody, type: noteType,
                        ...
                        HStack(spacing: 8) {
                            ForEach(NoteLabel.allCases, id: \.self) { label in
                                Button {
                                    ...
                                    .background(Capsule().fill(selectedLabel == label ? label.color.opacity(0.3) : Color.white.opacity(0.04)))
                                }
                            }
                        }
                    }
