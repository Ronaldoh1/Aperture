...
struct ReferredFriend: Identifiable, Codable {
  ...
  init(
    id: String = UUID().uuidString, displayName: String, joinedAt: Date = Date(),
    coursesCompleted: Int = 0, isActive: Bool = true
  ) {
    self.id = id
...
struct SparkAFriendHubView: View {
  ...
  private var statsOverview: some View {
    HStack(spacing: 0) {
      statItem(
        value: "\(sparkManager.stats.totalShares)", label: "Invites Sent", icon: "paperplane")
      Divider().frame(height: 50)
      statItem(
        value: "\(sparkManager.stats.friendsJoined)", label: "Friends Joined",
        icon: "person.badge.plus")
      Divider().frame(height: 50)
      statItem(
        value: "+\(sparkManager.stats.xpEarned)", label: "XP Earned", icon: "star.fill",
        valueColor: .yellow)
    }
    .padding()
    .background(
  ...
  private func statItem(value: String, label: String, icon: String, valueColor: Color = .white)
    -> some View
  {
    VStack(spacing: 8) {
  ...
  private var howItWorksSection: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      VStack(spacing: 12) {
        howItWorksRow(
          number: "1", title: "Share Your Code", description: "Send your spark code to friends")
        howItWorksRow(
          number: "2", title: "They Join", description: "Friends sign up with your code")
        howItWorksRow(
          number: "3", title: "Earn Rewards", description: "Get 100 XP per friend who joins")
      }
    }
    .padding()
    .background(
