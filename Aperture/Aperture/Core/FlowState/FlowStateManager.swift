...
struct FlowSessionView: View {
    ...
    NavigationStack {
      ...
      .sheet(isPresented: $showEndSession) {
        EndSessionSheet(
          ...
          onComplete: {
            manager.endSession(
              notes: endSessionNotes.isEmpty ? nil : endSessionNotes, mood: selectedMood)
  ...
  private var flowBackground: some View {
    ZStack {
      ...
      Circle()
        ...
        .animation(
          .easeInOut(duration: 2).repeatForever(autoreverses: true), value: manager.isActive)
    }
  }
  ...
  private var durationPreview: some View {
    VStack(spacing: 8) {
      ...
      Text(
        "Total: \(manager.prepareDuration + manager.enteringDuration + manager.deepWorkDuration + manager.restDuration) minutes"
      )
      .font(.system(size: 12))
      .foregroundColor(.gray)
    }
    ...
    NavigationStack {
      Form {
        Section(header: Text("Phase Durations (minutes)")) {
          Stepper(
            "Prepare: \(manager.prepareDuration)m", value: $manager.prepareDuration, in: 1...10)
          Stepper(
            "Entering: \(manager.enteringDuration)m", value: $manager.enteringDuration, in: 3...15)
          Stepper(
            "Deep Work: \(manager.deepWorkDuration)m", value: $manager.deepWorkDuration, in: 15...90
          )
