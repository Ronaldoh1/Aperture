...
struct BudgetModeView: View {
    ...
    NavigationStack {
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 20) {
            // Header
            VStack(spacing: 10) {
              Image(systemName: "dollarsign.circle.fill").font(.system(size: 44)).foregroundColor(
                Color(hex: "#2ECC71"))
              Text("BUDGET MODE").font(.system(size: 10, weight: .bold)).tracking(3)
                .foregroundColor(Color(hex: "#2ECC71"))
              Text("Upgrade What Matters First").font(
                .system(size: 20, weight: .bold, design: .rounded)
              ).foregroundColor(Palette.text.primary)
              Text(
                "You don't need to buy all organic tomorrow. Here's what to switch FIRST for maximum impact on any budget."
              )
            ...
            VStack(alignment: .leading, spacing: 10) {
              Text("YOUR WEEKLY GROCERY BUDGET").font(.system(size: 10, weight: .bold)).tracking(
                1.5
              ...
              HStack(spacing: 8) {
                ForEach(BudgetTier.allCases, id: \.self) { tier in
                  Button {
                    ...
                    VStack(spacing: 4) {
                      Text(tier.label).font(.system(size: 11, weight: .bold)).foregroundColor(
                        budget == tier ? .black : Palette.text.muted)
                      Text(tier.range).font(.system(size: 9)).foregroundColor(
                        budget == tier ? .black.opacity(0.7) : Palette.text.muted)
                    }
                    .frame(maxWidth: .infinity).padding(.vertical, 12)
                    .background(
                      RoundedRectangle(cornerRadius: 10).fill(
                        budget == tier ? Color(hex: "#2ECC71") : Color.white.opacity(0.05)))
            ...
            VStack(alignment: .leading, spacing: 14) {
              HStack(spacing: 6) {
                ...
                Text("SWITCH THESE FIRST").font(.system(size: 10, weight: .bold)).tracking(1.5)
                  .foregroundColor(Color(hex: "#E74C3C"))
              }
              Text(
                "Highest pesticide load + biggest health impact. Switch these before anything else."
              )
              ...
              RoundedRectangle(cornerRadius: 16).fill(Color(hex: "#E74C3C").opacity(0.04))
                .overlay(
                  RoundedRectangle(cornerRadius: 16).stroke(
                    Color(hex: "#E74C3C").opacity(0.15), lineWidth: 1)))

            // Clean Fifteen
            VStack(alignment: .leading, spacing: 14) {
              HStack(spacing: 6) {
                Image(systemName: "checkmark.circle.fill").foregroundColor(Color(hex: "#2ECC71"))
                Text("SAVE MONEY HERE — CONVENTIONAL IS FINE").font(
                  .system(size: 10, weight: .bold)
                ).tracking(1).foregroundColor(Color(hex: "#2ECC71"))
              }
              Text(
                "Low pesticide residue even when non-organic. Buy conventional and save your budget for the Dirty Dozen."
              )
              .font(.system(size: 12, weight: .medium)).foregroundColor(Palette.text.muted)

              ForEach(BudgetSwapDatabase.cleanFifteen, id: \.self) { item in
                HStack(spacing: 10) {
                  Image(systemName: "checkmark.circle").font(.system(size: 14)).foregroundColor(
                    Color(hex: "#2ECC71"))
                  Text(item).font(.system(size: 14, weight: .medium)).foregroundColor(
                    Palette.text.primary)
                  Spacer()
                  Text("SAVE $").font(.system(size: 10, weight: .bold)).foregroundColor(
                    Color(hex: "#2ECC71"))
                }
              }
            }.padding(16).background(
              RoundedRectangle(cornerRadius: 16).fill(Color(hex: "#2ECC71").opacity(0.04))
                .overlay(
                  RoundedRectangle(cornerRadius: 16).stroke(
                    Color(hex: "#2ECC71").opacity(0.15), lineWidth: 1)))

            // Budget tips
            VStack(alignment: .leading, spacing: 12) {
              Text("💡 BUDGET HACKS").font(.system(size: 10, weight: .bold)).tracking(1.5)
                .foregroundColor(Palette.accent.gold)
              ForEach(budget.tips, id: \.self) { tip in
                HStack(alignment: .top, spacing: 10) {
                  Image(systemName: "lightbulb.fill").font(.system(size: 12)).foregroundColor(
                    Palette.accent.gold)
                  Text(tip).font(.system(size: 13, weight: .medium)).foregroundColor(
                    Palette.text.secondary
                  ).lineSpacing(3)
                }
              }
            }.padding(16).background(
              RoundedRectangle(cornerRadius: 16).fill(Palette.accent.gold.opacity(0.04))
                .overlay(
                  RoundedRectangle(cornerRadius: 16).stroke(
                    Palette.accent.gold.opacity(0.15), lineWidth: 1)))
      ...
      .toolbar {
        ...
        ToolbarItem(placement: .principal) {
          Text("BUDGET MODE").font(.system(size: 14, weight: .bold, design: .rounded)).tracking(2)
            .foregroundColor(Color(hex: "#2ECC71"))
        }
      }
    }
  ...
  private func swapRow(_ swap: BudgetSwap, priority _: SwapPriority) -> some View {
    HStack(spacing: 12) {
      Text(swap.icon).font(.system(size: 20))
      VStack(alignment: .leading, spacing: 2) {
        Text(swap.item).font(.system(size: 14, weight: .bold, design: .rounded)).foregroundColor(
          Palette.text.primary)
        Text(swap.reason).font(.system(size: 11, weight: .medium)).foregroundColor(
          Palette.text.muted
        ).lineLimit(2)
      }
      Spacer()
      if let savings = budget.savings(for: swap) {
        Text(savings).font(.system(size: 10, weight: .bold, design: .monospaced)).foregroundColor(
          Color(hex: "#2ECC71"))
      }
...
enum BudgetTier: String, CaseIterable {
  ...
  func savings(for _: BudgetSwap) -> String? {
    switch self {
...
enum BudgetSwapDatabase {
  static let dirtyDozen: [BudgetSwap] = [
    BudgetSwap(
      item: "Strawberries", icon: "🍓",
      reason: "Highest pesticide residue of any produce — up to 22 different pesticides found"),
    BudgetSwap(
      item: "Spinach", icon: "🥬",
      reason: "Contains permethrin, a neurotoxic insecticide. Wash won't remove it."),
    BudgetSwap(
      item: "Kale / Collards", icon: "🥗",
      reason: "DCPA (Dacthal) detected — an herbicide classified as possible carcinogen"),
    BudgetSwap(
      item: "Nectarines", icon: "🍑",
      reason: "94% of samples had 2+ pesticides. Thin skin = easy absorption."),
    BudgetSwap(
      item: "Apples", icon: "🍎",
      reason: "Diphenylamine residue — banned in the EU since 2012. Still on US apples."),
    BudgetSwap(
      item: "Grapes", icon: "🍇",
      reason: "Up to 15 pesticides per sample. Thin skin, no peeling option."),
    BudgetSwap(
      item: "Bell Peppers", icon: "🫑",
      reason: "87 different pesticides detected across samples. High variety = high concern."),
    BudgetSwap(
      item: "Cherries", icon: "🍒", reason: "High pesticide load. Go organic or frozen organic."),
    BudgetSwap(
      item: "Peaches", icon: "🍑",
      reason: "Fuzzy skin traps pesticides. Peeling helps but doesn't eliminate."),
    BudgetSwap(item: "Pears", icon: "🍐", reason: "Multiple fungicide residues detected."),
    BudgetSwap(
      item: "Celery", icon: "🌿",
      reason: "No protective outer peel. Absorbs everything from the soil."),
    BudgetSwap(
      item: "Tomatoes", icon: "🍅",
      reason: "Thin skin absorbs pesticides. Canned organic is a budget alternative."),
  ]
...
struct ShoppingListView: View {
    ...
    NavigationStack {
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 20) {
            VStack(spacing: 10) {
              Image(systemName: "cart.fill").font(.system(size: 44)).foregroundColor(
                Color(hex: "#2ECC71"))
              Text("QUANTUM SHOPPING LIST").font(.system(size: 10, weight: .bold)).tracking(3)
                .foregroundColor(Color(hex: "#2ECC71"))
              Text("Your Conscious Grocery Run").font(
                .system(size: 20, weight: .bold, design: .rounded)
              ).foregroundColor(Palette.text.primary)
            }.padding(.top, 12)
            ...
            HStack(spacing: 10) {
              ...
              Button {
                ...
                Image(systemName: "plus.circle.fill").font(.system(size: 32)).foregroundColor(
                  Color(hex: "#2ECC71"))
              }.buttonStyle(PlainButtonStyle())
            }
              ...
              if !catItems.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                  HStack {
                    ...
                    Text(cat.rawValue.uppercased()).font(.system(size: 10, weight: .bold)).tracking(
                      1.5
                    ).foregroundColor(cat.color)
                    Spacer()
                    Text("\(catItems.count)").font(
                  ...
                  RoundedRectangle(cornerRadius: 14).fill(Color.white.opacity(0.03))
                    .overlay(
                      RoundedRectangle(cornerRadius: 14).stroke(
                        cat.color.opacity(0.1), lineWidth: 1)))
              }
            }
      ...
      .toolbar {
        ...
        ToolbarItem(placement: .principal) {
          Text("SHOPPING LIST").font(.system(size: 14, weight: .bold, design: .rounded)).tracking(2)
            .foregroundColor(Color(hex: "#2ECC71"))
        }
      }
    }
    ...
    HStack(spacing: 12) {
      Button {
        ...
        Image(systemName: item.checked ? "checkmark.circle.fill" : "circle")
          .font(.system(size: 20)).foregroundColor(
            item.checked ? Color(hex: "#2ECC71") : Palette.text.muted)
      }.buttonStyle(PlainButtonStyle())
  ...
  private func addCustomItem() {
    ...
    list.add(
      ShoppingItem(
        name: newItemText.trimmingCharacters(in: .whitespaces), quantity: nil, category: .other,
        isOrganic: false))
    newItemText = ""
    HapticManager.shared.medium()
  }
...
class ShoppingListManager: ObservableObject {
  ...
  func addFromMealPlan(_ foods: [(name: String, qty: String?, category: ShoppingCategory)]) {
    for food in foods {
      ...
      let shouldBeOrganic = BudgetSwapDatabase.dirtyDozen.contains(where: {
        $0.item.lowercased() == food.name.lowercased()
      })
      let newItem = ShoppingItem(
        name: food.name, quantity: food.qty, category: food.category, isOrganic: shouldBeOrganic)
    ...
    NavigationStack {
      ZStack {
        ...
        ScrollView(showsIndicators: false) {
          VStack(spacing: 20) {
            VStack(spacing: 10) {
              Image(systemName: "map.fill").font(.system(size: 44)).foregroundColor(
                Color(hex: "#2ECC71"))
              Text("LOCAL MARKET FINDER").font(.system(size: 10, weight: .bold)).tracking(3)
                .foregroundColor(Color(hex: "#2ECC71"))
              Text("Find Clean Food Near You").font(
                .system(size: 20, weight: .bold, design: .rounded)
              ).foregroundColor(Palette.text.primary)
            }.padding(.top, 12)
              ...
              HStack(spacing: 8) {
                ForEach(MarketCategory.allCases, id: \.self) { cat in
                  Button {
                    ...
                    .background(
                      Capsule().fill(
                        selectedCategory == cat ? Color(hex: "#2ECC71") : Color.white.opacity(0.06))
                    )
                  }.buttonStyle(PlainButtonStyle())
                }
              }
            ...
            if !locationManager.results.isEmpty {
              Map(
                coordinateRegion: $locationManager.region, annotationItems: locationManager.results
              ) { market in
                MapAnnotation(coordinate: market.coordinate) {
                  VStack(spacing: 2) {
                    Image(systemName: "mappin.circle.fill").font(.system(size: 24)).foregroundColor(
                      Color(hex: "#2ECC71"))
                    Text(market.name).font(.system(size: 8, weight: .bold)).foregroundColor(
                      Palette.text.primary
                    )
            ...
            if locationManager.isSearching {
              HStack(spacing: 10) {
                ...
                Text("Searching nearby...").font(.system(size: 13)).foregroundColor(
                  Palette.text.muted)
              }.padding(.top, 20)
            } else if locationManager.results.isEmpty {
              VStack(spacing: 12) {
                Image(systemName: "location.slash.fill").font(.system(size: 36)).foregroundColor(
                  Palette.text.muted)
                Text("Enable location or search a category").font(
                  .system(size: 14, weight: .medium)
                ).foregroundColor(Palette.text.muted)
              }.padding(.top, 30)
            } else {
              VStack(alignment: .leading, spacing: 8) {
                Text("\(locationManager.results.count) MARKETS FOUND").font(
                  .system(size: 10, weight: .bold)
                ).tracking(1.5).foregroundColor(Color(hex: "#2ECC71"))
            ...
            // Market type tips
            VStack(alignment: .leading, spacing: 12) {
              Text("🧭 WHY EACH MARKET TYPE MATTERS").font(.system(size: 10, weight: .bold))
                .tracking(1).foregroundColor(Palette.accent.gold)
              let marketTypes: [MarketCategory] = MarketCategory.allCases.filter { $0 != .all }
              ForEach(marketTypes, id: \.self) { cat in
                HStack(alignment: .top, spacing: 10) {
                  Text(cat.icon).font(.system(size: 16))
                  VStack(alignment: .leading, spacing: 2) {
                    Text(cat.rawValue).font(.system(size: 13, weight: .bold)).foregroundColor(
                      Palette.text.primary)
                    Text(cat.whyVisit).font(.system(size: 11, weight: .medium)).foregroundColor(
                      Palette.text.muted
                    ).lineSpacing(2)
                  }
                }
      ...
      .toolbar {
        ...
        ToolbarItem(placement: .principal) {
          Text("MARKET FINDER").font(.system(size: 14, weight: .bold, design: .rounded)).tracking(2)
            .foregroundColor(Color(hex: "#2ECC71"))
        }
      }
  ...
  private func marketCard(_ market: MarketResult) -> some View {
    HStack(spacing: 14) {
      ZStack {
        ...
        Image(systemName: "storefront.fill").font(.system(size: 18)).foregroundColor(
          Color(hex: "#2ECC71"))
      }
      VStack(alignment: .leading, spacing: 3) {
        Text(market.name).font(.system(size: 14, weight: .bold, design: .rounded)).foregroundColor(
          Palette.text.primary)
      ...
      if let dist = market.distance {
        VStack(spacing: 2) {
          Text(String(format: "%.1f", dist)).font(
      ...
      Button {
        ...
        Image(systemName: "arrow.triangle.turn.up.right.circle.fill").font(.system(size: 26))
      ...
      RoundedRectangle(cornerRadius: 12).fill(Color.white.opacity(0.04))
        .overlay(
          RoundedRectangle(cornerRadius: 12).stroke(
            Color(hex: "#2ECC71").opacity(0.1), lineWidth: 1)))
  }
...
enum MarketCategory: String, CaseIterable {
  ...
  var whyVisit: String {
    ...
    case .healthFood:
      return "Curated selection of clean supplements, organic staples, and hard-to-find superfoods."
    case .organic:
      return "Certified organic produce without the Whole Foods markup. Often locally sourced."
    case .farmersMarket:
      return
        "Direct from farm = peak freshness, highest nutrition, lowest food miles. Talk to the person who grew it."
    case .asian:
      return
        "Fermented foods (kimchi, miso, natto), fresh herbs, and whole spices at a fraction of health store prices."
    case .mexican:
      return
        "Nopales, jicama, fresh herbs, dried chiles, and traditional foods your abuela knew were medicine."
    case .bulk:
      return
        "Buy exactly what you need. Zero packaging waste. Nuts, seeds, grains, and spices at wholesale prices."
    case .indian:
      return
        "The world's best spice selection. Turmeric, ashwagandha, moringa, plus lentils and rice in bulk."
    case .middleEastern:
      return
        "Tahini, dates, sumac, za'atar, pomegranate, and the Mediterranean diet staples proven to extend life."
    case .wholefoods:
      return
        "Strict quality standards (365 Banned Ingredient list). Use the bulk section and sales to manage cost."
    }
  }
}
  ...
  nonisolated func locationManager(_: CLLocationManager, didUpdateLocations locations: [CLLocation])
  {
    ...
    Task { @MainActor in
      ...
      self.region = MKCoordinateRegion(
        center: loc.coordinate, span: MKCoordinateSpan(latitudeDelta: 0.15, longitudeDelta: 0.15))
    }
  }

  func search(category: MarketCategory, radius _: Double) {
    isSearching = true
    results = []
    ...
    search.start { [weak self] response, _ in
      Task { @MainActor in
        ...
        for item in response.mapItems {
          ...
          let parts: [String?] = [
            item.placemark.thoroughfare, item.placemark.locality, item.placemark.administrativeArea,
          ]
