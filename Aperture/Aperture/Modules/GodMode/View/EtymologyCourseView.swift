...
final class EtymologyProgressManager: ObservableObject {
  ...
  private func loadProgress() {
    guard let data = defaults.data(forKey: progressKey),
      let decoded = try? JSONDecoder().decode(EtymologyProgress.self, from: data)
    else {
      return
    }
    progress = decoded
...
struct EtymologyCourseView: View {
  ...
  private var progressCard: some View {
    ...
    return VStack(spacing: 16) {
      progressCardHeader(completedLessons: completedLessons, totalLessons: totalLessons)
  ...
  private func progressCardHeader(completedLessons: Int, totalLessons: Int) -> some View {
    HStack {
  ...
  @ViewBuilder
  private var earnedGeometriesRow: some View {
  ...
  private func moduleCard(_ module: EtymologyModule, index _: Int) -> some View {
          ...
          ForEach(Array(lesson.quiz.options.enumerated()), id: \.offset) { index, option in
            Button {
              ...
              HStack {
                ...
                if quizAnswer == index {
                  Image(
                    systemName: showQuizResult
                      ? (index == lesson.quiz.correctIndex
                        ? "checkmark.circle.fill" : "xmark.circle.fill") : "circle.fill"
                  )
                  .foregroundColor(
                    showQuizResult
                      ? (index == lesson.quiz.correctIndex ? .green : .red) : Color(hex: "#9C27B0"))
              ...
              .background(
                RoundedRectangle(cornerRadius: 12)
                  .fill(
                    quizAnswer == index
                      ? Color(hex: "#9C27B0").opacity(0.2) : Color.white.opacity(0.05)
                  )
                  .overlay(
                    RoundedRectangle(cornerRadius: 12)
                      .stroke(
                        quizAnswer == index ? Color(hex: "#9C27B0") : Color.clear, lineWidth: 1)
                  )
              )
            }
