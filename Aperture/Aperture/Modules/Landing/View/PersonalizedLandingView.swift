...
// Shows primary path, quick access, continue section, and Dragon wisdom
...
struct PersonalizedLandingView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showDragonCompanion) {
            DragonCompanionView()
        }
    ...
    // MARK: - Dragon Wisdom Section
    
    private var dragonWisdomSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("🐉 \(journeyStore.dragonName.uppercased())'S WISDOM")
                    .font(.system(size: 12, weight: .bold))
                    .foregroundColor(Palette.accent.gold)
                    .tracking(2)
                Spacer()
            ...
            Button(action: { showDragonCompanion = true }) {
                VStack(alignment: .leading, spacing: 12) {
                    // Wisdom quote
                    Text(dailyWisdom)
...
struct QuickAccessButton: View {
    ...
    private func handleTap() {
        ...
        case .currentCourse:
            selectedTab = 3
        case .dragon:
            DragonBubbleManager.shared.openDragonView()
        case .timeline:
            selectedTab = 1
        case .cosmos:
            selectedTab = 2
