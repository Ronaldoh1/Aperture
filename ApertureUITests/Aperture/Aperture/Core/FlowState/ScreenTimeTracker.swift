...
enum AppUsageCategory: String, CaseIterable, Codable {
    ...
    var commonApps: [String] {
        switch self {
        case .socialMedia:
            return ["Instagram", "TikTok", "Twitter", "X", "Facebook", "Snapchat", "LinkedIn", "Threads", "BeReal"]
        case .entertainment:
            return ["YouTube", "Netflix", "Spotify", "Disney+", "Hulu", "HBO Max", "Twitch", "Prime Video"]
        case .productivity:
...
struct AppUsageEntry: Identifiable, Codable {
    ...
    init(id: UUID = UUID(), category: AppUsageCategory, appName: String? = nil, minutesUsed: Int, date: Date = Date(), isManual: Bool = true) {
        self.id = id
...
struct DailyUsageSummary: Identifiable, Codable {
    ...
    init(date: Date, entries: [AppUsageEntry] = [], budgetMinutes: Int = 30) {
        ...
        self.totalMinutes = entries.reduce(0) { $0 + $1.minutesUsed }
        self.socialMediaMinutes = entries.filter { $0.category == .socialMedia }.reduce(0) { $0 + $1.minutesUsed }
        self.budgetMinutes = budgetMinutes
        self.budgetRemaining = max(0, budgetMinutes - socialMediaMinutes)
    }
...
final class ScreenTimeTracker: ObservableObject {
    ...
    private func loadEntries() -> [AppUsageEntry] {
        guard let data = defaults.data(forKey: entriesKey),
              let entries = try? JSONDecoder().decode([AppUsageEntry].self, from: data) else {
            return []
        }
        return entries
    }
    ...
    private func loadWeekSummaries() {
        guard let data = defaults.data(forKey: summariesKey),
              let summaries = try? JSONDecoder().decode([DailyUsageSummary].self, from: data) else {
            weekSummaries = []
            return
        }
    ...
    func generateTodaySummary() {
        ...
        if let existing = weekSummaries.firstIndex(where: { calendar.isDate($0.date, inSameDayAs: today) }) {
        ...
        VStack(spacing: 16) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(tracker.todaySummary?.socialMediaMinutes ?? 0) / \(tracker.socialMediaBudget) min")
                        .font(.title2.bold())
