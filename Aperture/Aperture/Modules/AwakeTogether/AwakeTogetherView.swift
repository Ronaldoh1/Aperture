...
struct AwakeTogetherView: View {
  ...
  private func featuredModuleCard(
    title: String, subtitle: String, icon: String, color: Color, badge: String, destination: AnyView
  ) -> some View {
  ...
  private func moduleCard(
    title: String, subtitle: String, icon: String, color: Color, destination: AnyView
  ) -> some View {
    NavigationLink(destination: destination) {
      VStack(alignment: .leading, spacing: 10) {
        ZStack {
