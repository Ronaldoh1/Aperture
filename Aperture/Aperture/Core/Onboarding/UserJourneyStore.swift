...
enum UserJourneyType: String, Codable, CaseIterable {
    ...
    var dragonGreeting: String {
        switch self {
        case .groundMe: return "I sense you're seeking peace. Let's find solid ground together."
        case .showPatterns: return "A seeker of truth! I'll help you see what others miss."
        case .teachPractice: return "Ready to transform? I'll guide you through the practices."
        case .giveSources: return "A scholar's heart! Let's explore the original sources."
        }
    }
...
struct QuickAccessItem: Codable, Identifiable, Equatable {
    ...
    static let defaultItems: [QuickAccessItem] = [
        QuickAccessItem(id: "practice", title: "Practice", icon: "figure.mind.and.body", destination: .practice),
        QuickAccessItem(id: "course", title: "Course", icon: "book.fill", destination: .currentCourse),
        QuickAccessItem(id: "dragon", title: "Dragon", icon: "flame.fill", destination: .dragon)
    ]
}
...
class UserJourneyStore: ObservableObject {
    ...
    @Published private(set) var dragonName: String = "Dragon"
    ...
    private let keys = (
        ...
        dragonName: "aperture.dragon.name.v1",
    ...
    private func load() {
        ...
        if let name = UserDefaults.standard.string(forKey: keys.dragonName), !name.isEmpty {
            dragonName = name
        }
    ...
    func completeOnboarding(journeyType: UserJourneyType, dragonName: String?) {
        self.hasCompletedOnboarding = true
        self.journeyType = journeyType
        
        if let name = dragonName, !name.isEmpty {
            self.dragonName = name
        }
        
        // Set default quick access based on journey
        self.quickAccessItems = defaultQuickAccess(for: journeyType)
        ...
        UserDefaults.standard.set(self.dragonName, forKey: keys.dragonName)
    ...
    func completeOnboarding() {
        self.hasCompletedOnboarding = true
        self.journeyType = .showPatterns // Default: seeker journey
    ...
    private func defaultQuickAccess(for journey: UserJourneyType) -> [QuickAccessItem] {
        ...
        case .showPatterns:
            return [
                QuickAccessItem(id: "timeline", title: "Timeline", icon: "clock.arrow.circlepath", destination: .timeline),
                QuickAccessItem(id: "cosmos", title: "Cosmos", icon: "globe.americas.fill", destination: .cosmos),
                QuickAccessItem(id: "dragon", title: "Dragon", icon: "flame.fill", destination: .dragon)
            ]
        case .teachPractice:
            return [
        ...
        case .giveSources:
            return [
                QuickAccessItem(id: "alexandria", title: "Library", icon: "books.vertical.fill", destination: .alexandria),
                QuickAccessItem(id: "timeline", title: "Timeline", icon: "clock.arrow.circlepath", destination: .timeline),
                QuickAccessItem(id: "dragon", title: "Dragon", icon: "flame.fill", destination: .dragon)
            ]
        }
    ...
    func resetForDebug() {
        ...
        UserDefaults.standard.removeObject(forKey: keys.dragonName)
