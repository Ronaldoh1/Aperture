...
struct TimelineEraCategoryBar: View {
  ...
  private func eraCategoryChip(
    label: String, icon: String, color: Color, isSelected: Bool, action: @escaping () -> Void
  ) -> some View {
    Button(action: {
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      VStack(spacing: 6) {
        ...
        // Category dots
        ForEach(Array(uniqueCategories.enumerated()), id: \.element) { _, category in
