...
final class ErrorHandler: ObservableObject {
    ...
    func handle(_ error: Error, context: String = "", file: String = #file, line: Int = #line, function: String = #function) {
    ...
    /// Async safe execute
    func safeExecuteAsync<T>(_ context: String, defaultValue: T, operation: () async throws -> T) async -> T {
        do {
...
extension JSONEncoder {
    ...
    func safeEncode<T: Encodable>(_ value: T, context: String = "") -> Data? {
        do {
            ...
            ErrorHandler.shared.handle(
                AppError.encodingFailed(type: String(describing: type(of: value)), reason: error.localizedDescription),
                context: context
            )
            return nil
        }
...
@MainActor
func withErrorBoundary<T>(_ context: String, defaultValue: T, operation: () async throws -> T) async -> T {
    do {
        return try await operation()
    } catch {
        ErrorHandler.shared.handle(error, context: context)
