...
struct ApertureIntroductionView: View {
    ...
    private var bottomControls: some View {
        VStack(spacing: 24) {
            ...
            if currentPage == pages.count - 1 {
                HStack(spacing: 6) {
                    Image(systemName: "iphone.radiowaves.left.and.right").font(.system(size: 11)).foregroundColor(.cyan.opacity(0.4))
                    Text("Shake your phone anytime to send feedback").font(.system(size: 11, weight: .medium)).foregroundColor(.white.opacity(0.25))
                }
