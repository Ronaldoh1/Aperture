...
struct AlexandriaCourseView: View {
  ...
  private var headerSection: some View {
    VStack(spacing: 20) {
      ...
      HStack(spacing: 16) {
        StatPill(
          icon: "square.stack.3d.up", value: "\(course.moduleCount)", label: "Modules",
          color: course.color)
        StatPill(
          icon: "text.book.closed", value: "\(course.lessonCount)", label: "Lessons",
          color: course.color)
        StatPill(
          icon: "clock", value: "\(Int(course.estimatedHours))h", label: "Duration",
          color: course.color)
      }
      ...
      if course.isUltraPremium {
        premiumBadge(
          text: "🔒 ULTRA PREMIUM — Complete ALL modules to unlock", color: Palette.accent.gold)
      } else if course.isPremium {
        premiumBadge(text: "☀️ PREMIUM FEATURE", color: Palette.primary.orange)
      } else if course.tier > 1 {
        premiumBadge(
          text: "TIER \(course.tier) — THE \(tierName(course.tier))", color: course.color)
      }
    }
  }
    ...
    Button {
      ...
      .background(
        RoundedRectangle(cornerRadius: 16)
          ...
          .overlay(
            RoundedRectangle(cornerRadius: 16).stroke(course.color.opacity(0.12), lineWidth: 1))
      )
    }
  ...
  private var dragonCommentSection: some View {
    ...
    .background(
      RoundedRectangle(cornerRadius: 14)
        ...
        .overlay(
          RoundedRectangle(cornerRadius: 14).stroke(Palette.accent.gold.opacity(0.2), lineWidth: 1))
    )
  }
...
struct ACLessonView: View {
  ...
  private func symbolDecoderView(_ elements: [(symbol: String, fear: String, truth: String)])
    -> some View
  {
    VStack(alignment: .leading, spacing: 12) {
      HStack(spacing: 8) {
      ...
      ForEach(Array(elements.enumerated()), id: \.offset) { _, element in
        SymbolDecoderCard(
          symbol: element.symbol, fear: element.fear, truth: element.truth, color: color)
      }
    }
    .padding(16)
    .background(
  ...
  private func timelineView(_ events: [(year: String, event: String, significance: String)])
    -> some View
  {
    VStack(alignment: .leading, spacing: 12) {
      HStack(spacing: 8) {
  ...
  private func comparisonView(
    left: String, right: String, rows: [(leftVal: String, rightVal: String)]
  ) -> some View {
    VStack(alignment: .leading, spacing: 12) {
      HStack {
        Text(left)
      ...
      RoundedRectangle(cornerRadius: 14)
        ...
        .overlay(
          RoundedRectangle(cornerRadius: 14).stroke(
            Palette.primary.violet.opacity(0.2), lineWidth: 1))
    )
  }
        ...
        RoundedRectangle(cornerRadius: 10)
          ...
          .overlay(
            RoundedRectangle(cornerRadius: 10).stroke(
              color.opacity(isRevealed ? 0.3 : 0.1), lineWidth: 1))
      )
    }
    .buttonStyle(PlainButtonStyle())
  }
