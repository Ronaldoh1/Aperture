...
final class FlowOrchestrator: ObservableObject {
  ...
  private func loadUserProfile() {
    guard let data = defaults.data(forKey: profileKey),
      let decoded = try? JSONDecoder().decode(FlowUserProfile.self, from: data)
    else {
      return
    }
    userProfile = decoded
  ...
  func runDistractionAudit() -> DistractionAudit {
    ...
    if socialMediaMinutes > 30 {
      let suggestion = DistractionSuggestion(
        ...
        description:
          "You're spending \(socialMediaMinutes) min/day. Try starting with \(max(10, socialMediaMinutes / 2)) min.",
    ...
    if profile.averageMeetingHoursPerDay > 3 {
      suggestions.append(
        DistractionSuggestion(
          ...
          title: "Meeting Diet",
          description:
            "\(Int(profile.averageMeetingHoursPerDay))+ hours in meetings daily. Decline or shorten.",
  ...
  private func loadWeeklyPlan() {
    guard let data = defaults.data(forKey: weeklyPlanKey),
      let decoded = try? JSONDecoder().decode(WeeklyFlowPlan.self, from: data)
    else {
      return
    }
    weeklyPlan = decoded
