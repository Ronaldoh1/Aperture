...
class PostAnalyzer: ObservableObject {
  ...
  private let triggerPatterns:
    [(
      pattern: String, category: HateTriggerCategory, severity: Int, explanation: String,
      alternative: String
    )] = [
      // Divisive
      (
        "all \\w+ are", .generalizing, 4, "Generalizing about entire groups creates division",
        "Some people who..."
      ),
      (
        "\\b(libtard|trumptard|snowflake|nazi)\\b", .dehumanizing, 5,
        "Name-calling dehumanizes and shuts down dialogue", "Someone with different views"
      ),
      ("you people", .divisive, 3, "Creates an us vs. them dynamic", "Some folks"),
      (
        "always|never", .generalizing, 2, "Absolute statements are rarely accurate",
        "Often/Sometimes"
      ),

      // Inflammatory
      (
        "wake up sheeple", .condescending, 4, "Implies others are stupid for disagreeing",
        "Consider looking into..."
      ),
      (
        "do your research", .condescending, 3, "Often dismissive rather than helpful",
        "Here's a source I found interesting..."
      ),
      (
        "i can't believe anyone", .condescending, 2, "Dismisses others' lived experiences",
        "I'm curious why some people..."
      ),

      // Fear-mongering
      (
        "they're coming for", .fearmongering, 4, "Creates fear without evidence",
        "There's concern about..."
      ),
      (
        "the end of", .fearmongering, 3, "Catastrophizing reduces nuanced thinking",
        "Significant changes to..."
      ),
      (
        "destroy(ing)?|ruin(ing)?", .inflammatory, 3,
        "Extreme language inflames rather than informs", "Negatively impact"
      ),
    ]
  ...
  func analyze(_ text: String) async -> PostAnalysis {
    ...
    let tagResult = sentimentTagger.tag(
