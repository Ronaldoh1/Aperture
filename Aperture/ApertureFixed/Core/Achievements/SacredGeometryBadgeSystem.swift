...
enum SacredGeometryShape: String, CaseIterable, Codable {
    ...
    var gnosticMeaning: String {
        switch self {
        case .vesicaPiscis:
            return "The womb of creation. Where two circles intersect, new reality is born. The portal between worlds."
        case .seedOfLife:
            return "The seven days of creation encoded in geometry. Each circle represents a stage of cosmic manifestation."
        case .flowerOfLife:
            return "The blueprint of the universe. All forms emerge from this pattern. Contains the Platonic solids within."
        case .hexagram:
            return "As above, so below. The union of masculine and feminine principles. The seal of Solomon."
        case .metatronsCube:
            return "The throne of God. Contains all Platonic solids. The key to understanding matter and spirit."
        case .infinityLoop:
            return "The eternal return. No beginning, no end. The continuous flow of consciousness."
        case .tetrahedron:
            return "Fire element. The simplest Platonic solid. Represents divine masculine energy and willpower."
        case .hexahedron:
        ...
        case .torus:
            return "The shape of the universe. Energy flows in, folds through center, emerges transformed."
        case .goldenSpiral:
            return "The fingerprint of God. Found in galaxies, shells, and DNA. The pattern of natural growth."
        case .treeOfLife:
            return "The map of consciousness. Ten spheres connected by 22 paths. The Kabbalistic key to creation."
        case .sriYantra, .shriYantra:
            return "The supreme yantra. Nine interlocking triangles create 43 smaller triangles. The cosmos in geometry."
        case .merkabaField:
            return "The light body vehicle. Counter-rotating tetrahedrons activate dimensional travel."
        case .cosmicEgg:
            return "The primordial form. All potential contained before manifestation. You have returned to source."
        case .yantra64:
            return "The 64 tetrahedron grid. The structure of the vacuum. Nassim Haramein's unified field geometry."
        case .platoniSolids:
            return "The five perfect forms. Earth, Water, Fire, Air, and Ether. The building blocks of reality."
        }
    }
...
final class SacredBadgeManager: ObservableObject {
    ...
    private func loadBadges() {
        guard let data = defaults.data(forKey: badgesKey),
              let decoded = try? JSONDecoder().decode([SacredBadge].self, from: data) else {
            return
        }
...
struct SacredGeometryView: View {
    let shape: SacredGeometryShape
    var lineColor: Color = Color(hex: "#FFD700")
...
struct VesicaPiscisShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addEllipse(in: CGRect(x: center.x - offset - radius, y: center.y - radius, width: radius * 2, height: radius * 2))
        path.addEllipse(in: CGRect(x: center.x + offset - radius, y: center.y - radius, width: radius * 2, height: radius * 2))
...
struct FlowerOfLifeShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for i in 0..<6 {
            ...
            path.addEllipse(in: CGRect(x: x1 - radius, y: y1 - radius, width: radius * 2, height: radius * 2))
            
            let dist = radius * sqrt(3)
            let x2 = center.x + dist * cos(angle2)
            let y2 = center.y + dist * sin(angle2)
            path.addEllipse(in: CGRect(x: x2 - radius, y: y2 - radius, width: radius * 2, height: radius * 2))
        }
...
struct MetatronsCubeShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for i in 0..<6 {
            ...
            points.append(CGPoint(x: center.x + radius * 2 * cos(angle), y: center.y + radius * 2 * sin(angle)))
        }
        
        for i in 0..<6 {
            let angle = CGFloat(i) * .pi / 3 - .pi / 2
            points.append(CGPoint(x: center.x + outerRadius * cos(angle), y: center.y + outerRadius * sin(angle)))
        }
...
struct InfinityShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.move(to: center)
        path.addCurve(to: CGPoint(x: center.x - width, y: center.y), control1: CGPoint(x: center.x - width * 0.5, y: center.y - height), control2: CGPoint(x: center.x - width, y: center.y - height * 0.5))
        path.addCurve(to: center, control1: CGPoint(x: center.x - width, y: center.y + height * 0.5), control2: CGPoint(x: center.x - width * 0.5, y: center.y + height))
        path.addCurve(to: CGPoint(x: center.x + width, y: center.y), control1: CGPoint(x: center.x + width * 0.5, y: center.y + height), control2: CGPoint(x: center.x + width, y: center.y + height * 0.5))
        path.addCurve(to: center, control1: CGPoint(x: center.x + width, y: center.y - height * 0.5), control2: CGPoint(x: center.x + width * 0.5, y: center.y - height))
...
struct PlatonicSolidShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        case .hexahedron:
            ...
            path.addRect(CGRect(x: center.x - halfSize, y: center.y - halfSize, width: halfSize * 2, height: halfSize * 2))
        ...
        default:
            path.addEllipse(in: CGRect(x: center.x - size, y: center.y - size, width: size * 2, height: size * 2))
        }
...
struct TorusShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addEllipse(in: CGRect(x: center.x - outerRadius, y: center.y - outerRadius * 0.5, width: outerRadius * 2, height: outerRadius))
        path.addEllipse(in: CGRect(x: center.x - innerRadius, y: center.y - innerRadius * 0.3, width: innerRadius * 2, height: innerRadius * 0.6))
...
struct GoldenSpiralShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for _ in 0..<50 {
            ...
            path.addLine(to: CGPoint(x: center.x + currentRadius * cos(currentAngle), y: center.y + currentRadius * sin(currentAngle)))
        }
...
struct BadgeTreeOfLifeShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let connections = [(0,1), (0,2), (1,3), (2,4), (1,5), (2,5), (3,5), (4,5), (3,6), (4,7), (5,8), (6,8), (7,8), (8,9)]
...
struct MerkabaShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addEllipse(in: CGRect(x: center.x - size * 0.3, y: center.y - size * 0.3, width: size * 0.6, height: size * 0.6))
...
struct CosmicEggShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addEllipse(in: CGRect(x: center.x - width, y: center.y - height, width: width * 2, height: height * 2))
        ...
        while radius < width * 0.8 {
            angle += 0.2
            radius += 1.5
            path.addLine(to: CGPoint(x: center.x + radius * cos(angle), y: center.y + radius * sin(angle) * 1.2))
        }
...
struct BadgeCollectionView: View {
    ...
    @AppStorage("hasSeenBadgeCollectionIntro") private var hasSeenIntro = false
    ...
    private var allPossibleBadges: [SacredGeometryShape] {
        SacredGeometryShape.allCases
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showFirstTimeGuide) {
                BadgeCollectionOnboarding(isPresented: $showFirstTimeGuide)
            }
            .onAppear {
                if !hasSeenIntro {
    ...
    private var heroSection: some View {
        VStack(spacing: 16) {
            ...
            Text("Sacred Geometry Badges")
                .font(.title2.bold())
                .foregroundColor(.white)
            
            Text("Earn sacred symbols as you explore consciousness, complete courses, and unlock hidden knowledge. Each badge represents a milestone in your awakening journey.")
                .font(.subheadline)
        ...
        VStack(spacing: 16) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(manager.uniqueShapesEarned().count) of \(allPossibleBadges.count) Shapes Discovered")
                        .font(.headline)
                        .foregroundColor(.white)
                }
                ...
                ZStack {
                    ...
                    Circle()
                        .trim(from: 0, to: manager.collectionProgress)
                        .stroke(
                            LinearGradient(
                    ...
                    Text("\(Int(manager.collectionProgress * 100))%")
                        .font(.caption.bold())
            ...
            // Stats row
            HStack(spacing: 20) {
                BadgeStatBox(title: "Total Earned", value: "\(manager.totalBadgeCount)", icon: "star.fill")
                BadgeStatBox(title: "Unique Shapes", value: "\(manager.uniqueShapesEarned().count)", icon: "seal")
                BadgeStatBox(title: "Rarity Score", value: rarityScore, icon: "sparkles")
            }
        }
        .padding()
        .background(
    ...
    private var rarityScore: String {
        let unique = manager.uniqueShapesEarned()
        let totalComplexity = unique.reduce(0) { $0 + $1.complexity }
    ...
    private var lockedBadgesSection: some View {
        let earnedShapes = Set(manager.earnedBadges.map { $0.shape })
        let lockedShapes = allPossibleBadges.filter { !earnedShapes.contains($0) }
        ...
        return VStack(alignment: .leading, spacing: 16) {
            HStack {
                ...
                Text("\(lockedShapes.count) remaining")
                    .font(.caption)
                    .foregroundColor(.white.opacity(0.4))
            }
            
            LazyVGrid(columns: columns, spacing: 16) {
                ForEach(lockedShapes, id: \.self) { shape in
                    LockedBadgeCard(shape: shape)
                }
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                EarnMethodRow(
                    ...
                    description: "Each course lesson awards a unique sacred geometry"
                )
                
                EarnMethodRow(
                    icon: "flame.fill",
                    title: "Maintain Streaks",
                    description: "Daily learning streaks unlock special badges"
                )
                
                EarnMethodRow(
                    icon: "star.fill",
                    title: "Explore Modules",
                    description: "Discover hidden knowledge across all realms"
                )
                
                EarnMethodRow(
                    icon: "person.2.fill",
                    title: "Share with Friends",
                    description: "Invite others and both receive rare geometries"
                )
            }
        }
        .padding()
        ...
        Button(action: onTap) {
            VStack(spacing: 8) {
                ZStack {
                    // Glow effect
                    Circle()
                        .fill(badge.lineColor.opacity(0.2))
                ...
                // Complexity stars
                HStack(spacing: 2) {
                    ForEach(0..<badge.shape.complexity, id: \.self) { _ in
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .stroke(badge.lineColor.opacity(0.3), lineWidth: 1)
                    )
            )
        }
    }
...
struct LockedBadgeCard: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            HStack(spacing: 2) {
                ForEach(0..<shape.complexity, id: \.self) { _ in
                    Circle()
...
struct BadgeCollectionOnboarding: View {
        ...
        ZStack {
            ...
            VStack(spacing: 30) {
                ...
                HStack(spacing: 8) {
                    ForEach(0..<3, id: \.self) { i in
                        Circle()
                            .fill(currentPage == i ? Color(hex: "#FFD700") : Color.white.opacity(0.3))
                ...
                TabView(selection: $currentPage) {
                    OnboardingPage(
                        ...
                        description: "As you explore Aperture, you'll earn sacred geometric symbols. Each shape holds ancient wisdom and marks your progress on the path of awakening."
                    )
                    .tag(0)
                    ...
                    OnboardingPage(
                        ...
                        title: "Earn Through Learning",
                        description: "Complete course lessons, explore modules, maintain streaks, and share with friends. Each action unlocks unique geometries from simple circles to complex sacred forms."
                    )
                    .tag(1)
                    ...
                    OnboardingPage(
                        ...
                        description: "Tap any badge to customize its colors, save it to your photos, or share it on social media. Your collection is yours to display proudly."
                    )
                    .tag(2)
                }
                ...
                Button {
                    ...
                    Text(currentPage < 2 ? "Next" : "Start Collecting")
                        .font(.headline)
                        .foregroundColor(.black)
                        .frame(maxWidth: .infinity)
                        .padding()
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            ZStack {
                ForEach(0..<3, id: \.self) { i in
                    Circle()
                        .stroke(iconColor.opacity(0.2 - Double(i) * 0.05), lineWidth: 1)
...
struct BadgeDetailView: View {
    ...
    @State private var lineColor: Color = Color(hex: "#FFD700")
    @State private var backgroundColor: Color = Color(hex: "#0a0a1a")
    @State private var glowColor: Color = Color(hex: "#7C4DFF")
    ...
    private func shareImage() {
        ...
        if let uiImage = renderer.uiImage {
            let activityVC = UIActivityViewController(
                activityItems: [uiImage, "My Sacred Geometry Badge: \(badge.shape.rawValue) from Aperture 🔮"],
