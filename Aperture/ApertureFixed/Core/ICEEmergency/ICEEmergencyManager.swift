...
final class ICEEmergencyManager: ObservableObject {
    ...
    @AppStorage("iceContactsData") private var contactsData: Data = Data()
...
struct ICEEmergencyView: View {
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // Tab Selector
                    HStack(spacing: 0) {
                        TabButton(title: manager.currentLanguage == .english ? "Rights" : "Derechos", isSelected: selectedTab == 0) { selectedTab = 0 }
                        TabButton(title: manager.currentLanguage == .english ? "Phrases" : "Frases", isSelected: selectedTab == 1) { selectedTab = 1 }
                        TabButton(title: manager.currentLanguage == .english ? "Contacts" : "Contactos", isSelected: selectedTab == 2) { selectedTab = 2 }
                        TabButton(title: manager.currentLanguage == .english ? "Hotlines" : "Líneas", isSelected: selectedTab == 3) { selectedTab = 3 }
                    }
    ...
    private var contactsTab: some View {
        ScrollView {
            VStack(spacing: 12) {
                Text(manager.currentLanguage == .english ? "📞 EMERGENCY CONTACTS" : "📞 CONTACTOS DE EMERGENCIA")
                    .font(.headline)
                    .foregroundStyle(.white)
                    .padding(.top)
                
                if manager.emergencyContacts.isEmpty {
                    Text(manager.currentLanguage == .english ? "No contacts added. Add contacts in Settings." : "No hay contactos. Agregue contactos en Configuración.")
